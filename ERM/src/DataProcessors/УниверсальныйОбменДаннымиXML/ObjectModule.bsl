...
// Добавляет новый xml-УзелИнформационнойБазы к указанному узлу-родителю.
// Функция может быть использована в обработчиках событий, программный код 
// которых хранится в правила обмена данными. Вызывается методом Выполнить().
// Сообщение "Не обнаружено ссылок на функцию" при проверке конфигурации 
// не является ошибкой проверки конфигурации.
...
//
// Возвращаемое значение:
//  Новый xml-УзелИнформационнойБазы, добавленный к указанному узлу-родителю.
//
...
// Копирует указанный xml-УзелИнформационнойБазы.
// Функция может быть использована в обработчиках событий, программный код 
// которых хранится в правила обмена данными. Вызывается методом Выполнить().
// Сообщение "Не обнаружено ссылок на функцию" при проверке конфигурации 
// не является ошибкой проверки конфигурации.
...
// Параметры: 
//  УзелРодитель   - xml-УзелИнформационнойБазы-родитель.
//  УзелИнформационнойБазы           - xml - подчиняемый УзелИнформационнойБазы.
//
...
//                             только формируется xml-УзелИнформационнойБазы ссылки.
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		ВременныйФайлЗаписей.ЗаписатьСтроку("</" + ИмяГоловногоУзла + ">"); // закрыть УзелИнформационнойБазы
...
//  (корневой УзелИнформационнойБазы), файл закрывается.
//
