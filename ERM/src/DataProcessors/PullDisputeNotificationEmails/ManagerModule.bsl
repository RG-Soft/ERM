...
Функция ОбработатьПисьма(Письма, ДанныеУчетнойЗаписи) Экспорт
	...
	Для Каждого Письмо Из Письма Цикл
		...
		Если Письмо.ДатаПолучения > МаксимальнаяДата Тогда
			 МаксимальнаяДата = Письмо.ДатаПолучения;
		КонецЕсли;
		
	КонецЦикла;
	
	НЗ = РегистрыСведений.ДатыПоследнейЗагрузкиПочтовыхСообщений.СоздатьНаборЗаписей();
	НЗ.Отбор.УчетнаяЗапись.Установить(ДанныеУчетнойЗаписи.Ссылка);
	
	НоваяЗапись = НЗ.Добавить();
	НоваяЗапись.УчетнаяЗапись = ДанныеУчетнойЗаписи.Ссылка;
	НоваяЗапись.ДатаЗагрузкиПисем = МаксимальнаяДата;
...
Процедура ОбработатьПисьмо(Письмо, ДанныеУчетнойЗаписи) Экспорт
	...
	Для Каждого InvoiceProblem Из InvoiceProblems Цикл
		...
		НовыйКомментарийИнвойса.CustomerInputDetails = Формат(ДатаНовогоКомментария, "ДФ=MM/dd/yyyy") + " - " + СозданноеПисьмо.Текст;
