...
//  Приемник				 - xml-УзелИнформационнойБазы объекта приемника.
...
//  УзелПредка				 - xml-УзелИнформационнойБазы предка объекта приемника.
...
//
// Возвращаемое значение:
//  xml-УзелИнформационнойБазы ссылки или значение приемника.
//
//
Функция ВыгрузитьПоПравилу(Источник					= Неопределено,
	...
	Если НеНужноВыгружатьПоСоответствиюЗначений Тогда
		...
		Если ПКО.СвойстваПоиска.Количество() > 0 
			...
			//	Формируем УзелИнформационнойБазы ссылки
...
// Создает новый xml-УзелИнформационнойБазы
// Функция может быть использована в обработчиках событий, программный код 
// которых хранится в правила обмена данными. Вызывается методом Выполнить().
//
// Параметры: 
...
// Подчиняет xml-УзелИнформационнойБазы указанному узлу-родителю.
//
// Параметры: 
//  УзелРодитель   - xml-УзелИнформационнойБазы-родитель.
//  УзелИнформационнойБазы           - подчиняемый УзелИнформационнойБазы.
//
...
//
// Параметры: 
//  УзелИнформационнойБазы           - xml-УзелИнформационнойБазы
//  Имя            - имя атрибута.
...
// Сохраняет в файл указанный xml-УзелИнформационнойБазы.
//
// Параметры:
//  УзелИнформационнойБазы           - xml-УзелИнформационнойБазы, сохраняемый в файл.
//
...
Процедура ЗагрузитьПравилоКонвертации(ПравилаОбмена, ЗаписьXML)
	...
	// закрываем УзелИнформационнойБазы
...
Процедура НачатьЧтениеСообщения(ЧтениеСообщения, АнализДанных = Ложь)
	...
	// Устанавливаем блокировку на УзелИнформационнойБазы отправителя.
	Попытка
...
// Читает из файла УзелИнформационнойБазы свойства объекта, устанавливает значение свойства.
//
// Параметры:
...
//  УзелКоллекцииСвойств - xml-УзелИнформационнойБазы коллекции свойств.
// 
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		ВременныйФайЗаписей.ЗаписатьСтроку("</" + ИмяГоловногоУзла + ">"); // закрыть УзелИнформационнойБазы
...
Процедура ВыгрузитьСвойства(Источник, 
	...
	Для каждого ПКС Из КоллекцияПКС Цикл
		...
		Если ПКС.УпрощеннаяВыгрузкаСвойства Тогда
			 ...
			 //	Создаем УзелИнформационнойБазы свойства
			УзелКоллекцииСвойств.ЗаписатьНачалоЭлемента("Свойство");
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	Если Не ЗначениеЗаполнено(УзелОбменаПолучатель)
		...
		СтрокаСообщения = НСтр("ru = 'Не найден УзелИнформационнойБазы обмена для загрузки данных. План обмена: %1, Код: %2'");
...
Функция ВыгрузитьИнформациюСопоставленияОбъектов(УзелИнформационнойБазы)
	...
	|ГДЕ
	|	СоответствияОбъектовИнформационныхБазИзменения.УзелИнформационнойБазы = &УзелИнформационнойБазы
	|";
