...
Процедура ПрочитатьФайлНаСервере(Результат)
	...
	ИначеЕсли НЕ ВсеГеомаркеты И GeoMarket = Справочники.GeoMarkets.ПустаяСсылка() Тогда
	...
	Иначе
		...
		МассивAccountUnit = Новый Массив;

		Если НЕ ВсеГеомаркеты Тогда
			...
			Для Индекс = 0 По МассивAccountUnit.Количество() - 1 Цикл
				МассивAccountUnit[Индекс] = СокрЛП(МассивAccountUnit[Индекс]);
			КонецЦикла;
...
Функция ПроверитьКорректностьДанных(ТаблицаДанных, МассивAccountUnit)
	...
	Если НЕ ВсеГеомаркеты Тогда
		...
		Запрос.Текст = Запрос.Текст + " И ТаблицаИсходныхДанных.AcctUnit В(&AcctUnit)";
...
&НаКлиенте
Процедура ВсеГеомаркетыПриИзменении(Элемент)
	
	Если ВсеГеомаркеты Тогда
		Элементы.GeoMarket.ТолькоПросмотр = Истина;
		GeoMarket = ПредопределенноеЗначение("Справочник.GeoMarkets.ПустаяСсылка");
	Иначе
		Элементы.GeoMarket.ТолькоПросмотр = Ложь;
	КонецЕсли;
...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных, МассивAccountUnit)
	...
	|	ВЫРАЗИТЬ( ТаблицаИсходныхДанных.Invoice КАК Строка(20)) КАК Invoice,
	...
	|ГДЕ
	|	КлючиИнвойсов.Source = Значение(Перечисление.ТипыСоответствий.Lawson)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	КлючиMemo.Memo.Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Lawson)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	КэшГеоМаркеты = РезультатЗапроса[6].Выгрузить();
	...
	ВыборкаДанные = РезультатЗапроса[7].Выбрать();
	...
	Пока ВыборкаДанные.Следующий() Цикл
		...
		Если ВыборкаДанные.TransType = "I" ИЛИ ВыборкаДанные.TransType = "C" ИЛИ ВыборкаДанные.TransType = "M" ИЛИ ВыборкаДанные.TransType = "D" Тогда
			СтруктураПоискаInvoice.ArInvoice = СокрЛП(ВыборкаДанные.Invoice);
			...
			Если СтрокиИнвойса.Количество() > 0 Тогда
				ПолеИнвойс = СтрокиИнвойса[0].Invoice;
				ТипТранзакции = "I";
			Иначе
				СтруктураПоискаМемо.ArInvoice =  СокрЛП(ВыборкаДанные.Invoice);
				СтруктураПоискаМемо.Client = ВыборкаДанные.Client;
				СтруктураПоискаМемо.Company = ВыборкаДанные.Company;
				СтрокиМемо = КэшМемо.НайтиСтроки(СтруктураПоискаМемо);
				Если СтрокиМемо.Количество() > 0 Тогда
					ПолеИнвойс = СтрокиМемо[0].Memo;
					ТипТранзакции = "M";
				Иначе
				ПолеИнвойс = ВыборкаДанные.Invoice;
				ТипТранзакции = ВыборкаДанные.TransType;
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ВыборкаДанные.TransType = "P" Тогда
			СтруктураПоискаBatch.ARBatchNbr = ВыборкаДанные.ArBatchNbr;
			...
			Если СтрокиCashBatch.Количество() > 0 Тогда
				...
				ТипТранзакции = "P";
			Иначе
				ПолеИнвойс = ВыборкаДанные.ArBatchNbr;
				ТипТранзакции = "P";
			КонецЕсли;
		Иначе
			ПолеИнвойс = ВыборкаДанные.Invoice;
