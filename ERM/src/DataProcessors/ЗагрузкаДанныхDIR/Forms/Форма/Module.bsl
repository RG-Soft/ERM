...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	|	врТЗТаблицаДанных.InvoiceDate КАК InvoiceDate,
	...
	|	ДоговорыКонтрагентов.Владелец КАК Client
	|ИЗ
	...
	|	И ДоговорыКонтрагентов.Наименование В
	|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|				ВТ_Итоговая.Agreement
	|			ИЗ
	...
	|	ВТ_Итоговая.PT_DaysFrom КАК PTDaysFrom,
	|	ВТ_Итоговая.PT_DaysTo КАК PTDaysTo,
	...
	|	ВТ_Итоговая.CustomerNumber КАК ClientID,
	...
	КэшДоговоров = РезультатЗапроса[4].Выгрузить();
	КэшДоговоров.Индексы.Добавить("Наименование, Client");
	...
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаDIR;
	Если Source = Перечисления.ТипыСоответствий.Radius Тогда
	...
	СтруктураПоискаДоговора = Новый Структура("Наименование, Client");
	...
	Пока ВыборкаДанные.Следующий() Цикл
		...
		ДоговорИнвойса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекИнвойс, "Contract");
		
		ИнвойсОбъект = ТекИнвойс.ПолучитьОбъект();

		Если НЕ ЗначениеЗаполнено(ДоговорИнвойса) Тогда
			
			СтруктураПоискаДоговора.Наименование = ВыборкаДанные.Agreement;
			СтруктураПоискаДоговора.Client = ВыборкаДанные.Client;
			СтрокиКэшаДоговоров = КэшДоговоров.НайтиСтроки(СтруктураПоискаДоговора);
			
			Если СтрокиКэшаДоговоров.Количество() = 0 Тогда
				ДоговорИнвойса = СоздатьДоговор(ВыборкаДанные);
				СтрокаКэшаДоговоров = КэшДоговоров.Добавить();
				СтрокаКэшаДоговоров.Наименование = ВыборкаДанные.Agreement;
				СтрокаКэшаДоговоров.Client = ВыборкаДанные.Client;
				СтрокаКэшаДоговоров.Договор = ДоговорИнвойса;
			Иначе
				ДоговорИнвойса = СтрокиКэшаДоговоров[0].Договор;
			КонецЕсли;
			
			ИнвойсОбъект.Contract = ДоговорИнвойса;
			
		КонецЕсли;
		
		ОбновитьДанныеДоговора(ДоговорИнвойса, ВыборкаДанные);
...
&НаСервере
Функция СоздатьДоговор(ВыборкаДанные)
	
	ДоговорОбъект = Справочники.ДоговорыКонтрагентов.СоздатьЭлемент();
	ДоговорОбъект.PTDaysFrom = ВыборкаДанные.PTDaysFrom;
	ДоговорОбъект.СрокОплаты = ВыборкаДанные.PTDaysTo;
	ДоговорОбъект.Дата = ТекущаяДата();
	ДоговорОбъект.ВидДоговора = Перечисления.ВидыДоговоровКонтрагентов.СПокупателем;
	ДоговорОбъект.Наименование = ВыборкаДанные.Agreement;
	ДоговорОбъект.Владелец = ВыборкаДанные.Client;
	ДоговорОбъект.Trigger = Справочники.TriggerTypes.НайтиПоНаименованию(ВыборкаДанные.Trigger,Истина);
	ДоговорОбъект.УстановленСрокОплаты = Истина;
...
&НаСервере
Процедура ОбновитьДанныеДоговора(Договор, ВыборкаДанные)
	
	ДоговорОбъект = Договор.ПолучитьОбъект();
	РГСофтКлиентСервер.УстановитьЗначение(ДоговорОбъект.PTDaysFrom, ВыборкаДанные.PTDaysFrom);
	РГСофтКлиентСервер.УстановитьЗначение(ДоговорОбъект.СрокОплаты, ВыборкаДанные.PTDaysTo);
	РГСофтКлиентСервер.УстановитьЗначение(ДоговорОбъект.УстановленСрокОплаты, Истина);
	РГСофтКлиентСервер.УстановитьЗначение(ДоговорОбъект.Trigger , Справочники.TriggerTypes.НайтиПоНаименованию(ВыборкаДанные.Trigger,Истина));
...
Процедура СторнироватьДвиженияТекущегоМесяца()
	...
	Запрос.Текст = 
		...
		|	-InvoicedDebtsОбороты.AmountОборот КАК Amount,
		|	-InvoicedDebtsОбороты.BaseAmountОборот КАК BaseAmount,
		|	&КонецПериода КАК Период
		|ИЗ
		|	РегистрНакопления.InvoicedDebts.Обороты(&НачалоПериода, &КонецПериода, , Invoice.Source = &Source) КАК InvoicedDebtsОбороты";
	...
	Если ТЗ_ДвиженияТекущегоМесяца.Количество() > 0 Тогда
		...
		ДокОбъект.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаDIR;
		Если Source = Перечисления.ТипыСоответствий.Radius Тогда
			ДокОбъект.Комментарий = "Списание текущих оборотов DIR по Radius";
		ИначеЕсли Source = Перечисления.ТипыСоответствий.Geofit Тогда
			ДокОбъект.Комментарий = "Списание текущих оборотов DIR по Geofit";
		ИначеЕсли Source = Перечисления.ТипыСоответствий.MFG Тогда
			ДокОбъект.Комментарий = "Списание текущих оборотов DIR по MFG";
		КонецЕсли;
		ДокОбъект.Ответственный = Пользователи.ТекущийПользователь();
		СтрокаТаблицыРегистров = ДокОбъект.ТаблицаРегистров.Добавить();
		СтрокаТаблицыРегистров.Имя = "InvoicedDebts";
		ДокОбъект.Движения.InvoicedDebts.Загрузить(ТЗ_ДвиженияТекущегоМесяца);
		ДокОбъект.Движения.InvoicedDebts.Записывать = Истина;
		ДокОбъект.Записать();
	КонецЕсли;
