...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	|ИЗ
	|	РегистрСведений.НастройкаСинхронизацииОбъектовСВнешнимиСистемами.СрезПоследних(&Период, ТипСоответствия = &Source
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов КАК
	...
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			BT_Итоговая.Agreement
	|		ИЗ
	|			BT_Итоговая КАК BT_Итоговая)
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	&Source КАК Source,
	...
	КэшДоговоров = РезультатЗапроса[3].Выгрузить();
...
Функция СоздатьДоговор(ВыборкаДанные)
	...
	ЗаполнитьЗначенияСвойств(ДоговорОбъект, ВыборкаДанные, "PTDaysFrom,PTDaysTo,FirstDateOfPayment");
...
Процедура СторнироватьДвиженияТекущегоМесяца()
	...
	Запрос.Текст = 
		...
		|	-InvoicedDebtsОбороты.AmountОборот,
		|	-InvoicedDebtsОбороты.BaseAmountОборот
		|ИЗ
		|	РегистрНакопления.InvoicedDebts.Обороты(&НачалоПериода, &КонецПериода,, Invoice.Source = &Source) КАК
		|		InvoicedDebtsОбороты";
