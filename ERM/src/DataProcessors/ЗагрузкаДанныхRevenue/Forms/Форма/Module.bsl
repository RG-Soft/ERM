...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	РучнаяКорректировка.Ссылка КАК Ссылка,
	|	РучнаяКорректировка.ClientID КАК ClientID,
	|	РучнаяКорректировка.Client КАК Client
	|ИЗ
	...
	|ВЫБРАТЬ
	|	&Source,
	|	СУММА(врТЗТаблицаДанных.Amount),
	|	СУММА(врТЗТаблицаДанных.AmountUSD),
	|	врТЗТаблицаДанных.CustomerNumber,
	|	ВТ_СоответствиеКлиентовCustomerNumber.ОбъектПриемника,
	|	ВТ_СоответствиеCurrency.ОбъектПриемника,
	|	Организации.Ссылка,
	...
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаRevenue;
	Если Source = Перечисления.ТипыСоответствий.Radius Тогда
...
Процедура СторнироватьДвиженияТекущегоМесяца()
	...
	Запрос.Текст = 
		...
		|	-RevenueОбороты.BaseAmountОборот КАК BaseAmount,
		|	&КонецПериода КАК Период,
		|	RevenueОбороты.Document КАК Document
		|ИЗ
		|	РегистрНакопления.Revenue.Обороты(&НачалоПериода, &КонецПериода, , Source = &Source) КАК RevenueОбороты";
	...
	Если ТЗ_ДвиженияТекущегоМесяца.Количество() > 0 Тогда
		...
		ДокОбъект.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаRevenue;
		Если Source = Перечисления.ТипыСоответствий.Radius Тогда
			ДокОбъект.Комментарий = "Списание текущей выручки по Radius";
		ИначеЕсли Source = Перечисления.ТипыСоответствий.Geofit Тогда
			ДокОбъект.Комментарий = "Списание текущей выручки по Geofit";
		ИначеЕсли Source = Перечисления.ТипыСоответствий.MFG Тогда
		...
		СтрокаТаблицыРегистров.Имя = "Revenue";
		ДокОбъект.Движения.Revenue.Загрузить(ТЗ_ДвиженияТекущегоМесяца);
		ДокОбъект.Движения.Revenue.Записывать = Истина;
		ДокОбъект.Записать();
	КонецЕсли;
