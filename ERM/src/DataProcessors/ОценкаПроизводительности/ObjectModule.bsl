...
Функция ВычислитьAPDEX(ПараметрыВычисления) Экспорт
	...
	|ПО
	|	КлючевыеОперации.КлючеваяОперация = УдалитьЗамерыВремени2.КлючеваяОперация
	|	И УдалитьЗамерыВремени2.ДатаНачалаЗамера МЕЖДУ &НачалоПериода И &КонецПериода
	|	%УсловиеКомментарий%
	|ГДЕ
	...
	|					1 
	|				ИЗ 
	|					РегистрСведений.УдалитьЗамерыВремени2 КАК ЗамерыВремениВнутр
	|				ГДЕ
	...
	|								КОГДА УдалитьЗамерыВремени2.ДатаНачалаЗамера >= &НачалоПериода%Номер%
	|										И УдалитьЗамерыВремени2.ДатаНачалаЗамера <= &КонецПериода%Номер%
	|									ТОГДА ВЫБОР
	|											КОГДА УдалитьЗамерыВремени2.ВремяВыполнения <= КлючевыеОперации.ЦелевоеВремя
	|												ТОГДА 1
	|											ИНАЧЕ 0
	|										КОНЕЦ
	|								ИНАЧЕ 0
	...
	|									ТОГДА ВЫБОР
	|											КОГДА УдалитьЗамерыВремени2.ВремяВыполнения > КлючевыеОперации.ЦелевоеВремя
	|													И УдалитьЗамерыВремени2.ВремяВыполнения <= КлючевыеОперации.ЦелевоеВремя * 4
	|												ТОГДА 1
	|											ИНАЧЕ 0
	|										КОНЕЦ
	|								ИНАЧЕ 0
	...
	ИначеЕсли ПараметрыВычисления.ВариантФильтраКомментарий = "Равен" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%УсловиеКомментарий%", "И УдалитьЗамерыВремени2.Комментарий = &Комментарий");
	...
	ИначеЕсли ПараметрыВычисления.ВариантФильтраКомментарий = "Содержит" Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%УсловиеКомментарий%", "И УдалитьЗамерыВремени2.Комментарий ПОДОБНО &Комментарий");
