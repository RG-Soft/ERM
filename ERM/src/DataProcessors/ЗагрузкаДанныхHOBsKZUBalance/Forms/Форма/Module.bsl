...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Заголовок = "Select a loading file";
...
Функция ПроверитьКорректностьДанных(ТаблицаДанных)
	...
	Запрос.Текст = 
		...
		|	ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU),
		...
		|					И ТипСоответствия = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)) КАК НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних
	...
	Запрос.УстановитьПараметр("ТипВнешнейСистемы", Перечисления.ТипыСоответствий.HOBsKZU);
	...
	Пока ВыборкаКомпаний.Следующий() Цикл
		...
		СтрокаКоллизии.КоллизияОтработана = Ложь;
		СтрокаКоллизии.Описание = "Not specified base element";
		СтрокаКоллизии.ТипСоответствия = Перечисления.ТипыСоответствий.HOBsKZU;
...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		СтрокаТЗ.InvoiceNo = ?(ЗначениеЗаполнено(СтрокаТЗ.Invoice), "KZ-" + СтрокаТЗ.Invoice, "KZ-#empty#") + ";" + СтрокаТЗ.Customer + ";" + Формат(Год(СтрокаТЗ.TransDate), "ЧГ=");
	КонецЦикла;
	...
	|			&Период,
	|			ТипСоответствия = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)
	...
	Запрос.УстановитьПараметр("Период", ПериодНач);
	Запрос.УстановитьПараметр("Source", Перечисления.ТипыСоответствий.HOBsKZU);
	...
	НаборЗаписей.Отбор.Source.Установить(Перечисления.ТипыСоответствий.HOBsKZU);
	...
	Док.Комментарий = "Ввод остатков по HOBsKZU";
	...
	ФильтрНезагружаемыхДанных = МенеджерФильтраНезагружаемыхДанных.ПолучитьТаблицуФильтра(Перечисления.ТипыСоответствий.HOBsKZU);
...
Процедура СформироватьЗаписиПоСписаниюТекущихОстатков()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.UnbilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)) КАК UnbilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.BilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)) КАК BilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.UnallocatedCash.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)) КАК UnallocatedCashОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.ManualTransactions.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)) КАК ManualTransactionsОстатки";
	...
	Док.Комментарий = "Списание текущих остатков по HOBsKZU";
	Док.Ответственный = Пользователи.ТекущийПользователь();
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаОстатков;
...
Функция СоздатьИнвойс(ДанныеДляЗаполнения)
	...
	ДокОбъект.Source = Перечисления.ТипыСоответствий.HOBsKZU;
...
Функция СоздатьРучнуюКорректировку(ДанныеДляЗаполнения)
	...
	РучнаяКорректировкаОбъект.Source = Перечисления.ТипыСоответствий.HOBsKZU;
