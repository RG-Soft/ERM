	...
	Иначе
		...
		Если не ЗначениеЗаполнено(SubGeomarket) тогда
			...
			Если ЗначениеЗаполнено(GeoMarket) тогда
				СабГеомаркет.Родитель = GeoMarket;
			...
			Иначе 
				...
				СабГеомаркет.Записать();				
			КонецЕсли;		
			    Sub = СабГеомаркет.ссылка.получитьОбъект(); 
			    Sub.Родитель = Геомаркет;
			
			НовоеПодразделение.GeoMarket = СабГеомаркет;
		иначе
			НовоеПодразделение.GeoMarket = SubGeomarket;
		КонецЕсли; 
		НовоеПодразделение.Записать();
		РГСофтКлиентСервер.УстановитьЗначение(AUОбъект.ПодразделениеОрганизации, НовоеПодразделение);
	КонецЕсли;
