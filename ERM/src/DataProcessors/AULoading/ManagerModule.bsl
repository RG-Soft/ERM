...
Процедура ЗагрузитьAU(ТекстОшибок, МассивСтруктурAU) 
	...
	Для Каждого СтруктураAU из МассивСтруктурAU Цикл
		...
		Попытка
			...
			ЗаполнитьБазовыйЭлементLocations(СтрокаДанныхAU.ПодразделениеОрганизации, СтруктураAU, ТЗДанныеHFM_Locations, ТекстОшибок);
			
			ЗаполнитьБазовыйЭлементСегмент(СтрокаДанныхAU.Сегмент, СтруктураAU, ТЗДанныеHFM_Segment, ТекстОшибок);
...
Процедура ЗаполнитьБазовыйЭлементLocations(ПодразделениеОрганизации, СтруктураAU, ТЗДанныеHFM_Locations, ТекстОшибок)
		...
		Если СтрокаHFM_Locations = Неопределено Тогда
			РГСофт.СообщитьИЗалоггировать(
				"Не найден MgtCty" + СтруктураAU.MgtCty,
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Обработки.AULoading,
				Неопределено,
				ТекстОшибок + ОписаниеОшибки());
...
Процедура ЗаполнитьБазовыйЭлементСегмент(Сегмент, СтруктураAU, ТЗДанныеHFM_Segment, ТекстОшибок)
	
		СтрокаHFM_Segment = ТЗДанныеHFM_Segment.Найти(СокрЛП(СтруктураAU.SsubSegment), "Код");
		РеквизитыСегмент = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сегмент, "БазовыйЭлемент");
		Если СтрокаHFM_Segment = Неопределено Тогда
			РГСофт.СообщитьИЗалоггировать(
				"Не найден Segment " + СтруктураAU.SsubSegment,
				УровеньЖурналаРегистрации.Ошибка,
				Метаданные.Обработки.AULoading,
				Неопределено,
				ТекстОшибок + ОписаниеОшибки());
		ИначеЕсли РеквизитыСегмент.БазовыйЭлемент <> СтрокаHFM_Segment.Ссылка Тогда
...
Процедура ПолучитьДанныеДляЗаполненияAU(МассивСтруктурAU, ТЗДанныхAU, ТЗДанныеHFM_Locations, ТЗДанныеHFM_Segment)
	...
	Для Каждого СтруктураAU из МассивСтруктурAU Цикл 
		...
		МассивHFM_Locations.Добавить(СокрЛП(СтруктураAU.MgtCty));
		МассивHFM_Segment.Добавить(СокрЛП(СтруктураAU.SsubSegment));
	КонецЦикла;
	
	МассивКодов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивКодов);
	МассивHFM_Locations = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивHFM_Locations);
	МассивHFM_Segment = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивHFM_Segment);
