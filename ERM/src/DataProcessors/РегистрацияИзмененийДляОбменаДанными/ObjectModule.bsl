...
//        * ИндексКартинки                - Число  - 1 = план обмена, 2 = УзелИнформационнойБазы , 3 = помеченный для удаления УзелИнформационнойБазы.
...
Функция СформироватьДеревоУзлов(ОбъектДанных = Неопределено, ИмяТаблицы = Неопределено) Экспорт
	...
	Иначе
		...
		ТекстЗапроса = "
			...
			|	КОЛИЧЕСТВО(ТаблицаИзменений.УзелИнформационнойБазы) КАК КоличествоИзмененийНаУзле
			|ИЗ
			|	ПланОбмена.{0} КАК ПланОбмена
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	" + ТекИмяТаблицы + ".Изменения КАК ТаблицаИзменений
			|ПО
			|	ТаблицаИзменений.УзелИнформационнойБазы = ПланОбмена.Ссылка
			|	" + ТекстЗапроса + "
			|ГДЕ
			|	НЕ ПланОбмена.ЭтотУзел
			|СГРУППИРОВАТЬ ПО 
...
//         * УзелОбмена              - ПланОбменаСсылка - Ссылка на УзелИнформационнойБазы обмена, для которого рассчитываем количество.
	...
	Для Каждого КлючЗначение Из Источник Цикл
		...
		Для Каждого Элемент Из КлючЗначение.Значение Цикл
			...
			Текст = Текст + ?(Текст = "", "", "ОБЪЕДИНИТЬ ВСЕ") + " 
				...
				|	УзелИнформационнойБазы                КАК УзелОбмена,
				...
				|	" + Элемент + ".Изменения
				|ГДЕ
				|	УзелИнформационнойБазы В (&СписокУзлов)
				|СГРУППИРОВАТЬ ПО
...
//
// Параметры:
//    УзелИнформационнойБазы              - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена для которого получаем информацию, 
...
//
// Параметры:
//    Ссылка - ПланОбменаСсылка - Ссылка на УзелИнформационнойБазы обмена.
...
Функция ИзмеренияНабораЗаписей(ИмяТаблицы, ВсеИзмерения = Ложь) Экспорт
	...
	Если Не ВсеИзмерения Тогда
		...
		НеУчитывать = "#НомерСообщения#УзелИнформационнойБазы#";
	...
	Если ТипЗнч(ОбъектРегистрации) = Тип("ОбъектМетаданных") Тогда
		...
		ИначеЕсли Характеристики.ЭтоНабор Тогда
			Для Каждого ИзмерениеРегистра Из ОбъектРегистрации.Измерения Цикл
				Если НРег(ИзмерениеРегистра.Имя) = "УзелИнформационнойБазы" Тогда
					Возврат;
				КонецЕсли;
			КонецЦикла;
			ПоляИзмерений = "";
...
Функция БСП_ОбновитьИЗарегистрироватьИОМГлавногоУзла(Знач Узел) Экспорт
	...
	Если (Не ДоступнаРаботаРИБ)                                      // Недоступна работа с ИОМ из-за версии БСП.
		Или (ПланыОбмена.ГлавныйУзел() <> Неопределено)              // Текущая база - подчиненный УзелИнформационнойБазы.
		Или (Не МетаПланОбменаУзла.РаспределеннаяИнформационнаяБаза) Тогда // УзелИнформационнойБазы-параметр не РИБ
		Возврат Результат;
	КонецЕсли;
