...
Процедура ПроверитьВозможностьДобавленияСтатуса(ТекстСообщенияОбОшибках, ОтмеченныеSalesOrders)
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	SalesOrder.Ссылка КАК ДокументСсылка
		...
		|ГДЕ
		|	CashBatch.Ссылка В(&ОтмеченныеSalesOrders)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТИПЗНАЧЕНИЯ(ВТ_Документы.ДокументСсылка) КАК ТипДокумента
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТ_Документы.ДокументСсылка.Source КАК ДокументСсылкаSource
		|ИЗ
		|	ВТ_Документы КАК ВТ_Документы";
	...
	ВыборкаТиповДокументов = МассивРезультатов[2].Выбрать();
	ВыборкаИсточников = МассивРезультатов[3].Выбрать();
	//ВыборкаРучныеКорректировки = МассивРезультатов[2].Выбрать();
	//ВыборкаСО = МассивРезультатов[3].Выбрать();
	ЗафиксироватьТранзакцию();
	
	// { RGS TAlmazova 24.07.2018 16:59:47 - S-E-0000877
	...
	//	Возврат;
	Если ВыборкаТиповДокументов.Количество() > 1 Тогда
		ТекстСообщенияОбОшибках = "Selected Sales orders and JV and CashBatch at the same time!";
		Возврат;
	ИначеЕсли ВыборкаИсточников.Количество() > 1 Тогда
		ТекстСообщенияОбОшибках = "Selected Sales orders or JV or CashBatch with different Sources";
		Возврат;
	// } RGS TAlmazova 24.07.2018 16:59:48 - S-E-0000877
...
Процедура PeriodПриИзменении(Элемент)
	...
	Иначе
		...
		ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(SalesOrders, "ДатаДляСравнения", ПериодОстатков - 1);
	КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивСчетовUnbilled = Новый Массив;
	МассивСчетовUnbilled.Добавить(ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12002"));
	МассивСчетовUnbilled.Добавить(ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12003"));
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(SalesOrders, "HFMUnbilledAccounts", МассивСчетовUnbilled);
...
Процедура SalesOrdersВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(SalesOrdersТекущиеДанные.SalesOrder) = Тип("ДокументСсылка.РучнаяКорректировка") Тогда
		ОткрытьФорму("Документ.РучнаяКорректировка.Форма.ФормаДокумента", Новый Структура("Ключ", SalesOrdersТекущиеДанные.SalesOrder));
	Иначе
		ОткрытьФорму("Документ.CashBatch.Форма.ФормаДокумента", Новый Структура("Ключ", SalesOrdersТекущиеДанные.SalesOrder));
	КонецЕсли;
...
Процедура ПолучитьДанныеДинамическогоСписка()
	...
	ЭлементОформления = Настройки.ПараметрыВывода.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("ВыводитьПараметрыДанных"));
	КонецЕсли;
	Если ЭлементОформления <> Неопределено Тогда
		ЭлементОформления.Использование = Истина;
		ЭлементОформления.Значение = ТипВыводаТекстаКомпоновкиДанных.НеВыводить;
