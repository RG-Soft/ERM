...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Индекс = 0 По СписокSalesOrders.Количество() - 1 Цикл
		Если ТипЗнч(СписокSalesOrders[Индекс].Значение) = Тип("ДокументСсылка.SalesOrder")
			И (СписокSalesOrders[Индекс].Значение.Source = Перечисления.ТипыСоответствий.Lawson ИЛИ СписокSalesOrders[Индекс].Значение.Source = Перечисления.ТипыСоответствий.OracleMI
			ИЛИ СписокSalesOrders[Индекс].Значение.Source = Перечисления.ТипыСоответствий.OracleSmith ИЛИ СписокSalesOrders[Индекс].Значение.Source = Перечисления.ТипыСоответствий.HOBs И СписокSalesOrders[Индекс].Значение.Company <> Справочники.Организации.НайтиПоКоду("385")) Тогда
	...
	Если ТипЗнч(СписокSalesOrders[0].Значение) = Тип("ДокументСсылка.РучнаяКорректировка") Тогда
...
Процедура SaveНаСервере(СообщениеОбОшибке)
	...
	Если ТипЗнч(СписокSalesOrders[0].Значение) = Тип("ДокументСсылка.SalesOrder") 
		И Billed = Перечисления.SalesOrderBilledStatus.ПустаяСсылка() Тогда
