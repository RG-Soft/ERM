...
Функция ПроверитьКорректностьДанных(ТаблицаДанных)
	...
	Запрос.Текст = 
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЛОЖЬ,
		|	""Failed to find AU (Oracle Smith)"",
		|	&ТипВнешнейСистемы,
		...
		|		ПО врТЗТаблицаДанных.AU = КостЦентры.Код
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	RadiusSubSubSegment.Значение КАК Ссылка,
		|	ЗНАЧЕНИЕ(Справочник.HFM_Technology.ПустаяСсылка) КАК БазовыйЭлемент,
		|	RadiusSubSubSegment.Значение.Код КАК Код
		|ИЗ
		|	Константа.RadiusSubSubSegment КАК RadiusSubSubSegment
		|ГДЕ
		|	RadiusSubSubSegment.Значение.БазовыйЭлемент = ЗНАЧЕНИЕ(Справочник.HFM_Technology.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	RadiusLocation.Значение КАК Ссылка,
		|	RadiusLocation.Значение.Код КАК Код,
		|	ЗНАЧЕНИЕ(Справочник.HFM_Locations.ПустаяСсылка) КАК БазовыйЭлемент
		|ИЗ
		|	Константа.RadiusLocation КАК RadiusLocation
		|ГДЕ
		|	RadiusLocation.Значение.БазовыйЭлемент = ЗНАЧЕНИЕ(Справочник.HFM_Locations.ПустаяСсылка)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.УстановитьПараметр("ТипВнешнейСистемы", Перечисления.ТипыСоответствий.Radius);
	...
	Пока ВыборкаСегментов.Следующий() Цикл
		...
		Если ВыборкаСегментов.БазовыйЭлемент.Пустая() Тогда
			...
			СтрокаКоллизии.ТипСоответствия = Перечисления.ТипыСоответствий.Radius;
...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		СтрокаТЗ.InvoiceNo = ?(ЗначениеЗаполнено(СтрокаТЗ.Invoice), "RD-" + СтрокаТЗ.Invoice, "RD-#empty#") + ";" + СтрокаТЗ.Customer + ";" + Формат(Год(СтрокаТЗ.TransDate), "ЧГ=");
	КонецЦикла;
	...
	"ВЫБРАТЬ
	|	ТаблицаИсходныхДанных.OrigCurrency КАК CurrencyCode,
	|	ТаблицаИсходныхДанных.AU КАК AUCode,
	...
	|			И (НЕ Организации.ПометкаУдаления)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КостЦентры КАК КостЦентры
	|		ПО врТЗТаблицаДанных.AUCode = КостЦентры.Код
	|			И (КостЦентры.Source = Значение(Перечисление.ТипыСоответствий.OracleSmith))
	|			И (НЕ КостЦентры.ПометкаУдаления)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Oracle КАК Oracle
	|		ПО врТЗТаблицаДанных.AccountCode = Oracle.Код
	|			И (НЕ Oracle.ПометкаУдаления)";
	
	Запрос.УстановитьПараметр("Период", ПериодНач);
	Запрос.УстановитьПараметр("Source", Перечисления.ТипыСоответствий.Radius);
	...
	КэшРучныхКорректировок.Индексы.Добавить("Company, AU, Account, Currency");
	...
	СтруктураПоискаJV = Новый Структура("Company, Client, AU, Account, Currency");
	...
	НаборЗаписей.Отбор.Source.Установить(Перечисления.ТипыСоответствий.Radius);
	...
	Док.Комментарий = "Ввод остатков по Радиусу";
	Док.Ответственный = Пользователи.ТекущийПользователь();
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаОстатков;
	...
	ФильтрНезагружаемыхДанных = МенеджерФильтраНезагружаемыхДанных.ПолучитьТаблицуФильтра(Перечисления.ТипыСоответствий.Radius);
	...
	ТЗ_ARBalance.Свернуть("Период, Client, Company, Source, TransType, Invoice, Account, Currency, AU", "Amount");
...
Процедура СформироватьЗаписиПоСписаниюТекущихОстатков()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.UnbilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Radius)) КАК UnbilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.BilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Radius)) КАК BilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.UnallocatedCash.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Radius)) КАК UnallocatedCashОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.ManualTransactions.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Radius)) КАК ManualTransactionsОстатки";
	...
	Док.Комментарий = "Списание текущих остатков по Радиусу";
	Док.Ответственный = Пользователи.ТекущийПользователь();
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаОстатков;
