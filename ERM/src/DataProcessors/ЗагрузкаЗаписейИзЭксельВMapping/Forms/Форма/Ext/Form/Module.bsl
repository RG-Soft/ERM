...
Процедура ПоказыватьНумирациюКолонокПриИзменении(Элемент)
	Элементы.СтруктураКолонок.Видимость = ПоказыватьНумирациюКолонок;
КонецПроцедуры
...
Процедура ДобавитьВРегистрНаСервере()
	...
	Для Каждого Строка Из ТаблицаДанных Цикл
		...
		Для Каждого Колонка Из Колонки Цикл
			Если Строка[Колонка] <> "" Тогда //проверка записи в регистре
				...
				Отбор.Вставить("Идентификатор", Строка[Колонка]);
		...
		КонецЦикла;
			...
			Контрагент = ?(ОбъектПриемникаСуществует,СуществующийКонтрагент,Строка.CustomerDesc);
			...
			Если Не ЗначениеЗаполнено(Справочники.Контрагенты.НайтиПоНаименованию(Контрагент)) Тогда 
				...
				КонтрагентЗапись.Наименование = Строка.CustomerDesc;
			...
			Если Массив.Количество() <> 0 Тогда //Добавление записей из массива
				Для Каждого Элемент Из Массив Цикл
					...
					Запись.Идентификатор = Строка[Элемент];
