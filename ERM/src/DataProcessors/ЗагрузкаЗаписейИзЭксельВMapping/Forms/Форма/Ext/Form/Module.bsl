...
Процедура ДобавитьВРегистрНаСервере()
	...
	ТаблицаКонтрагентов.Колонки.Добавить("Контрагент", Новый ОписаниеТипов ("Строка"));
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		ТекущаяСтрокаТаблицаКонтрагентов = ТаблицаКонтрагентов.Добавить();
		ТекущаяСтрокаТаблицаКонтрагентов.Контрагент = СтрокаТаблицы.CustomerDesc;
		Для Каждого Колонка Из Колонки Цикл
			Если СтрокаТаблицы[Колонка] <> "" Тогда
				...
				ТекущаяСтрокаТаблицыНеПустыхЗначений.Идентификатор = СтрокаТаблицы[Колонка];  Справочники.Контрагенты.Выбрать()
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВЫРАЗИТЬ(ТаблицаКонтрагентов.Контрагент КАК СТРОКА(100)) КАК Контрагент
	               ...
	               |		ПО ТаблицаКонтрагентов.Контрагент = Контрагенты.Наименование";
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Если ОбъектСуществует Тогда
			Контрагент = СуществующийКонтрагент;
		Иначе
			Контрагент = СтрокаТаблицы.CustomerDesc;
			ТекСтрокаТаблицаКонтрагентов = ТаблицаКонтрагентов.Найти(Контрагент, "Наименование");
			Если Не ЗначениеЗаполнено(ТекСтрокаТаблицаКонтрагентов) Тогда
				КонтрагентЗапись = Справочники.Контрагенты.СоздатьЭлемент();
				КонтрагентЗапись.Наименование = Контрагент;
				КонтрагентЗапись.УстановитьНовыйКод();
				КонтрагентЗапись.ОбменДанными.Загрузка = Истина;
				КонтрагентЗапись.Записать();
				Контрагент = КонтрагентЗапись.Ссылка;
				НоваяСтрока = ТаблицаКонтрагентов.Добавить();
				НоваяСтрока.Наименование = Контрагент;
			Иначе
				Контрагент = ТекСтрокаТаблицаКонтрагентов.Ссылка;
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого Элемент Из МассивНеПустыхКолонок Цикл
			...
			Запись.ОбъектПриемника = Контрагент;
