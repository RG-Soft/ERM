...
Процедура ПолучитьДанныеДляСопоставленияНаСервере()
	...
	WSПрокси = Обработки.MappingBillingClients.СоздатьWSПрокси(УзелОбмена, WSСсылка);
...
Процедура ПолучитьДанныеДляСопоставления(Команда)
	...
	Если UnmappedData.Количество() > 0 Тогда
		Оповещение = Новый ОписаниеОповещения("ПолучитьДанныеДляСопоставленияЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Оповещение, "Unmapped data is not empty. Continue?", РежимДиалогаВопрос.ДаНет, 10);
	Иначе
		ПолучитьДанныеДляСопоставленияНаСервере();
	КонецЕсли;
...
&НаКлиенте
Процедура ПолучитьДанныеДляСопоставленияЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УзелОбмена = ПланыОбмена.rgsНачальноеЗаполнениеКлиентовERM.НайтиПоКоду("BL");
...
&НаКлиенте
Процедура MappedDataПередУдалением(Элемент, Отказ)
	
	MappedDataПередУдалениемНаСервере(ОбщегоНазначенияКлиентСервер.СкопироватьМассив(Элемент.ВыделенныеСтроки), MappedData, УзелОбмена);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура MappedDataПередУдалениемНаСервере(ИдентификаторыВыделенныхСтрок, Знач MappedData, УзелОбмена)
	
	НЗ = РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.СоздатьНаборЗаписей();
	НЗ.Отбор.УзелИнформационнойБазы.Установить(УзелОбмена);
	
	Для каждого ТекИдентификатор Из ИдентификаторыВыделенныхСтрок Цикл
		
		СтрокаКоллекции = MappedData.НайтиПоИдентификатору(ТекИдентификатор);
		
		Если СтрокаКоллекции = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НЗ.Отбор.Ссылка.Установить(СтрокаКоллекции.ERMObject);
		НЗ.Отбор.Идентификатор.Установить(СтрокаКоллекции.GUID);
