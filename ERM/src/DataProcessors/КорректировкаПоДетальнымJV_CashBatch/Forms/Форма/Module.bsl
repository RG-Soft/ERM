...
Процедура ПроставитьОтметкуНаОбнуляющихсяСтрокахНаСервере()
	...
	Отбор = Построитель.Отбор.Добавить("AmountОстаток");
	Отбор.ВидСравнения = ВидСравнения.ИнтервалВключаяГраницы;
	Отбор.ЗначениеС = -1;
	Отбор.ЗначениеПо = 1;
	Отбор.Использование = Истина;
	Построитель.Выполнить();
	ТЗ_АналитикаВТоталеМеньшеЕдиницы = Построитель.Результат.Выгрузить();
	
	ТЗ_АналитикаВТоталеМеньшеЕдиницы.Свернуть("Source,Currency,Client,CustomerID,Company,Account,AU");
	...
	Для каждого СтрокаТаблицы Из ТаблицаОстатков Цикл
		...
		Строки = ТЗ_АналитикаВТоталеМеньшеЕдиницы.НайтиСтроки(Отбор);
