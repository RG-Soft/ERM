...
Функция ПроверитьКорректностьДанных(ТаблицаДанных)
	...
	Запрос.Текст = 
		...
		|					И ТипСоответствия = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Lawson)) КАК НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних
	...
	Запрос.УстановитьПараметр("ТипВнешнейСистемы", Перечисления.ТипыСоответствий.Geofit);
	...
	Пока ВыборкаСегментов.Следующий() Цикл
		...
		Если ВыборкаСегментов.БазовыйЭлемент.Пустая() Тогда
			...
			СтрокаКоллизии.ТипСоответствия = Перечисления.ТипыСоответствий.Geofit;
...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		СтрокаТЗ.InvoiceNo = ?(ЗначениеЗаполнено(СтрокаТЗ.Invoice), "GF-" + СтрокаТЗ.Invoice, "GF-#empty#") + ";" + СтрокаТЗ.Customer + ";" + Формат(Год(СтрокаТЗ.TransDate), "ЧГ=");
	КонецЦикла;
	...
	Запрос.УстановитьПараметр("Период", ПериодНач);
	Запрос.УстановитьПараметр("Source", Перечисления.ТипыСоответствий.Geofit);
	...
	НаборЗаписей.Отбор.Source.Установить(Перечисления.ТипыСоответствий.Geofit);
	...
	Док.Комментарий = "Ввод остатков по Геофиту";
	...
	ФильтрНезагружаемыхДанных = МенеджерФильтраНезагружаемыхДанных.ПолучитьТаблицуФильтра(Перечисления.ТипыСоответствий.Geofit);
...
Процедура СформироватьЗаписиПоСписаниюТекущихОстатков()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.UnbilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Geofit)) КАК UnbilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.BilledAR.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Geofit)) КАК BilledARОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.UnallocatedCash.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Geofit)) КАК UnallocatedCashОстатки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрНакопления.ManualTransactions.Остатки(&ПериодОстатков, Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Geofit)) КАК ManualTransactionsОстатки";
	...
	Док.Комментарий = "Списание текущих остатков по Геофиту";
	Док.Ответственный = Пользователи.ТекущийПользователь();
	Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаОстатков;
...
Функция СоздатьИнвойс(ДанныеДляЗаполнения)
	...
	ДокОбъект.Source = Перечисления.ТипыСоответствий.Geofit;
...
Функция СоздатьРучнуюКорректировку(ДанныеДляЗаполнения)
	...
	РучнаяКорректировкаОбъект.Source = Перечисления.ТипыСоответствий.Geofit;
