...
Функция ОбработатьПолучениеСтруктурыПочтовогоСообщения(СтруктураПараметров, ДополнительныеПараметры)
	...
	ИначеЕсли ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		ЗарегистрироватьПредметыПисьма(ДокументEMail.Ссылка, ДополнительныеПараметры.Инвойсы);
	//}RGS AArsentev 26.01.2017 S-E-0000048
	КонецЕсли;
...
Процедура ВыбратьTemplateЗавершение_ToClient(Результат, ДополнительныеПараметры) Экспорт
	...
	Если СтруктураПараметров <> Неопределено Тогда
		...
		ПолучитьМассивАдресов = ПолучитьАдресаКлиентов(МассивКлиентов);
		...
		СтруктураПараметров.Вставить("Адресаты", ПолучитьМассивАдресов);
...
Функция ПолучитьАдресаКлиентов(МассивКлиентов)
	...
	Возврат Результат.ВыгрузитьКолонку("Представление");
