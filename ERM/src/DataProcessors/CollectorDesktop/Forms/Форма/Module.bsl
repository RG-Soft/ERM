...
Процедура AddStatus(Команда)
	...
	Иначе
		...
		ПараметрыФормы.Вставить("RemainingAmount", InvoicesТекущиеДанные.RemainingAmount);
		ПараметрыФормы.Вставить("Currency", InvoicesТекущиеДанные.Currency);
...
Процедура AddStatusForSelectedInvoices(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Для каждого ТекВыделеннаяСтрока Из Элементы.Invoices.ВыделенныеСтроки Цикл
			...
			InvoicesRemainingAmount = InvoicesRemainingAmount + ДанныеСтроки.RemainingAmount;
		КонецЦикла;
		
		InvCurrency = Элементы.Invoices.ДанныеСтроки(Элементы.Invoices.ВыделенныеСтроки[0]).Currency;
