...
Процедура AddStatus(Команда)
	...
	Иначе
		...
		СтруктураДанныхТекущейСтроки = Новый Структура("Invoice, FiscalInvNo, RemainingAmount, Currency, , DateEntered, DateIdentified, DisputeDistributedDate, DisputCollectableDate,  ConfirmedBy, CustomerInputDetails, CustomerRepresentative, ForecastDate, RemedialWorkPlan, RWDTargetDate, Agreement, Status, StatusOfDispute");
		ЗаполнитьЗначенияСвойств(СтруктураДанныхТекущейСтроки, InvoicesТекущиеДанные);
		
		ПараметрыФормы.Вставить("СтруктураДанныхТекущейСтроки", СтруктураДанныхТекущейСтроки);
...
Процедура AddStatusForSelectedInvoices(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Для каждого ВыделеннаяСтрока Из Элементы.Invoices.ВыделенныеСтроки Цикл
		
			ДанныеСтроки = Элементы.Invoices.ДанныеСтроки(ВыделеннаяСтрока);
			МассивСтрок.Добавить(ДанныеСтроки);
			
		КонецЦикла; 
		
		//S-E-0001342 - заполнение формы на основании данных в выбранных строках
		ПараметрыФормы = ЗаполнениеСтруктурыЗначенийПолей(МассивСтрок);
...
&НаСервере
Функция ЗаполнениеСтруктурыЗначенийПолей(ВыделенныеСтроки)

		ОтмеченныеИнвойсы = Новый СписокЗначений;
		
		ПараметрыФормы = Новый Структура;
		
		InvoicesRemainingAmount = 0;
		
		СоответствиеStatusCRMID = Новый Соответствие;
		
		СовпадениеПоStatusCRMID = Истина;
		
		НаборыЗначенийПолей = Новый ТаблицаЗначений;
		НаборыЗначенийПолей.Колонки.Добавить("FiscalInvNo");
		НаборыЗначенийПолей.Колонки.Добавить("Status");
		НаборыЗначенийПолей.Колонки.Добавить("StatusOfDispute");
		НаборыЗначенийПолей.Колонки.Добавить("DateEntered");
		НаборыЗначенийПолей.Колонки.Добавить("DateIdentified");
		НаборыЗначенийПолей.Колонки.Добавить("DisputeDistributedDate");
		НаборыЗначенийПолей.Колонки.Добавить("DisputCollectableDate");
		НаборыЗначенийПолей.Колонки.Добавить("TriggerDate");
		НаборыЗначенийПолей.Колонки.Добавить("ConfirmedBy");
		НаборыЗначенийПолей.Колонки.Добавить("CustomerInputDetails");
		НаборыЗначенийПолей.Колонки.Добавить("CustomerRepresentative");
		НаборыЗначенийПолей.Колонки.Добавить("ForecastDate");
		НаборыЗначенийПолей.Колонки.Добавить("RemedialWorkPlan");
		НаборыЗначенийПолей.Колонки.Добавить("RWDTargetdate");
		НаборыЗначенийПолей.Колонки.Добавить("Agreement");
		НаборыЗначенийПолей.Колонки.Добавить("PTFrom");
		НаборыЗначенийПолей.Колонки.Добавить("PTTo");
		НаборыЗначенийПолей.Колонки.Добавить("Trigger");
		
		Для каждого ТекВыделеннаяСтрока Из ВыделенныеСтроки Цикл
			
			СтруктураПоиска = Новый Структура("FiscalInvNo, Status, StatusOfDispute, DateEntered, DateIdentified, DisputeDistributedDate, DisputCollectableDate, TriggerDate, ConfirmedBy, CustomerInputDetails, CustomerRepresentative, ForecastDate, RemedialWorkPlan, RWDTargetDate, Agreement, PTFrom, PTTo, Trigger",
											 Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено);
			
			ТекущийИнвойс = ТекВыделеннаяСтрока.Invoice;
			Контракт = ТекущийИнвойс.Contract;
			РеквизитыКонтракта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контракт,"PTDaysFrom,СрокОплаты,Trigger");
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, ТекВыделеннаяСтрока);
			СтруктураПоиска.Вставить("TriggerDate", ТекущийИнвойс.TriggerDate);
			СтруктураПоиска.Вставить("Agreement", Контракт);
			СтруктураПоиска.Вставить("PTFrom", РеквизитыКонтракта.PTDaysFrom);
			СтруктураПоиска.Вставить("PTTo", РеквизитыКонтракта.СрокОплаты);
			СтруктураПоиска.Вставить("Trigger", РеквизитыКонтракта.Trigger);

			
			Если НаборыЗначенийПолей.НайтиСтроки(СтруктураПоиска).Количество() = 0 Тогда
				
				НоваяСтрокаТЗ = НаборыЗначенийПолей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаТЗ, ТекВыделеннаяСтрока);
				НоваяСтрокаТЗ.TriggerDate = ТекущийИнвойс.TriggerDate;
				НоваяСтрокаТЗ.Agreement = Контракт;
				НоваяСтрокаТЗ.PTFrom = РеквизитыКонтракта.PTDaysFrom;
				НоваяСтрокаТЗ.PTTo = РеквизитыКонтракта.СрокОплаты;
				НоваяСтрокаТЗ.Trigger = РеквизитыКонтракта.Trigger;
				
			КонецЕсли;
			
			ЭлементСписка = ОтмеченныеИнвойсы.НайтиПоЗначению(ТекущийИнвойс);
			Если ЭлементСписка = Неопределено Тогда
				ОтмеченныеИнвойсы.Добавить(ТекущийИнвойс);
			КонецЕсли;
			
			InvoicesRemainingAmount = InvoicesRemainingAmount + ТекВыделеннаяСтрока.RemainingAmount;
			
			Если СоответствиеStatusCRMID.Количество() = 0 Тогда
				
				СоответствиеStatusCRMID.Вставить(ТекВыделеннаяСтрока.Status, ТекВыделеннаяСтрока.Client.CRMID);
				
			ИначеЕсли СовпадениеПоStatusCRMID Тогда
				
				Если СоответствиеStatusCRMID.Получить(ТекВыделеннаяСтрока.Status) = Неопределено Или СоответствиеStatusCRMID.Получить(ТекВыделеннаяСтрока.Status) <> ТекВыделеннаяСтрока.Client.CRMID Тогда
					
					СовпадениеПоStatusCRMID = Ложь;
					Прервать;
		...
		Если СовпадениеПоStatusCRMID Тогда
			
			СтруктураПоиска = Новый Структура("FiscalInvNo, Status, StatusOfDispute, DateEntered, DateIdentified, DisputeDistributedDate, DisputCollectableDate, TriggerDate, ConfirmedBy, CustomerInputDetails, CustomerRepresentative, ForecastDate, RemedialWorkPlan, RWDTargetDate, Agreement, PTFrom, PTTo, Trigger",
											 Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено, Неопределено);
		
			Если НаборыЗначенийПолей.Количество() > 1 Тогда
				
				СтрокаИсключающихКолонок = " ";
				
				Для каждого Колонка Из НаборыЗначенийПолей.Колонки Цикл
				
					СхлопнутыйМассивЗначенийВКолонке = ОбщегоНазначенияКлиентСервер.СвернутьМассив(НаборыЗначенийПолей.ВыгрузитьКолонку(Колонка.Имя));
					
					Если СхлопнутыйМассивЗначенийВКолонке.Количество() > 1 Тогда
						
						СтрокаИсключающихКолонок = СтрокаИсключающихКолонок + Колонка.Имя + ", ";
						
					КонецЕсли;
					
				КонецЦикла;
				
				ЗаполнитьЗначенияСвойств(СтруктураПоиска, НаборыЗначенийПолей[0], , Лев(СтрокаИсключающихКолонок, СтрДлина(СтрокаИсключающихКолонок) - 2));
			
			Иначе
				
				ЗаполнитьЗначенияСвойств(СтруктураПоиска, НаборыЗначенийПолей[0]);
				
			КонецЕсли;
			
			ПараметрыФормы.Вставить("СтруктураДанныхДляЗаполнения", СтруктураПоиска);
			
		КонецЕсли; 
		
		InvCurrency = ВыделенныеСтроки[0].Currency;
		
		ПараметрыФормы.Вставить("СписокИнвойсов", ОтмеченныеИнвойсы);
		ПараметрыФормы.Вставить("RemainingAmount", InvoicesRemainingAmount);
		ПараметрыФормы.Вставить("Currency", InvCurrency);
...
Процедура ForecastDateПриИзменении(Элемент)
	...
	ГруппаОтбораИ = ОтборыСписковКлиентСервер.СоздатьГруппуЭлементовОтбора(Invoices.Отбор.Элементы, "Группа И ForecastDate",
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "ForecastDate", НачалоДня(ForecastDate.ДатаНачала),
		ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "ForecastDate", КонецДня(ForecastDate.ДатаОкончания),
...
Процедура FiscalInvDateПриИзменении(Элемент)
	...
	ГруппаОтбораИ = ОтборыСписковКлиентСервер.СоздатьГруппуЭлементовОтбора(Invoices.Отбор.Элементы, "Группа И FiscalInvDate",
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "FiscalInvDateНачалоДня", НачалоДня(FiscalInvDate.ДатаНачала),
		ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "FiscalInvDateНачалоДня", КонецДня(FiscalInvDate.ДатаОкончания),
		ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
...
Процедура InvoiceDateПриИзменении(Элемент)
	...
	//ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Invoices, "InvDateНачалоДня", НачалоДня(InvoiceDate),
		//ВидСравненияКомпоновкиДанных.Равно, , ИспользоватьОтбор);
	ГруппаОтбораИ = ОтборыСписковКлиентСервер.СоздатьГруппуЭлементовОтбора(Invoices.Отбор.Элементы, "Группа И",
		ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "InvDateНачалоДня", НачалоДня(InvoiceDate.ДатаНачала),
		ВидСравненияКомпоновкиДанных.БольшеИлиРавно);
	ОтборыСписковКлиентСервер.rgsУстановитьЭлементОтбораКоллекции(ГруппаОтбораИ.Элементы, "InvDateНачалоДня", КонецДня(InvoiceDate.ДатаОкончания),
		ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
...
&НаКлиенте
Процедура TCCПриИзменении(Элемент)
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(Invoices, "TCC", TCC, , ,TCC);
