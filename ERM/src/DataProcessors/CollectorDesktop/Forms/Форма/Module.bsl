...
Функция ОбработатьПолучениеСтруктурыПочтовогоСообщения(СтруктураПараметров, ДополнительныеПараметры)
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Структура") И ДополнительныеПараметры.Свойство("Dispute") Тогда
		...
		КонецЦикла;
		РегистрыСведений.EmailsObjects.ЗарегистрироватьПредметыПисьма(ДокументEMail.Ссылка, СозданныеКомментарии, Перечисления.ВариантОтправленияСообщения.Escalation);
		РегистрыСведений.EmailsObjects.ЗарегистрироватьПредметыПисьма(ДокументEMail.Ссылка, ДополнительныеПараметры.МассивИнвойсовДляЭскалации, Перечисления.ВариантОтправленияСообщения.Escalation);
	//{RGS AArsentev 26.01.2017 S-E-0000048
	ИначеЕсли ТипЗнч(ДополнительныеПараметры) = Тип("Структура") Тогда
		РегистрыСведений.EmailsObjects.ЗарегистрироватьПредметыПисьма(ДокументEMail.Ссылка, ДополнительныеПараметры.Инвойсы, Перечисления.ВариантОтправленияСообщения.MailToClient);
	//}RGS AArsentev 26.01.2017 S-E-0000048
	КонецЕсли;
...
Функция ДобавитьКомментарийОбЭскалации(ДополнительныеПараметры, SLBAssignedTo)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаписьОчередиУведомлений.ДатаУведомления = КалендарныеГрафики.ПолучитьДатуПоКалендарю(КалендарныеГрафики.ПроизводственныйКалендарьРоссийскойФедерации(), RWDTargetDate, 1);
