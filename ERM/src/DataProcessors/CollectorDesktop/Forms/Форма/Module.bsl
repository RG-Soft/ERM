...
Функция ОбработатьПолучениеСтруктурыПочтовогоСообщения(СтруктураПараметров, ДополнительныеПараметры)
	...
	Если ТипЗнч(ДополнительныеПараметры) = Тип("Массив") Тогда
		...
		ИдентификаторыКомментариев = Новый Массив;
		Для каждого КлючЗначение Из СозданныеКомментарии Цикл
			ИдентификаторыКомментариев.Добавить(Строка(КлючЗначение.Значение.УникальныйИдентификатор()));
		КонецЦикла;
		Если ИдентификаторыКомментариев.Количество() > 0 Тогда
			СтрокаТЧ = ДокументEMail.Headers.Добавить();
			СтрокаТЧ.Name = "X-1C-InvoiceProblem-ID";
			СтрокаТЧ.Value = СтрСоединить(ИдентификаторыКомментариев, ",");
		КонецЕсли; 
...
Функция ДобавитьКомментарийОбЭскалации(МассивИнвойсов, SLBAssignedTo)
	...
	СозданныеКомментарии = Новый Соответствие;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СозданныеКомментарии.Вставить(ВыборкаДетальныеЗаписи.Invoice, ЗаписьРегистра.Problem);
