...
Процедура ЗаполнитьСтруктуруКолонокПоУмолчанию()
	...
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "ClosedAdvance";
	СтрокаТЗ.ИмяКолонки = "Closed Advance";
	СтрокаТЗ.ТипКолонки = ТипСтрока;
	СтрокаТЗ.Обязательная = Истина;
...
Процедура СоздатьЗаписиРегистра(ТаблицаДанных)
	...
	|	ТаблицаИсходныхДанных.ClosedAdvance
	|ПОМЕСТИТЬ врТЗТаблицаДанных
	|ИЗ
	|	&ВнешняяТаблицаДанных КАК ТаблицаИсходныхДанных";
	...
	|	ВЫБОР
	|		КОГДА &Source В (ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Geofit), ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Radius))
	|			ТОГДА ВЫБОР
	|					КОГДА врТЗТаблицаДанных.ClosedAdvance = ""Closed Advance""
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ
	...
	НаборЗаписей = РегистрыСведений.InvoicePrepayments.СоздатьНаборЗаписей();
	...
	Пока ВыборкаДанные.Следующий() Цикл
		...
		Если ВыборкаДанные.TransType = "I" Тогда
			...
			Если Source = Перечисления.ТипыСоответствий.Radius Или Source = Перечисления.ТипыСоответствий.Geofit Тогда
				
				НаборЗаписей.Очистить();
				НаборЗаписей.Отбор.Invoice.Установить(ТекИнвойс);
				НоваяЗапись = НаборЗаписей.Добавить();
				НоваяЗапись.Invoice = ТекИнвойс;
				НоваяЗапись.Prepayment = ВыборкаДанные.Prepayment;
...
Процедура СторнироватьДвиженияТекущегоМесяца()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрНакопления.Payments.Обороты(&НачалоПериода, &КонецПериода, Запись, Source = &Source) КАК PaymentsОбороты";
