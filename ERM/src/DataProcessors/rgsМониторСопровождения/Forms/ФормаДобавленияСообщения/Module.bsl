...
Процедура ЗаполнитьСпискиУчастниковВопроса()
	...
	Попытка
		...
		УчастникиОбсуждения.Загрузить(ТаблицаУчастниковОбсуждения);
	Исключение
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьГруппыПользователейОбластей();
	
	Если СписокГруппПользователейПоОбластям.Количество() > 0 И ЗначениеЗаполнено(Объект.Проект) Тогда
		Элементы.ОбластьОтветственных.Видимость = Истина;
		Элементы.ОбластьОтветственных.СписокВыбора.ЗагрузитьЗначения(СписокГруппПользователейПоОбластям.ВыгрузитьЗначения());	
	КонецЕсли;
...
Функция ПолучитьПараметрыИзмененияВопроса()
	...
	ПараметрыПередачи.Вставить("ТаблицаУчастников", УчастникиОбсуждения.Выгрузить());
...
&НаКлиенте
Процедура ИсторияОбсужденияПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
...
КонецПроцедуры
 ...
 &НаСервере
Процедура ЗаполнитьГруппыПользователейОбластей()
	
	Если НЕ ЗначениеЗаполнено(Объект.Проект) 
		ИЛИ НЕ ЗначениеЗаполнено(Объект.Пользователь)
		ИЛИ НЕ ЗначениеЗаполнено(Объект.Пароль)
		ИЛИ НЕ ЗначениеЗаполнено(Объект.АдресБазы) Тогда
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Прокси.Пользователь =  Объект.Пользователь;
		Прокси.Пароль = Объект.Пароль; 
		ГруппыПользователейОбластей = ЗначениеИзСтрокиВнутр(Прокси.ГруппыПользователейОбластей(Объект.Проект));
		
		СписокГруппПользователейПоОбластям.ЗагрузитьЗначения(ГруппыПользователейОбластей.ВыгрузитьКолонку("ГруппаПользователейНаименование"));
		
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось загрузить список областей ответсвтенных!";
		Сообщение.Сообщить(); 
	КонецПопытки;
...
&НаКлиенте
Процедура ОбластьОтветственныхПриИзменении(Элемент)
	
	ЗаполнитьСоставОбсуждающихПоОбласти();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСоставОбсуждающихПоОбласти()
	...
	Попытка
		...
		ТаблицаПользователейОбласти = ЗначениеИзСтрокиВнутр(Прокси.СписокОтвечающихПользователейПоГруппе(Объект.Проект, ОбластьОтветственных));
		ТаблицаПользователейОбласти.Свернуть("ГУИДПользователя, ИмяПользователя, СторонаПользователя");
		
		Кому.Очистить();
		
		МассивСтрокУдаления = Новый Массив;
		Для каждого СтрокаУчастникОбсуждения Из УчастникиОбсуждения Цикл
			Если СтрокаУчастникОбсуждения.СторонаПользователя = "Отвечающий" Тогда
				МассивСтрокУдаления.Добавить(СтрокаУчастникОбсуждения);   
			КонецЕсли;
		КонецЦикла;
		
		Для каждого СтрокаУдаления Из МассивСтрокУдаления Цикл
			УчастникиОбсуждения.Удалить(СтрокаУдаления);
		КонецЦикла;
		
		Для каждого ПользовательОбласти Из ТаблицаПользователейОбласти Цикл
			Если ПользовательОбласти.СторонаПользователя = "Отвечающий" Тогда
				НоваяСтрока = УчастникиОбсуждения.Добавить();
				НоваяСтрока.ГУИДПользователя = ПользовательОбласти.ГУИДПользователя;
				НоваяСтрока.ИмяПользователя = ПользовательОбласти.ИмяПользователя;
				НоваяСтрока.СторонаПользователя = "Отвечающий";	
				
				Кому.Добавить(ПользовательОбласти.ИмяПользователя);
			КонецЕсли;
		КонецЦикла;
	...
	Исключение
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не удалось загрузить список пользователей по области!";
		Сообщение.Сообщить(); 
	КонецПопытки; 
