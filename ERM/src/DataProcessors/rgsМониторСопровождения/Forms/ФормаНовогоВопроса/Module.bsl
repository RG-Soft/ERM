...
Процедура ЗаполнитьСоставОбсуждающихПоОбласти()
	...
	Попытка
		...
		УчастникиОбсуждения.Загрузить(ТаблицаПользователейОбласти);
		УдалитьДублиТекПользователя();
		НоваяСтрока = УчастникиОбсуждения.Добавить();
		НоваяСтрока.ГУИДПользователя = ГУИДТекПользователя;
		НоваяСтрока.ИмяПользователя = ИмяТекПользователя;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ГУИДПроекта <> "" И СписокГруппПользователейПоОбластям.Количество() = 0 Тогда
		ЗаполнитьСоставОбсуждающихПоУмолчанию(ГУИДПроекта);
		Элементы.ОбластьОтветственных.Видимость = Ложь;
		Элементы.Кому.ТолькоПросмотр = Ложь;
	ИначеЕсли СписокГруппПользователейПоОбластям.Количество() > 0 И ЗначениеЗаполнено(Объект.Проект) Тогда
		НоваяСтрока = УчастникиОбсуждения.Добавить();
		НоваяСтрока.ГУИДПользователя = ГУИДТекПользователя;
		НоваяСтрока.ИмяПользователя = ИмяТекПользователя;
		НоваяСтрока.СторонаПользователя = "Спрашивающий";
		...
		Элементы.Кому.ТолькоПросмотр = Истина;
...
Процедура ПроектПриИзменении(Элемент)
	...
	Если ПроектДо <> Объект.Проект И СписокГруппПользователейПоОбластям.Количество() = 0 Тогда
		ЗаполнитьСоставОбсуждающихПоУмолчанию(ГУИДПроекта);
		Элементы.ОбластьОтветственных.Видимость = Ложь;
		Элементы.Кому.ТолькоПросмотр = Ложь;
		ЗаполнитьРеквизитыФормы();
