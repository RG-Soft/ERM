...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// { RGS AGorlenko 19.03.2020 18:33:48 - S-E-0001345-Отслеживание возможности поменять статус с Invoice paid на любой другой
	РазрешеноМенятьInvoicePaid = ?(Status = Перечисления.InvoiceStatus.InvoicePaid 
					И Запись.Invoice.Source <> Перечисления.ТипыСоответствий.Radius
					И Запись.Invoice.Source <> Перечисления.ТипыСоответствий.Geofit
					И Запись.Invoice.Source <> Перечисления.ТипыСоответствий.MFG, Истина, Ложь);
	// } RGS AGorlenko 19.03.2020 18:35:13 - S-E-0001345-Отслеживание возможности поменять статус с Invoice paid на любой другой
...
// { RGS AGorlenko1 19.03.2020 18:29:46 - S-E-0001345-Запрет менять статус с InvoicePaid на любой другой (по всем источникам, кроме Radius, Geofit, MFG)
&НаКлиенте
Процедура StatusОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	Если РазрешеноМенятьInvoicePaid И ВыбранноеЗначение <> ПредопределенноеЗначение("Перечисление.InvoiceStatus.InvoicePaid") Тогда
		
		ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.InvoiceStatus.InvoicePaid");
		Сообщить("Cannot change invoice status from ""Invoice paid"" to any other");
		
	КонецЕсли;
	
КонецПроцедуры// } RGS AGorlenko1 19.03.2020 18:31:30 - S-E-0001345-Запрет менять статус с InvoicePaid на любой другой (по всем источникам, кроме Radius, Geofit, MFG)
