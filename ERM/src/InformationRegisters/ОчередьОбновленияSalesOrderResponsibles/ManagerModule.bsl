...
Процедура ОтметитьПопыткуВыполнения(Source, Идентификатор1, Идентификатор2) Экспорт
	
	НЗ = РегистрыСведений.ОчередьОбновленияSalesOrderResponsibles.СоздатьНаборЗаписей();
	НЗ.Отбор.Source.Установить(Source);
	НЗ.Отбор.Идентификатор1.Установить(Идентификатор1);
	НЗ.Отбор.Идентификатор2.Установить(Идентификатор2);
	НЗ.Прочитать();
	
	Если НЗ.Количество() > 0 Тогда
		НЗ[0].ПоследняяОбработка = ТекущаяДата();
		НЗ.Записать();
	КонецЕсли;
...
Функция ОбработатьПорцию(РазмерПорции = 10) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.ОчередьОбновленияSalesOrderResponsibles КАК ОчередьОбновленияSalesOrderResponsibles
		|ГДЕ
		|	ДОБАВИТЬКДАТЕ(ОчередьОбновленияSalesOrderResponsibles.ПоследняяОбработка, ЧАС, 4) < &ТекущаяДата
		|
		|УПОРЯДОЧИТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ТекПорция.Source КАК Source,
		|	ВТ_ТекПорция.Идентификатор1 КАК Идентификатор1,
		|	ВТ_ТекПорция.Идентификатор2 КАК Идентификатор2,
		|	ВложенныйЗапрос.SalesOrder,
		|	ВложенныйЗапрос.Responsible
		|ИЗ
		|	ВТ_ТекПорция КАК ВТ_ТекПорция
		...
		|		ГДЕ
		|			ВТ_Ответственные.Идентификатор2 <> НЕОПРЕДЕЛЕНО
		|			И ВТ_Ответственные.Идентификатор2 <> """"
		|			И ВТ_Ответственные.Идентификатор2 <> ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
		|
		|		ОБЪЕДИНИТЬ
		...
		|		ГДЕ
		|			ВТ_Исключения.Source ЕСТЬ NULL) КАК ВложенныйЗапрос
		|		ПО ВТ_ТекПорция.Source = ВложенныйЗапрос.Source
		|		И ВТ_ТекПорция.Идентификатор1 = ВложенныйЗапрос.Идентификатор1
		|		И ВТ_ТекПорция.Идентификатор2 = ВложенныйЗапрос.Идентификатор2
		|ИТОГИ
		|ПО
		|	Source,
		|	Идентификатор1,
	...
	Пока ВыборкаSource.Следующий() Цикл
		...
		Пока ВыборкаИдентификатор1.Следующий() Цикл
			...
			Пока ВыборкаИдентификатор2.Следующий() Цикл
				...
				НеудачнаяОбработка = Ложь;
				
				Пока ВыборкаSalesOrder.Следующий() Цикл
					
					Если Не ЗначениеЗаполнено(ВыборкаSalesOrder.SalesOrder) Тогда
						ОтметитьПопыткуВыполнения(ВыборкаSalesOrder.Source, ВыборкаSalesOrder.Идентификатор1, ВыборкаSalesOrder.Идентификатор2);
						НеудачнаяОбработка = Истина;
						Продолжить;
					КонецЕсли;
				...
				Если Не НеудачнаяОбработка Тогда
