...
Функция СформироватьТаблицуВерсий()
	...
	|	ВерсииОбъектов.УзелИнформационнойБазы
	|ИЗ
	...
	Для Каждого Версия Из ТаблицаВерсий Цикл
		Если ПустаяСтрока(Версия.Узел) Тогда
			Версия.УзелИнформационнойБазы = ИмяЭтойИнформационнойБазы;
		КонецЕсли;
	КонецЦикла;
	
	Результат = ТаблицаВерсий.Скопировать(ТаблицаВерсий.НайтиСтроки(Новый Структура("СоответствуетОтбору", Истина)),
		"НомерВерсии, АвторВерсии, ДатаВерсии, Комментарий, ЕстьДанныеВерсии, ВерсияВладелец, УзелИнформационнойБазы, ТекущаяВерсия");
...
Процедура ОбновитьСписокВерсий()
	...
	Если ЕстьВерсии Тогда
		...
		ТаблицаВерсий.Свернуть("УзелИнформационнойБазы");
		Элементы.ДеревоВерсийУзел.Видимость = ТаблицаВерсий.Количество() > 1 Или ТаблицаВерсий.Количество() = 1 И ТаблицаВерсий[0].УзелИнформационнойБазы <> ИмяЭтойИнформационнойБазы();
	Иначе
