...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	Возврат "ПланОбмена.rgsНачальноеЗаполнениеКлиентовERM.Форма.ПодробнаяИнформация";
КонецФункции
...
Процедура ОпределитьНастройки(Настройки, ИдентификаторНастройки) Экспорт
	...
	Иначе
		Настройки.Вставить("НаименованиеКонфигурацииКорреспондента", НСтр("ru = 'Начальное заполнение клиентов ERM'"));
	КонецЕсли;
...
Функция ПолучитьУзелОбменаПоИдентификаторуКорреспондента(ИдентификаторКорреспондента) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	rgsОбменКлиентамиЧерезУниверсальныйФормат.ИдентификаторКорреспондента = &ИдентификаторКорреспондента
		|	И НЕ rgsОбменКлиентамиЧерезУниверсальныйФормат.ПометкаУдаления";
	
	Запрос.УстановитьПараметр("ИдентификаторКорреспондента", ИдентификаторКорреспондента);
...
Процедура ПолучитьВерсииФорматаОбмена(ВерсииФормата) Экспорт
	ВерсииФормата.Вставить("1.0", rgsМенеджерОбменаНачальноеЗаполнениеКлиентовERM);
КонецПроцедуры
