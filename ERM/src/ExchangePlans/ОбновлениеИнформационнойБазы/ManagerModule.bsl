...
// Ищет УзелИнформационнойБазы плана обмена по очереди и возвращает ссылку на него.
...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число") Или Очередь = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Невозможно получить УзелИнформационнойБазы плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.'");
	КонецЕсли;
