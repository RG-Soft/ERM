...
Функция GetRequestAttachedFiles(Request)
	...
	Пока ВыборкаПрисоединенныхФайлов.Следующий() Цикл
		...
		СписокПрисоединенныхФайловXDTO.ДвоичныеДанныеФайлов.Добавить(ОбменДаннымиXDTOСервер.ОбъектXDTOИзДанныхXDTO(КомпонентыОбмена, СтруктураXDTO, ТипПрисоединенногоФайлаXDTO, ПрисоединенныйФайлXDTO));
...
Функция ПолучитьУзел(ИдентификаторКорреспондента)
	...
	Если НЕ ЗначениеЗаполнено(УзелОбмена) Тогда
		ВызватьИсключение "В базе ERM не настроен УзелИнформационнойБазы обмена для синхронизации с текущей базой.";
	КонецЕсли;
