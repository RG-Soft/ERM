...
Функция ПроверкаПодключения(ИмяПланаОбмена, КодУзлаПланаОбмена, СообщениеОбОшибке)
	...
	// Проверяем наличие узла плана обмена (возможно УзелИнформационнойБазы уже удален).
	Если ПланыОбмена[ИмяПланаОбмена].НайтиПоКоду(КодУзлаПланаОбмена).Пустая() Тогда
		СообщениеОбОшибке = НСтр("ru = 'Заданный УзелИнформационнойБазы плана обмена не найден. Обратитесь к администратору приложения.'; en = 'Presetting not foung. Please contact with application administrator'");
		Возврат Ложь;
	КонецЕсли;
