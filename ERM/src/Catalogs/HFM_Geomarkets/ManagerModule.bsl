...
Функция ВыгрузитьДанныеДляPowerBI(Ссылка, Операция, ПараметрыЛогирования) Экспорт
	...
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "Код, ПометкаУдаления, Наименование, Родитель, ManagementGeomarket");
	...
	Если Операция = Перечисления.ОперацииPowerBI.Create Тогда
		...
		ТекОбъект = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_HFM_Geomarkets.СоздатьОбъект();
	...
	Иначе
		...
		РегистрыСведений.ЛогВыгрузкиPowerBI.ЗалогироватьСобытие(ПараметрыЛогирования);
		СсылкаВоВнешнемИсточнике = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_HFM_Geomarkets.ПолучитьСсылку(Строка(Ссылка.УникальныйИдентификатор()));
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитов.ManagementGeomarket) Тогда
		ТекОбъект.ManagementGeomarketID = Строка(СтруктураРеквизитов.ManagementGeomarket.УникальныйИдентификатор());
	Иначе
		ТекОбъект.ManagementGeomarketID = NULL;
	КонецЕсли;
