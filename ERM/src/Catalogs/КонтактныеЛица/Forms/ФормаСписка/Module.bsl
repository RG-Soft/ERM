...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		// { RGS TAlmazova 24.04.2019 11:39:53 - TODO у пользователя ошибка при открытии формы, заглушка
		//Элементы.ФормаИспользоватьОсновным.Видимость = Ложь;
		Если Элементы.Найти("ФормаИспользоватьОсновным") <> Неопределено Тогда
			Элементы.ФормаИспользоватьОсновным.Видимость = Ложь;
		КонецЕсли;
		// } RGS TAlmazova 24.04.2019 11:40:44 - TODO у пользователя ошибка при открытии формы, заглушка
