...
Процедура ПриЗаписи(Отказ)
	...
	Если AccountType = Перечисления.ТипыКлиентов.ParentAccount Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |			,
		               |			ГоловнойКонтрагент = &ParentClient
		               |				И Контрагент.TCC <> &TCC
		               |				И НЕ Контрагент.ПометкаУдаления) КАК ИерархияКонтрагентовСрезПоследних";
		Запрос.УстановитьПараметр("ParentClient", Ссылка);
		Запрос.УстановитьПараметр("TCC", TCC);
...
Процедура ПроверитьДублиПоНаименованию(Отказ)
	...
	Если Не РольДоступна("Администрирование") И Не Результат.Пустой() Тогда
