...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(ТекущийОбъект.ОсновнойБанковскийСчет) Тогда
		//Если основной банковский счет не заполнен, а данные для сохранения есть,
		//то нужно установить ссылку для поля ОсновнойБанковскийСчет
		//В ПриЗаписи эта ссылка будет использована для сохранения основного банковского счета
		Если ЗначениеЗаполнено(БанковскийСчет.НомерСчета)
		   И ЗначениеЗаполнено(БанковскийСчет.Банк) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ИННВведенКорректно И (Объект.КППВведенКорректно ИЛИ ЭтоФизЛицо) Тогда
...
Процедура ЮрАдресКонтрагентаНажатие(Элемент, СтандартнаяОбработка)
	...
	rgsУправлениеКонтактнойИнформациейКлиент.ГиперссылкаНажатие(ЭтаФорма, Элемент, СтандартнаяОбработка);
...
Процедура ПочтовыйАдресКонтрагентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	rgsУправлениеКонтактнойИнформациейКлиент.ПредставлениеНачалоВыбора(ЭтаФорма, Элемент, Модифицированность, СтандартнаяОбработка);
...
Процедура ПодготовитьФормуНаСервере()
	...
	Если Параметры.Свойство("ЗначенияЗаполнения") 
		...
		Если ТипЗнч(ОграничениеВидаКонтрагента) = Тип("ПеречислениеСсылка.ЮридическоеФизическоеЛицо") Тогда
			...
			Если ОграничениеВидаКонтрагента = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
				СписокВыбораВида.Добавить("ФизическоеЛицо", "Natural person");
			...
			ИначеЕсли ОграничениеВидаКонтрагента = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
				СписокВыбораВида.Добавить("ЮридическоеЛицо", "Legal entity");
				СписокВыбораВида.Добавить("ОбособленноеПодразделение", "General subject");
				СписокВыбораВида.Добавить("ГосударственныйОрган", "State agency");
			КонецЕсли;
	...
	Элементы.AccountType.ТолькоПросмотр = НЕ Объект.Ссылка.Пустая();
...
Процедура УправлениеФормой(Форма)
	...
	АдресЗаголовок = НСтр("ru='Address and phone number'");
...
Процедура РазместитьКонтактнуюИнформацию()
	...
	ИсключаемыеВиды.Добавить(Справочники.ВидыКонтактнойИнформации.CRMАдресКонтрагента);
	...
	rgsУправлениеКонтактнойИнформациейКлиентСервер.УстановитьПустоеПредставлениеАдреса(ЭтотОбъект);
...
Процедура ОбработатьЗакрытиеФормыСпискаДублей(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ОтобразитьРезультатПроверкиКонтрагентовВФорме(ЭтотОбъект);
...
Процедура УстановитьВидимостьЭлементов()
	...
	// { RGS AGorlenko 22.08.2016 18:03:28 - 
	//Элементы.НаименованиеПолное.Заголовок = ?(КонтрагентФизическоеЛицо, НСтр("ru = 'ФИО'"), НСтр("ru = 'Полное наименование'"));
	Элементы.НаименованиеПолное.Заголовок = ?(КонтрагентФизическоеЛицо, НСтр("ru = 'ФИО'"), НСтр("ru = 'Full name'"));
	// } RGS AGorlenko 22.08.2016 18:03:30 - 
	...
	Если НЕ КонтрагентЗарегистрированВРФ Тогда
		Элементы.РегистрационныйНомер.Заголовок= НСтр("ru = 'Reg. number'");
		Элементы.РегистрационныйНомер.Подсказка 		= НСтр("ru = 'Registration number assigned to a foreign organization in the country of registration (incorporation)'");
		Элементы.РегистрационныйНомер.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
		Элементы.РегистрационныйНомер.ПодсказкаВвода 	= НСтр("ru = 'Registration number in the country of registration'");
		Элементы.РегистрационныйНомер.Ширина 			= 31;
		Элементы.РегистрационныйНомер.Маска 			= "";
		Элементы.ИНН.Подсказка = НСтр("ru = 'Taxpayer identification number assigned to a foreign organization 
											|as soon as registration with the tax authority of the Russian Federation'");
		Элементы.ИНН.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
	ИначеЕсли КонтрагентФизическоеЛицо Тогда
		Элементы.РегистрационныйНомер.Заголовок		= НСтр("ru = 'PSRNSP'"); // ОГРНИП
		Элементы.РегистрационныйНомер.Подсказка 		= 
			НСтр("ru = 'Primary State Registration Number of the Sole Proprietor'"); 
		...
		Элементы.ИНН.Подсказка = НСтр("ru = 'Taxpayer Identification Number'");
		Элементы.ИНН.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
	Иначе
		// Юридическое лицо
		Элементы.РегистрационныйНомер.Заголовок= НСтр("ru = 'PSRN'"); // ОГРН
		Элементы.РегистрационныйНомер.Подсказка 		= 
			НСтр("ru = 'Primary State Registration Number'");
		Элементы.РегистрационныйНомер.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
		Элементы.РегистрационныйНомер.ПодсказкаВвода 	= "";
	...
	УстановитьВидимостьДатыДеактивации(ЭтаФорма);
...
Процедура ВыполнитьЗаполнениеРеквизитовКонтрагента(СтруктураРеквизитов)
	...
	ЗаполнитьЭлементКонтактнойИнформации("КонтактнаяИнформацияПолеCRMАдресКонтрагента", СтруктураРеквизитов.ЮридическийАдрес);
...
Процедура ЗаписатьОсновнойБанковскийСчет(ТекущийОбъект)
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.ОсновнойБанковскийСчет) И ИзмененБанковскийСчет Тогда
		...
		//ОбъектРегистрации.Валютный = (ОбъектРегистрации.ВалютаденежныхСредств <> ОбщегоНазначенияВызовСервераПовтИсп.ПолучитьВалютуРегламентированногоУчета());
...
Процедура ЗаполнитьРеквизитыКонтрагента(РеквизитыКонтрагента, ПроверитьИНН)
	...
	Если ЭтоЮрЛицо Тогда
		...
		ЗаполнитьЭлементКонтактнойИнформации("КонтактнаяИнформацияПолеCRMАдресКонтрагента", РеквизитыКонтрагента.ЮридическийАдрес);
...
&НаСервере
Процедура УстановитьДополнительнуюИнформацию()
	...
	Если Результат.Пустой() Тогда
		ParentClient = Справочники.Контрагенты.ПустаяСсылка();
	Иначе
		Выборка = Результат.Выбрать();
		Выборка.Следующий();
		ParentClient = Выборка.ParentClient;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СостояниеАктивностиПриИзменении(Элемент)
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьВидимостьДатыДеактивации(Форма)
	
	Форма.Элементы.ДатаДеактивации.Видимость = Форма.Объект.СостояниеАктивности = ПредопределенноеЗначение("Перечисление.СостоянияАктивностиКонтрагентов.Неактивен");
