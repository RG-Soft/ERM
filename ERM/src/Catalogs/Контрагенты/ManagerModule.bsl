...
// { RGS AGorlenko 03.03.2016 17:54:27 - убираем ненужный функционал
...
//	
//КонецФункции
// } RGS AGorlenko 03.03.2016 17:54:37 - убираем ненужный функционал
...
Процедура ЗаполнитьОсновныеДоговораКонтрагентов(Параметры) Экспорт
	...
	// { RGS AGorlenko 03.03.2016 17:56:26 - отключаем лишний функционал
	//Запрос = Новый Запрос;
	//Запрос.Текст = 
	//"ВЫБРАТЬ ПЕРВЫЕ 1000
	...
	//	КонецЕсли;
	//КонецЕсли;
	// } RGS AGorlenko 03.03.2016 17:56:39 - отключаем лишний функционал
...
Функция ВыгрузитьДанныеДляPowerBI(Ссылка, Операция, ПараметрыЛогирования) Экспорт
	...
	СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Ссылка, "CRMID, ПометкаУдаления, Наименование,
		| AccountType, Intercompany, СостояниеАктивности, CreditRating, CreditAssessmentId, CreditAssessmentStatus,
		| CreditLimit, crmPostalCode, crmCountry, crmCity, crmStreetAddress");
	...
	Если Операция = Перечисления.ОперацииPowerBI.Create Тогда
		...
		ТекОбъект = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_Clients.СоздатьОбъект();
	...
	Иначе
		...
		РегистрыСведений.ЛогВыгрузкиPowerBI.ЗалогироватьСобытие(ПараметрыЛогирования);
		СсылкаВоВнешнемИсточнике = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_Clients.ПолучитьСсылку(Строка(Ссылка.УникальныйИдентификатор()));
		ТекОбъект = СсылкаВоВнешнемИсточнике.ПолучитьОбъект();
	КонецЕсли;
	ТекОбъект.DeletionMark = СтруктураРеквизитов.ПометкаУдаления;
	ТекОбъект.CRMID = СтруктураРеквизитов.CRMID;
	ТекОбъект.Description = СтруктураРеквизитов.Наименование;
	ТекОбъект.AccountType = Строка(СтруктураРеквизитов.AccountType);
	ТекОбъект.Intercompany = СтруктураРеквизитов.Intercompany;
	ТекОбъект.ActiveState = Строка(СтруктураРеквизитов.СостояниеАктивности);
	ТекОбъект.CreditRating = Строка(СтруктураРеквизитов.CreditRating);
	ТекОбъект.CreditAssessmentId = СтруктураРеквизитов.CreditAssessmentId;
	ТекОбъект.CreditAssessmentStatus = Строка(СтруктураРеквизитов.CreditAssessmentStatus);
	ТекОбъект.CreditLimit = СтруктураРеквизитов.CreditLimit;
	ТекОбъект.crmPostalCode = СтруктураРеквизитов.crmPostalCode;
	ТекОбъект.crmCountry = СтруктураРеквизитов.crmCountry;
	ТекОбъект.crmCity = СтруктураРеквизитов.crmCity;
	ТекОбъект.crmStreetAddress = СтруктураРеквизитов.crmStreetAddress;
	...
	РегистрыСведений.ИерархияКонтрагентов.ВыгрузитьДанныеДляPowerBI(Ссылка, ПараметрыЛогирования);
...
// Возвращаемое значение:
// 	Структура - включает поля Результат, Сообщение.
Функция УдалитьДанныеВPowerBI(Ссылка, ПараметрыЛогирования) Экспорт
	...
	РегистрыСведений.ЛогВыгрузкиPowerBI.ЗалогироватьСобытие(ПараметрыЛогирования);
	РегистрыСведений.ИерархияКонтрагентов.УдалитьДанныеВPowerBI(Ссылка, ПараметрыЛогирования);
	Попытка
		ТекОбъект.Удалить();
	Исключение
		СтруктураРезультата.Сообщение = ОписаниеОшибки();
