...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Менеджер = ПланыОбмена[Параметры.УзелИнформационнойБазы.Метаданные().Имя];
	
	СистемнаяИнформация = Новый СистемнаяИнформация;
	Если Параметры.УзелИнформационнойБазы = Менеджер.ЭтотУзел() Тогда
		ВызватьИсключение
			НСтр("ru = 'Создание начального образа для данного узла невозможно.'");
	Иначе
		ВидБазы = 0; // файловая база
		ТипСУБД = "";
		Узел = Параметры.УзелИнформационнойБазы;
...
Процедура СоздатьНачальныйОбраз(Команда)
	...
	Иначе
		...
		ПараметрыЗадания.Вставить("УзелИнформационнойБазы", Узел);
