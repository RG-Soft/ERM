...
Функция ПолучитьТекстЗапроса(ГлубинаDSO) Экспорт
	...
	ШаблонОбъединенияОстатков = "
		...
		|					{(&ПериодОстатков%1)},
		|					Source В (&Sources)
		|						И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК UnallocatedMemoОстатки) КАК ВложенныйЗапросОстатки%1
	...
	МассивРесурсовБиллингаБезПереоценки = Новый Массив;
	...
	Для ТекУровень = 0 По ГлубинаDSO - 24 Цикл
		...
		МассивРесурсовБиллингаBilling2.Добавить(СтрШаблон("ВЫРАЗИТЬ(СУММА(Billing.Amount / ВнутренниеКурсыВалютСрезПоследних%1.Курс * ВнутренниеКурсыВалютСрезПоследних%1.Кратность) КАК ЧИСЛО(15,2)) КАК USDAmount%1", ТекУровеньСтрокой));
		МассивРесурсовБиллингаБезПереоценки.Добавить("СУММА(Billing.USDAmount)");
	...
	ТекстЗапроса = 
		...
		|	И Billing.Method В (&МетодыРасчетаБиллингаСПереоценкой)
		|
		|СГРУППИРОВАТЬ ПО
		|	Billing.Период,
		|	Billing.Client,
		...
		|	Billing.AU,
		|	Billing.Source,
		|	" + СтрСоединить(МассивРесурсовБиллингаБезПереоценки, "," + Символы.ПС) + "
		|ИЗ
		|	РегистрСведений.Billing КАК Billing
		|ГДЕ
		|	Billing.Период <= &Период
		|	И Billing.Активность
		|	И Billing.Период >= ДОБАВИТЬКДАТЕ(&Период, МЕСЯЦ, -&ГлубинаDSO)
		|	И НЕ Billing.Method В (&МетодыРасчетаБиллингаСПереоценкой)
		|
		|СГРУППИРОВАТЬ ПО
		|	Billing.Период,
		|	Billing.Client,
