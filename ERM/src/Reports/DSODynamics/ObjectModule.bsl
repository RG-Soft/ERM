...
//	НаборДанных.АвтоЗаполнениеДоступныхПолей = Ложь;
	...
	//Поле.Поле = "AR0";
	//Поле.ПутьКДанным = "AR0";
	//Поле.Заголовок = "AR " + Формат(ДобавитьМесяц(ПараметрПериодDSO.Значение.ДатаОкончания, -0), "ДФ=MM.yyyy");
	//Поле.ОграничениеИспользования.Поле = Ложь;
	//Поле.ОграничениеИспользования.Группировка = Ложь;
	//Поле.ОграничениеИспользования.Порядок = Ложь;
...
Процедура ДобавитьВычисляемыеПоля(КонецПериода, КоличествоМесяцев)
	...
	Для Смещение = 0 По КоличествоМесяцев Цикл
		...
		ПолеDSO.Выражение = "rgsРасчетПоказателей.РассчитатьDSO(AR" + СмещениеСтрокой + ", &Период, Billing" + Формат(0 + Смещение, "ЧН=0; ЧГ=0") + "_" + СмещениеСтрокой + ", Billing" + Формат(1 + Смещение, "ЧГ=0")
...
Процедура ДобавитьРесурсы(ГлубинаDSO)
	...
	Для Смещение = 0 По ГлубинаDSO - 24 Цикл
		...
		ПолеDSO.ПутьКДанным = "DSO_" + Смещение;
		ПолеDSO.Выражение = "rgsРасчетПоказателей.РассчитатьDSO(СУММА(AR" + СмещениеСтрокой + "), &Период, СУММА(Billing" + Формат(0 + Смещение, "ЧН=0; ЧГ=0") + "_" + СмещениеСтрокой + "), СУММА(Billing" + Формат(1 + Смещение, "ЧГ=0")
