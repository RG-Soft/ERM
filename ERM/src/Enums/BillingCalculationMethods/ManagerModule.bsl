...
Функция ПолучитьТекстЗапросаTransactionClassification()
	...
	|	ВТ_ДанныеТранзакций.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket КАК ManagementGeomarket,
	...
	|ВТ_ДанныеТранзакций.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket,
...
Функция ПолучитьТекстЗапросаARandCollection()
	...
	|	ВложенныйЗапросОстатки.Currency КАК Currency,
	...
	|				&ПериодОстатков,
	|				Source В (&Sources)
	|					И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК UnbilledARОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				&ПериодОстатков,
	|				Source В (&Sources)
	|					И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК BilledARОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				&ПериодОстатков,
	|				Source В (&Sources)
	|					И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК UnallocatedCashОстатки
	|	ГДЕ
	|		ВЫБОР
	...
	|				&ПериодОстатков,
	|				Source В (&Sources)
	|					И Account.БазовыйЭлемент В ИЕРАРХИИ (&СчетAR)) КАК ManualTransactionsОстатки
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|				&ПериодОстатков,
	|				Source В (&Sources)
	|					И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК UnallocatedMemoОстатки) КАК ВложенныйЗапросОстатки
	...
	|			,
	|			Source В (&Sources)
	|				И Account.БазовыйЭлемент В ИЕРАРХИИ(&СчетAR)) КАК PaymentsОбороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВнутренниеКурсыВалют.СрезПоследних(&ПериодОстатков, ) КАК ВнутренниеКурсыВалютСрезПоследних
	...
	|	ЕСТЬNULL(ВТ_ПолныеОстатки.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket, ВТ_Collection.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket) КАК ManagementGeomarket,
...
Функция ПолучитьТекстЗапросаRevenue(Метод)
	...
	|	RevenueОбороты.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket КАК ManagementGeomarket,
...
Функция ПолучитьБиллинг(НачалоПериода, КонецПериода) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ПолучитьБиллингTradeRevenue(НачалоПериода, КонецПериода), ТаблицаБиллинга);
...
Функция ПолучитьБиллингARandCollection(НачалоПериода, КонецПериода) Экспорт
	...
	СчетARВерхнегоУровня = rgsНастройкаКонфигурацииПовтИсп.ПолучитьЗначениеНастройки("СчетARВерхнегоУровня");
	Запрос.УстановитьПараметр("СчетAR", СчетARВерхнегоУровня);
...
Функция ПолучитьБиллингTradeRevenue(НачалоПериода, КонецПериода) Экспорт
	
	Запрос = Новый Запрос(ПолучитьТекстЗапроса(Перечисления.BillingCalculationMethods.TradeRevenue));
	Запрос.УстановитьПараметр("Sources", ПолучитьИсточникиПоМетоду(Перечисления.BillingCalculationMethods.TradeRevenue));
	Запрос.УстановитьПараметр("НачалоПериода", НачалоПериода);
	Запрос.УстановитьПараметр("КонецПериода", КонецПериода);
