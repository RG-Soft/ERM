...
//		КонецЕсли;
		...
		Если (Окр(РеквизитыТранзакции.Amount,2) <> Окр(ДетализацияПоКлиенту.Итог("Amount"),2) ИЛИ Окр(РеквизитыТранзакции.BaseAmount,2) <> Окр(ДетализацияПоКлиенту.Итог("BaseAmount"),2)) 
		...
		Если НЗ_BilledAR.Количество() > 0 Тогда
			...
			Для Каждого Запись Из НЗ_BilledAR Цикл
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					ИзменитьКлиентаВДокументе(Запись.Invoice, СтрокаТабЧасти.Client, Запись.Source, Отказ);
					НовоеДвижение.Invoice = Запись.Invoice;
					НовоеДвижение.Client = СтрокаТабЧасти.Client;
					НовоеДвижение.ClientID = ПолучитьClientIDИзMapping(СтрокаТабЧасти.Client, Запись.Source, Отказ);
		...
		Если НЗ_UnbilledAR.Количество() > 0 Тогда
			...
			Для Каждого Запись Из НЗ_UnbilledAR Цикл
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					ИзменитьКлиентаВДокументе(Запись.SalesOrder, СтрокаТабЧасти.Client, Запись.Source, Отказ);
		...
		Если НЗ_UnallocatedCash.Количество() > 0 Тогда
			...
			Для Каждого Запись Из НЗ_UnallocatedCash Цикл
				...
				Для Каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					ИзменитьКлиентаВДокументе(Запись.CashBatch, СтрокаТабЧасти.Client, Запись.Source, Отказ);
		...
		Если НЗ_UnallocatedMemo.Количество() > 0 Тогда
			...
			Для Каждого Запись Из НЗ_UnallocatedMemo Цикл
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					ИзменитьКлиентаВДокументе(Запись.Memo, СтрокаТабЧасти.Client, Запись.Source, Отказ);
		...
		Если НЗ_ManualTransactions.Количество() > 0 Тогда
			...
			Для Каждого Запись Из НЗ_ManualTransactions Цикл
				...
				Если НЕ ЗначениеЗаполнено(RelatedDocumentDate) Тогда
					
					RelatedDocumentDate = НовоеДвижениеСторно.РучнаяКорректировка.Дата;
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					НовоеДвижение.РучнаяКорректировка = НайтиСоздатьJV(СтрокаТабЧасти.Client, Отказ);
					НовоеДвижение.Client = СтрокаТабЧасти.Client;
					НовоеДвижение.ClientID = ПолучитьClientIDИзMapping(СтрокаТабЧасти.Client, Запись.Source, Отказ);
					НовоеДвижение.Amount = СтрокаТабЧасти.Amount * ЗнакДвижения;
					НовоеДвижение.BaseAmount = СтрокаТабЧасти.BaseAmount * ЗнакДвижения;
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					...
					Если ТипЗнч(Запись.Document) = Тип("ДокументСсылка.РучнаяКорректировка") Тогда
						...
						Если НЕ ЗначениеЗаполнено(RelatedDocumentDate) Тогда
							RelatedDocumentDate = НовоеДвижениеСторно.РучнаяКорректировка.Дата;
						КонецЕсли;
						НовоеДвижение.Document = НайтиСоздатьJV(СтрокаТабЧасти.Client, отказ);
					Иначе
						ИзменитьКлиентаВДокументе(Запись.Document, СтрокаТабЧасти.Client, Запись.Source, Отказ);
...
//						КонецЕсли;
		...
		НЗ_InvoicedDebts = РегистрыНакопления.InvoicedDebts.СоздатьНаборЗаписей();
		НЗ_InvoicedDebts.Отбор.Регистратор.Установить(ДокументОснование);
		НЗ_InvoicedDebts.Прочитать();
		
		Если НЗ_InvoicedDebts.Количество() > 0 Тогда
			
			ДвиженияInvoicedDebts = Движения.InvoicedDebts;
			
			Для Каждого Запись Из НЗ_InvoicedDebts Цикл
				НовоеДвижениеСторно = ДвиженияInvoicedDebts.Добавить();
				ЗаполнитьЗначенияСвойств(НовоеДвижениеСторно,Запись);
				НовоеДвижениеСторно.Период = Запись.Период;
				НовоеДвижениеСторно.Amount = -Запись.Amount;
				НовоеДвижениеСторно.BaseAmount = -Запись.BaseAmount;
				...
				Для каждого СтрокаТабЧасти Из ДетализацияПоКлиенту Цикл
					НовоеДвижение = ДвиженияInvoicedDebts.Добавить();
					ЗаполнитьЗначенияСвойств(НовоеДвижение, ЭтотОбъект);
					НовоеДвижение.Период = Запись.Период;
					НовоеДвижение.Source = Запись.Source;
					ИзменитьКлиентаВДокументе(Запись.Invoice, СтрокаТабЧасти.Client, Запись.Source, Отказ);
			...
			ДвиженияInvoicedDebts.Записывать = Истина;
...
Процедура ИзменитьКлиентаВДокументе(Документ, Клиент, Source, Отказ)
	
	ClientID = ПолучитьClientIDИзMapping(Клиент, Source, Отказ);
...
Функция ПолучитьClientIDИзMapping(Клиент, Source, Отказ)
	...
	Запрос.Текст = 
		...
		|	НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних.ТипОбъектаВнешнейСистемы КАК ТипОбъектаВнешнейСистемы,
		...
		|ГДЕ
		|	НЕ НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних.Идентификатор ПОДОБНО ""#empty#""";
...
Функция НайтиСоздатьJV(Клиент, Отказ)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	РучнаяКорректировка.Ссылка КАК РучнаяКорректировка
		|ИЗ
		|	Документ.РучнаяКорректировка КАК РучнаяКорректировка
		|ГДЕ
		|	РучнаяКорректировка.Source = &Source
		|	И РучнаяКорректировка.Company = &Company
		|	И РучнаяКорректировка.AU = &AU
		|	И РучнаяКорректировка.Account = &Account
		|	И РучнаяКорректировка.Currency = &Currency
		|	И РучнаяКорректировка.Client = &Client
		|	И НАЧАЛОПЕРИОДА(РучнаяКорректировка.Дата, МЕСЯЦ) = &Дата
	...
	Запрос.УстановитьПараметр("Дата", НачалоМесяца(RelatedDocumentDate));
	...
	Иначе
		...
		РучнаяКорректировкаОбъект.Дата = RelatedDocumentDate;
...
//		ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
		...
		ClientID = ПолучитьClientIDИзMapping(Клиент, РеквизитыТранзакции.Source, Отказ);
		
		Если НЕ Отказ И НЕ Клиент.Предопределенный Тогда
