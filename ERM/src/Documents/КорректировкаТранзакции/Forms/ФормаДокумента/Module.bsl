...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
		ЗначенияРеквизитовТранзакции = ПолучитьРеквизитыТранзакции(Отказ);
		ТранзакцияClient = ЗначенияРеквизитовТранзакции.Client;
		ТранзакцияAccount = ЗначенияРеквизитовТранзакции.Account;
		ТранзакцияCompany = ЗначенияРеквизитовТранзакции.Company;
		ТранзакцияCurrency = ЗначенияРеквизитовТранзакции.Currency;
		ТранзакцияAU = ЗначенияРеквизитовТранзакции.AU;
		ТранзакцияLegalEntity = ЗначенияРеквизитовТранзакции.LegalEntity;
		// { RGS TAlmazova 15.06.2018 11:21:13 - S-E-0000759
		ТранзакцияLocation = ЗначенияРеквизитовТранзакции.Location;
		ТранзакцияSubSubSegment = ЗначенияРеквизитовТранзакции.SubSubSegment;
		// } RGS TAlmazova 15.06.2018 11:21:22 - S-E-0000759
		Если НЕ ЗначенияРеквизитовТранзакции.RelatedDocumentDate = Неопределено Тогда
			Объект.RelatedDocumentDate = ЗначенияРеквизитовТранзакции.RelatedDocumentDate;
		Иначе
			Элементы.RelatedDocumentDate.Видимость = Ложь;
		КонецЕсли;
		Если Параметры.Ключ.Пустая() Тогда
			...
			СтрокаКлиент.Amount = ЗначенияРеквизитовТранзакции.Amount;
			СтрокаКлиент.BaseAmount = ЗначенияРеквизитовТранзакции.BaseAmount;
		КонецЕсли;
...
&НаСервере
Процедура УстановитьДатуСвязаннойРучнойКорректировки(СвязаннаяРучнаяКорректировка)
	
	Если НЕ ЗначениеЗаполнено(Объект.RelatedDocumentDate) Тогда
		Объект.RelatedDocumentDate = СвязаннаяРучнаяКорректировка.Дата;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьРеквизитыТранзакции(Отказ)
	Возврат Документы.КорректировкаТранзакции.ПолучитьРеквизитыТранзакции(Объект.ДокументОснование, Отказ);
КонецФункции
...
Процедура РаспределитьСумму()
	...
	ЗначенияРеквизитовТранзакции = ПолучитьРеквизитыТранзакции(Ложь);
	ОстатокСуммы = ЗначенияРеквизитовТранзакции.Amount - Объект.ДетализацияПоКлиенту.Итог("Amount");
	//ОстатокСуммыUSD = ЗначенияРеквизитовТранзакции.BaseAmount - Объект.ДетализацияПоКлиенту.Итог("BaseAmount");
	Если ОстатокСуммы <> 0 Тогда
		...
		Если (ЗначенияРеквизитовТранзакции.BaseAmount) <> 0 Тогда
			ТаблицаСуммВалюта = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(ЗначенияРеквизитовТранзакции.BaseAmount, ТаблицаСумм);
