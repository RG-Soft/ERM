...
Процедура РаспределитьСумму()
	...
	ЗначенияРеквизитовТранзакции = ПолучитьРеквизитыТранзакции();
	ОстатокСуммы = ЗначенияРеквизитовТранзакции.ТранзакцияBaseAmount - Объект.ДетализацияПоКлиенту.Итог("BaseAmount");
	Если ОстатокСуммы <> 0 Тогда
		...
		НоваяСтрока.BaseAmount = ОстатокСуммы; 
		ТаблицаСуммВалюта = ТабличнаяЧастьДокумента.ВыгрузитьКолонку("BaseAmount");
		ТаблицаСумм = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(ЗначенияРеквизитовТранзакции.ТранзакцияAmount, ТаблицаСуммВалюта);
		ТабличнаяЧастьДокумента.ЗагрузитьКолонку(ТаблицаСумм, "Amount");
