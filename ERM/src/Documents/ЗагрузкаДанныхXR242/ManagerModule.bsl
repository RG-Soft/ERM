...
Процедура СоздатьКорректировкиПоИнвойсам(НайденныеInvoice, МинимальнаяДатаИнвойса, МаксимальнаяДатаИнвойса, ДокументЗагрузки)
	...
	|	ВТ_Инвойсы.CustomerNumber КАК ClientID,
	...
	|ГДЕ
	|	ЕСТЬNULL(InvoicedDebtsОбороты.AmountОборот, 0) = 0"
	;
	...
	Если ТЗ_ДляКорректировки.Количество() > 0 Тогда
		...
		Док.ТипКорректировки = Перечисления.ТипыКорректировкиРегистров.ЗагрузкаXR242;
...
Процедура ДозаполнитьИнвойс(СтрокаТЗ)
	...
	РГСофтКлиентСервер.УстановитьЗначение(ИнвойсОбъект.ClientID, СтрокаТЗ.ClientID);
