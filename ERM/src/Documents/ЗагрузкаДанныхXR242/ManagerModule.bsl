...
Процедура ОбновитьРеквизитыInvoices(СтруктураПараметров, АдресХранилища) Экспорт
	...
	|		И ЕСТЬNULL(КлючиSalesOrdersB.SalesOrder.JobEndDate, ДАТАВРЕМЯ(1, 1, 1)) = ДАТАВРЕМЯ(1, 1, 1)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Выборка.Invoice) Тогда
				...
				Если ЗначениеЗаполнено(Выборка.SalesOrder) И JobEndDate <> Дата(1, 1, 1) Тогда
					
					SalesOrderОбъект = Выборка.SalesOrder.ПолучитьОбъект();
				...
				Если ЗначениеЗаполнено(Выборка.SalesOrderB) И JobEndDate <> Дата(1, 1, 1) Тогда
					
					SalesOrderОбъект = Выборка.SalesOrderB.ПолучитьОбъект();
					РГСофтКлиентСервер.УстановитьЗначение(SalesOrderОбъект.JobEndDate, JobEndDate);
					SalesOrderОбъект.ОбменДанными.Загрузка = Истина;
					SalesOrderОбъект.Записать();
					СтрокаТЗ = ОбновленныеSO.Добавить();
