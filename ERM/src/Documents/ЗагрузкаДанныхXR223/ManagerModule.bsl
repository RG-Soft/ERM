...
Процедура СоздатьNoteAllocationПоФайлу(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ДатаОстатков = КонецМесяца(РГСофт.ПреобразоватьВДату(ВыборкаДата.GlDate, "Date"));
	...
	Запрос.Текст =
		...
		|			И КлючиИнвойсов.Company = ВТ_ДанныеФайла.Company
		|			И ВТ_ДанныеФайла.PaymentType = ""I""
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Пока ВыборкаДляNoteAllocation.Следующий() Цикл
		...
		Если ВыборкаДляNoteAllocation.СообщениеОбОшибке = "" Тогда
			...
			ДокументNoteAllocation.Дата = РГСофт.ПреобразоватьВДату(ВыборкаДляNoteAllocation.GlDate, "Date");
