...
//	ОбновленныеMemo.Колонки.Добавить("Memo", Новый ОписаниеТипов("ДокументСсылка.Memo"));
	...
	НЗ = РегистрыСведений.СоответствиеInvoiceCreditNote.СоздатьНаборЗаписей();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НЗ.Очистить();
		НЗ.Отбор.CreditNote.Установить(ВыборкаДетальныеЗаписи.CreditNote);
		НЗ.Отбор.Invoice.Установить(ВыборкаДетальныеЗаписи.InvoiceMemo);
		Если ВыборкаДетальныеЗаписи.TransactionType = "I" Тогда
