...
Функция СоздатьNoteAllocationsНаСервере()
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Note allocations created'");
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
...
Процедура СоздатьNoteAllocations(Команда)
	...
	Иначе
		...
		Если Объект.СозданныеNoteAllocations.Количество() > 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("☆ NoteAllocations  created ☆ Best regards, your EPM ッ");
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("☆ Documents for NoteAllocation not found in file ☆ Best regards, your EPM ッ");
		КонецЕсли;
