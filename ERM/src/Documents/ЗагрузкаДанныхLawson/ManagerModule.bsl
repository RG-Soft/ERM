	...
	Пока rs.EOF() = 0 Цикл
		...
		Для каждого ЭлементСоответствия Из СоответствиеКолонок Цикл
			...
			Попытка
				...
				Если ЭлементСоответствия.Значение = "ArInvoice" и rs.Fields("System").Value <> "GL" И rs.Fields("System").Value <> "CB" И ТекЗначение = Null Тогда
...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КлючиДетальныхРучныхКорректировок.Source,
		|	КлючиДетальныхРучныхКорректировок.Company,
		|	КлючиДетальныхРучныхКорректировок.Journal,
		|	КлючиДетальныхРучныхКорректировок.LineNbr,
		|	КлючиДетальныхРучныхКорректировок.Date,
		|	КлючиДетальныхРучныхКорректировок.РучнаяКорректировка
		|ИЗ
...
Процедура ЗаполнитьПроводкуDSS(ПроводкаDSSОбъект, ДанныеДляЗаполнения, КэшСчетовЛоусон, ПериодЗагрузки, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует)
	...
	Если ДатаИзDescription <> Неопределено Тогда
...
//			СтруктураПоискаРучнойКорректировки.Currency = ПроводкаDSSОбъект.Currency;
			...
			СтруктураПоискаДетальнойРучнойКоррерктировки.Journal = ПроводкаDSSОбъект.Journal;
			СтруктураПоискаДетальнойРучнойКоррерктировки.LineNbr = ПроводкаDSSОбъект.LineNbr;
			СтруктураПоискаДетальнойРучнойКоррерктировки.Date = ПроводкаDSSОбъект.Date;
			...
			Если СтрокиДетальнойРучнойКорректировки.Количество() = 0 Тогда
				...
				НоваяСтрокаКэша.Source = Перечисления.ТипыСоответствий.Lawson;
				НоваяСтрокаКэша.Company = ПроводкаDSSОбъект.Company;
				НоваяСтрокаКэша.Journal = ПроводкаDSSОбъект.Journal;
				НоваяСтрокаКэша.LineNbr = ПроводкаDSSОбъект.LineNbr;
				НоваяСтрокаКэша.Date = ПроводкаDSSОбъект.DateFromDescription;
