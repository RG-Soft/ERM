...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.КлючиИнвойсов КАК КлючиИнвойсов
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_КлючиПоискаInvoiceSalesOrders КАК ВТ_КлючиПоискаInvoiceSalesOrders
		|		ПО (КлючиИнвойсов.ArInvoice = ПОДСТРОКА(ВТ_КлючиПоискаInvoiceSalesOrders.ArInvoice,1,10) + ""B"")
		|			И КлючиИнвойсов.Company = ВТ_КлючиПоискаInvoiceSalesOrders.Company
		|ГДЕ
		|	КлючиИнвойсов.Source = &Source
		|;
		...
		|	РегистрСведений.КлючиSalesOrders КАК КлючиSalesOrders
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_КлючиПоискаInvoiceSalesOrders КАК ВТ_КлючиПоискаInvoiceSalesOrders
		|		ПО (КлючиSalesOrders.ArInvoice = ПОДСТРОКА(ВТ_КлючиПоискаInvoiceSalesOrders.ArInvoice,1,10) + ""B"")
		|			И КлючиSalesOrders.Company = ВТ_КлючиПоискаInvoiceSalesOrders.Company
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
Процедура ЗаполнитьПроводкуDSS(ПроводкаDSSОбъект, ДанныеДляЗаполнения, КэшСчетовЛоусон, ПериодЗагрузки, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует)
	...
	//S-E-0001422 -новый алгоритм по сплит каренси, часть проводок с А в конце. В ключи пишем им B
	...
	//ИначеЕсли Прав(ПроводкаDSSОбъект.ArInvoice, 1) = "A" Тогда
	//	ПроводкаDSSОбъект.ArInvoice = Лев(ПроводкаDSSОбъект.ArInvoice,10) + "B";
	//КонецЕсли;
	//Если Прав(ПроводкаDSSОбъект.ArInvoice, 1) = "A" Тогда
	//	ПроводкаDSSОбъект.ArInvoice = Лев(ПроводкаDSSОбъект.ArInvoice,10) + "B";
	//КонецЕсли;	
	Если (Найти(ДанныеДляЗаполнения.Description, "SPLIT INVC B") > 0) И Прав(ПроводкаDSSОбъект.ArInvoice, 1) <> "B" Тогда
			ПроводкаDSSОбъект.ArInvoice = ПроводкаDSSОбъект.ArInvoice + "B";
	КонецЕсли;
	Если Прав(ПроводкаDSSОбъект.ArInvoice, 1) = "B" Тогда
		ПроводкаDSSОбъект.SplitCurrency = Истина;
	КонецЕсли;
...
Функция СформироватьСвязанныеОбъекты(ПроводкаDSSОбъект, КэшИнвойсов, КэшSalesOrders, КэшCashBatch, КэшBatchAllocation, КэшРучныхКоррерктировок, КэшДетальныхРучныхКоррерктировок, КэшМемо, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, ТранзакцияОбнулена, Отказ, ТекстСообщенияОбОшибках)
	...
	Если ПроводкаDSSОбъект.System = "BL" Тогда
		...
		Если ПроводкаDSSОбъект.SourceCode = "DM" ИЛИ ПроводкаDSSОбъект.SourceCode = "CM" Тогда
			...
			//ArInvoiceДляПоиска нужен так как если документ с A в конце, то это псевдо сплит каренси и нужно подставить B
			ArInvoiceДляПоиска = ?(Прав(ПроводкаDSSОбъект.ArInvoice, 1) = "A",Лев(ПроводкаDSSОбъект.ArInvoice,10) + "B", ПроводкаDSSОбъект.ArInvoice);   
			СтруктураПоискаInvoice.ArInvoice = ArInvoiceДляПоиска;	
			...
			Если СтрокиИнвойса.Количество() = 0 Тогда
				...
				Если ТекИнвойс = Неопределено Тогда
					...
					НоваяСтрокаКэша.ArInvoice = ArInvoiceДляПоиска;
			...
			Если СтрЗаканчиваетсяНа(СтруктураПоискаSalesOrder.ArInvoice, "B") ИЛИ СтрЗаканчиваетсяНа(СтруктураПоискаSalesOrder.ArInvoice, "A") Тогда
			...
			Если СтрокиSalesOrder.Количество() = 0 Тогда
				...
				Если СтрЗаканчиваетсяНа(ПроводкаDSSОбъект.ArInvoice, "B") ИЛИ СтрЗаканчиваетсяНа(ПроводкаDSSОбъект.ArInvoice, "A") Тогда
	...
	ИначеЕсли ПроводкаDSSОбъект.System = "AR" Тогда
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "RQ" ИЛИ ПроводкаDSSОбъект.SourceCode = "RX" Тогда // рассматриваем отдельно, т.к. это должно относиться к существующему бэтчу
			...
			//Если ПроводкаDSSОбъект.AccountLawson.БазовыйЭлемент = rgsНастройкаКонфигурацииПовтИсп.ПолучитьЗначениеНастройки("СчетHFMBilled") ИЛИ ПроводкаDSSОбъект.AccountLawson.БазовыйЭлемент = rgsНастройкаКонфигурацииПовтИсп.ПолучитьЗначениеНастройки("СчетHFMAdvance") Тогда // 120102 или 209000
			Если ПроводкаDSSОбъект.AccountLawson = ПланыСчетов.Lawson.ReceivedNotApplied ИЛИ ПроводкаDSSОбъект.AccountLawson.БазовыйЭлемент = rgsНастройкаКонфигурацииПовтИсп.ПолучитьЗначениеНастройки("СчетHFMAdvance") ИЛИ ПроводкаDSSОбъект.AccountLawson = ПланыСчетов.Lawson.UnidentifiedCashReceipts Тогда // 120102 или 209000
...
Функция СоздатьИнвойс(ПроводкаDSSОбъект, ПолноеЗаполнение = Ложь)
	...
	Если ПроводкаDSSОбъект.SplitCurrency Тогда
		ДокОбъект.DocID = ПроводкаDSSОбъект.ArInvoice;
	ИначеЕсли СтрДлина(ПроводкаDSSОбъект.ArInvoice) = 11 И СтрЗаканчиваетсяНа(ПроводкаDSSОбъект.ArInvoice, "A") Тогда
		ДокОбъект.DocID = Лев(ПроводкаDSSОбъект.ArInvoice, 10) +  "B";
	Иначе
		ДокОбъект.DocID = ПроводкаDSSОбъект.ArInvoice + "B";
	КонецЕсли;
...
Функция НайтиИнвойсSplitCurrency(ПроводкаDSSОбъект)
	...
	Если ПроводкаDSSОбъект.SplitCurrency Тогда
		...
		Если ВыборкаДетальныеЗаписи.Количество() = 1 Тогда
			...
			Если НЕ СтрЗаканчиваетсяНа(ВыборкаДетальныеЗаписи.DocNumber, "B") Тогда
...
Процедура ОбновитьСтатусыSalesOrder(ДокументЗагрузки)
	...
	|					И Account.БазовыйЭлемент В (&СчетAR)) КАК UnbilledARОстатки
	...
	Запрос.УстановитьПараметр("СчетAR", rgsНастройкаКонфигурации.ЗначениеНастройки("СчетARВерхнегоУровня"));
