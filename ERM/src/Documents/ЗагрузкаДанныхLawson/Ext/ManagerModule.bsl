...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВТ_КлючиBatch
		|ИЗ
		|	ВТ_ДанныеФайла КАК ВТ_ДанныеФайла
		|ГДЕ
		|	ВТ_ДанныеФайла.ArBatchNbr <> 0
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ArBatchNbr
		|;
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КлючиCashBatch.ArBatchNbr,
		|	КлючиCashBatch.CashBatch
		|ИЗ
		|	РегистрСведений.КлючиCashBatch КАК КлючиCashBatch
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_КлючиBatch КАК ВТ_КлючиBatch
		|		ПО КлючиCashBatch.ArBatchNbr = ВТ_КлючиBatch.ArBatchNbr
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КлючиBatchAllocation.ArBatchNbr,
		|	КлючиBatchAllocation.BatchAllocation
		|ИЗ
		|	РегистрСведений.КлючиBatchAllocation КАК КлючиBatchAllocation
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_КлючиBatch КАК ВТ_КлючиBatch
		|		ПО КлючиBatchAllocation.ArBatchNbr = ВТ_КлючиBatch.ArBatchNbr
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	КэшСчетовЛоусон = РезультатЗапроса[9].Выгрузить();
	КэшСчетовЛоусон.Индексы.Добавить("КодЧислом");
	
	КэшИнвойсов = РезультатЗапроса[10].Выгрузить();
	КэшИнвойсов.Индексы.Добавить("ArInvoice");
	
	КэшSalesOrders = РезультатЗапроса[11].Выгрузить();
	КэшSalesOrders.Индексы.Добавить("ArInvoice");
	КэшSalesOrders.Индексы.Добавить("Company");
	
	КэшCashBatch = РезультатЗапроса[12].Выгрузить();
	КэшCashBatch.Индексы.Добавить("ArBatchNbr");
	
	КэшBatchAllocation = РезультатЗапроса[13].Выгрузить();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СформироватьСвязанныеОбъекты(ПроводкаDSSОбъект, КэшИнвойсов, КэшSalesOrders, КэшCashBatch, КэшBatchAllocation);
...
Процедура СформироватьСвязанныеОбъекты(ПроводкаDSSОбъект, КэшИнвойсов, КэшSalesOrders, КэшCashBatch, КэшBatchAllocation)
	...
	Если ПроводкаDSSОбъект.System = "BL" Тогда
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "JE" Тогда
			...
			Если СтрокиSalesOrder.Количество() = 0 Тогда
				ТекSalesOrder = СоздатьSalesOrder(ПроводкаDSSОбъект);
				НоваяСтрокаКэша = КэшSalesOrders.Добавить();
				НоваяСтрокаКэша.ArInvoice = ПроводкаDSSОбъект.ArInvoice;
				НоваяСтрокаКэша.Company = ПроводкаDSSОбъект.Company;
				НоваяСтрокаКэша.SalesOrder = ТекSalesOrder;
	...
	ИначеЕсли ПроводкаDSSОбъект.System = "AR" Тогда
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "RP" Тогда
			...
			Иначе
				ДобавитьСвязанныйОбъект(ПроводкаDSSОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.CashBatch, СтрокаBatchAllocation.CashBatch);
			КонецЕсли;
	...
	ИначеЕсли ПроводкаDSSОбъект.System = "GL" Тогда
		...
		Если ПроводкаDSSОбъект.SourceCode = "JE" Тогда
			...
			// TODO RGS AGorlenko 08.06.2016: придумать как-нибудь, чтобы ручные корректировки все вместе получать
			СоздатьРучнуюКорректировку(ПроводкаDSSОбъект);
...
Функция СоздатьИнвойс(ПроводкаDSSОбъект)
...
Функция СоздатьSalesOrder(ПроводкаDSSОбъект, Invoice = Неопределено)
...
Процедура СоздатьРучнуюКорректировку(ПроводкаDSSОбъект)
	
	РучнаяКорректировка = ПолучитьРучнуюКорректировку(ПроводкаDSSОбъект.КонтрагентLawson, ПроводкаDSSОбъект.AccountLawson);
	...
	Если РучнаяКорректировка = Неопределено Тогда
		...
		//НЗ = РегистрыСведений.DSSСвязанныеДокументы.СоздатьНаборЗаписей();
		//НЗ.Отбор.ПроводкаDSS.Установить(ПроводкаDSSОбъект.Ссылка);
		//НЗ.Отбор.ТипСвязанногоОбъекта.Установить(Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.РучнаяКорректировка);
		//НоваяЗапись = НЗ.Добавить();
		//НоваяЗапись.ПроводкаDSS = ПроводкаDSSОбъект.Ссылка;
		//НоваяЗапись.ТипСвязанногоОбъекта = Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.РучнаяКорректировка;
		//НоваяЗапись.СвязанныйОбъект = РучнаяКорректировкаОбъект.Ссылка;
		//НЗ.Записать(Истина);
	...
	Иначе
		...
		ДобавитьСвязанныйОбъект(ПроводкаDSSОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.РучнаяКорректировка, РучнаяКорректировка);
