	...
	Если ПроводкаDSSОбъект.System = "BL" Тогда
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "JE" Тогда
			...
			Иначе
				...
				Если ПроводкаDSSОбъект.JeTypeLawson = "A" И НачалоМесяца(ПроводкаDSSОбъект.AccountingPeriod) = НачалоМесяца(ДатаНачалаЗагрузки) И Не ЭтоПроводкаПоСчетуВыручки Тогда
					ОбнулитьСуммыПроводки(ПроводкаDSSОбъект);
				КонецЕсли;
