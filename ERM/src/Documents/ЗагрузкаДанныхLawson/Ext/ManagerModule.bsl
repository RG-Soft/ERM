...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	КэшCashBatch.Индексы.Добавить("Client, Account, Currency");
	
	КэшBatchAllocation = РезультатЗапроса[12].Выгрузить();
	КэшBatchAllocation.Индексы.Добавить("Client, Account, Currency");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		НайденаОшибкаПриПоискеСвязанныхОбъектов = СформироватьСвязанныеОбъекты(ПроводкаDSSОбъект, КэшИнвойсов, КэшSalesOrders, КэшCashBatch, КэшBatchAllocation, КэшОрганизаций);
...
Функция СформироватьСвязанныеОбъекты(ПроводкаDSSОбъект, КэшИнвойсов, КэшSalesOrders, КэшCashBatch, КэшBatchAllocation, КэшОрганизаций)
	
	НайденаОшибка = Ложь;
	
	СтруктураПоискаSalesOrder = Новый Структура("ArInvoice, Company");
	СтруктураПоискаBatch = Новый Структура("Client, Account, Currency");
