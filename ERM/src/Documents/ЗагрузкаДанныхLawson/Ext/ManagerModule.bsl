	...
	Если ПроводкаDSSОбъект.System = "BL" Тогда
		...
		Если ПроводкаDSSОбъект.SourceCode = "DM" ИЛИ ПроводкаDSSОбъект.SourceCode = "CM" Тогда
			...
			Иначе
				...
				ДозаполнитьИнвойсИзПроводки(ТекИнвойс, ПроводкаDSSОбъект, Отказ, ТекстСообщенияОбОшибках);
...
Процедура ДозаполнитьИнвойсИзПроводки(Инвойс, ПроводкаDSSОбъект, Отказ, ТекстСообщенияОбОшибках)
	РеквизитыИнвойса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Инвойс, "Company,Номер,Дата,Client,Location,SubSubSegment,AU,Account,Currency,Amount");
	Для каждого Реквизит из РеквизитыИнвойса Цикл
		Если Реквизит.Ключ <> "Номер" И Реквизит.Ключ <> "Дата" И Реквизит.Ключ <> "Client" И Реквизит.Ключ <> "Account" И Реквизит.Ключ <> "Amount" Тогда
			Если ЗначениеЗаполнено(Реквизит.Значение) И Реквизит.Значение <> ПроводкаDSSОбъект[Реквизит.Ключ] Тогда
		...
		КонецЕсли;
	КонецЦикла;
	Если ЗначениеЗаполнено(Инвойс["Номер"]) И Инвойс["Номер"] <> ПроводкаDSSОбъект.ArInvoice Тогда
		ТекОшибка = "Different analytics for invoice " + Инвойс["Номер"] + " in the database and the uploaded document.";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	//Если ЗначениеЗаполнено(Инвойс["Дата"]) И Инвойс["Дата"] <> ПроводкаDSSОбъект.DateLawson Тогда
	//	ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
	//	Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
	//		ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
	//	КонецЕсли;
	//	Отказ = Истина;
	//КонецЕсли;
	Если ЗначениеЗаполнено(Инвойс["Account"]) И Инвойс["Account"] <> ПроводкаDSSОбъект.AccountLawson Тогда
		ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Инвойс["Client"]) И Инвойс["Client"] <> ПроводкаDSSОбъект.КонтрагентLawson Тогда
		ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Инвойс["Amount"]) И Инвойс["Amount"] <> ПроводкаDSSОбъект.TranAmount Тогда
		ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Для каждого Реквизит из РеквизитыИнвойса Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если НЕ ЗначениеЗаполнено(Инвойс["Номер"]) Тогда
			ИнвойсОбъект.Номер = ПроводкаDSSОбъект.ArInvoice;
		КонецЕсли;
		//Если НЕ ЗначениеЗаполнено(Инвойс["Дата"]) Тогда
		//	ИнвойсОбъект.Дата = ПроводкаDSSОбъект.DateLawson;
		//КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Инвойс["Account"]) Тогда
			ИнвойсОбъект.Account = ПроводкаDSSОбъект.AccountLawson;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Инвойс["Client"]) Тогда
			ИнвойсОбъект.Client = ПроводкаDSSОбъект.КонтрагентLawson;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Инвойс["Amount"]) Тогда
