...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДатыНачалаЗагрузки.GeoMarket,
		|	ДатыНачалаЗагрузки.DateStartLoading как ДатаНачалаЗагрузки,
		|	ДатыНачалаЗагрузки.DateBalances как ДатаВыверенныхОстатков
		|ИЗ
		|	РегистрСведений.ДатыНачалаЗагрузки КАК ДатыНачалаЗагрузки
		|ГДЕ
		|	ДатыНачалаЗагрузки.Source = &Source";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Даты = ТаблицаДаты.Найти(ВыборкаДетальныеЗаписи.Location.GeoMarket, "GeoMarket");
		Если Даты = Неопределено Тогда
			ТекОшибка = "Start date loading and date balances missing for " + ВыборкаДетальныеЗаписи.Location.GeoMarket;
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "JE" Тогда
			...
			Если СтрокиSalesOrder.Количество() = 0 Тогда
				...
				Если ПроводкаDSSОбъект.JeTypeLawson = "A" Тогда
					Если ПроводкаDSSОбъект.DateLawson > ДатаВыверенныхОстатков Тогда
						ТекОшибка = "Error! The Sale Order date is greater than the date of verified balances for " + ПроводкаDSSОбъект.Ссылка;
						Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
							ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
						КонецЕсли;
						Отказ = Истина;
					ИначеЕсли ПроводкаDSSОбъект.DateLawson < ДатаНачалаЗагрузки Тогда
						ТекОшибка = "Error! Date of loading Sale Order is less than the start date for " + ПроводкаDSSОбъект.Ссылка;
						Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
							ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
						КонецЕсли;
						Отказ = Истина;
