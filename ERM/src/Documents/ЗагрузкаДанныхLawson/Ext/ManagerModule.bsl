...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ТаблицаДанныхКонтрольныеСуммы = ТаблицаДанных.Скопировать(Отбор,"AU, Account, TranAmount, BaseAmount");
	...
	ТаблицаДанныхКонтрольныеСуммы.Свернуть("AU, Account", "TranAmount, BaseAmount");
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КонтрольныеСуммыПриЗагрузке.AU КАК AU,
		...
		|		ПО КонтрольныеСуммыПриЗагрузке.AU = ДанныеТранзакции.ПроводкаDSSAU
		|			И КонтрольныеСуммыПриЗагрузке.Account = ДанныеТранзакции.ПроводкаDSSAccount
		|ГДЕ
		|	(ЕСТЬNULL(ДанныеТранзакции.ПроводкаDSSTranAmount, 0) - ЕСТЬNULL(КонтрольныеСуммыПриЗагрузке.Amount, 0) <> 0
		|			ИЛИ ЕСТЬNULL(ДанныеТранзакции.ПроводкаDSSBaseAmount, 0) - ЕСТЬNULL(КонтрольныеСуммыПриЗагрузке.BaseAmount, 0) <> 0)
		|	И КонтрольныеСуммыПриЗагрузке.ДокументЗагрузки = &ДокументЗагрузки";
	...
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ТекОшибка = "Checksums do not match for AU " + ВыборкаДетальныеЗаписи.AU;
			Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
				ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
		...
		ИначеЕсли ПроводкаDSSОбъект.SourceCode = "JE" Тогда
			...
			Если СтрокиSalesOrder.Количество() = 0 Тогда
				...
				Если ПроводкаDSSОбъект.JeTypeLawson = "A" Тогда
					...
					Если ПроводкаDSSОбъект.AccountingPeriod >= ДатаВыверенныхОстатков И НачалоМесяца(ПроводкаDSSОбъект.AccountingPeriod) > НачалоМесяца(ДатаНачалаЗагрузки) Тогда
