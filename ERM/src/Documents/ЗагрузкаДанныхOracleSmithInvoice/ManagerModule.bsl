...
Процедура ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ДанныеФайла = РаботаСФайлами.ДвоичныеДанныеФайла(СтруктураПараметров.ИсточникДанных);
	...
	RegExpTable.Pattern = "^([\w\s]{10})\s{2}([\w\s]{8})\s{2}([-\w\s]{8})\s{2}([\s\S]{25})\s{2}([\w]{8})\s{2}(\d{2}-(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)-\d{4})\s{2}(\d{2}-(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)-\d{4})\s{2}([\s\S]{17})\s{2}([\s\S]{17})\s{2}((\d{4}).\w{6}.\d{3}.\d{4}.\d{3}.\d{4}.\d{6})\s*\n";
	...
	Если ЧислоВхожденийТаблиц>0 Тогда
		...
		Для СчВхожденийТаблиц = 0 По ЧислоВхожденийТаблиц-1 Цикл
			...
			НоваяСтрокаТаблицаДанных.DocNumber = СокрЛП(МассивДанныхВСтроке.Item(0));
			НоваяСтрокаТаблицаДанных.CustomerNumber = МассивДанныхВСтроке.Item(4);
			НоваяСтрокаТаблицаДанных.CompanyCodeStr = Лев(МассивДанныхВСтроке.Item(11),4);
			НоваяСтрокаТаблицаДанных.InvoiceAmount = СтрЗаменить(СтрЗаменить(СтрЗаменить(МассивДанныхВСтроке.Item(9),",",""),"(","-"),")","");
			НоваяСтрокаТаблицаДанных.CreationDate = РаботаСФайламиСлужебный.ДатаRFC1123(МассивДанныхВСтроке.Item(5));
