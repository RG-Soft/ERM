...
Процедура ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища) Экспорт
	...
	RegExpTable.Pattern = "([(IN|CM)]{0,2}[\d]+)\s+((Invoice-|Credit-)\s*[\d,A-Z,a-z,.\(,\)\s]+)\s+([^\s]{8})\s+(\d{2}-(NOV|OCT)-\d{4})\s+(\d{2}-(NOV|JAN)-\d{4})\s+(\(?[\d.,]+\)?)\s+(\(?[\d.,]+\)?)\s+((\d{4}).\d{6}.\d{3}.\d{4}.\d{3}.\d{4}.\d{6})\s*\n";
	...
	ТаблицаДанных.Колонки.Добавить("CompanyCode", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(4, 0, ДопустимыйЗнак.Неотрицательный)));
	...
	Если ЧислоВхожденийТаблиц>0 Тогда
		...
		Для СчВхожденийТаблиц = 0 По ЧислоВхожденийТаблиц-1 Цикл
			...
			//ЧислоПодвыраженийТаблиц=МассивДанныхВСтроке.Count();
			//Для СчПодвыраженийТаблиц = 0 По ЧислоПодвыраженийТаблиц-1 Цикл
			...
			НоваяСтрокаТаблицаДанных.CompanyCode = МассивДанныхВСтроке.Item(11);
			НоваяСтрокаТаблицаДанных.InvoiceAmount = СтрЗаменить(СтрЗаменить(СтрЗаменить(МассивДанныхВСтроке.Item(8),",",""),"(","-"),")","");
			НоваяСтрокаТаблицаДанных.CreationDate = МассивДанныхВСтроке.Item(4);
				
			//КонецЦикла;
		КонецЦикла;
	Иначе
		//НоваяСтрока = ДеревоСовпадений.Строки.Добавить();
		//НоваяСтрока.НайденнаяСтрока =  "Вхождений шаблона не найдено";
	КонецЕсли;
