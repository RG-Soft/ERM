...
&НаСервере
Функция ОбновитьInvoiceНаСервере()
	...
	//Если Истина Тогда
		...
		Документы.ЗагрузкаДанныхOracleSmithInvoice.ОбновитьРеквизитыInvoice(СтруктураПараметров, АдресХранилища);
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Smith Sales orders update'");
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
			"Документы.ЗагрузкаДанныхOracleSmithInvoice.ОбновитьРеквизитыInvoice", 
			СтруктураПараметров, 
			НаименованиеЗадания);
...
&НаКлиенте
Процедура ОбновитьInvoice(Команда)
	
	Результат = ОбновитьInvoiceНаСервере();
...
Функция ПрочитатьФайлНаСервере()
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, УникальныйИдентификатор);
		Документы.ЗагрузкаДанныхOracleSmithInvoice.ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища);
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		НаименованиеЗадания = НСтр("ru = 'Smith Invoice details loading'");
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
			"Документы.ЗагрузкаДанныхOracleSmithInvoice.ЗагрузитьДанныеИзФайла", 
			СтруктураПараметров, 
			НаименованиеЗадания);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если РольДоступна("ReadOnly") Тогда
		...
		Элементы.ОбновитьInvoice.Доступность = Ложь;
	КонецЕсли;
	//RGS VSobolev S-E-0000786 15.03.2019
...
Процедура ЗагрузитьСтруктуруКолонок(Источник)
	...
	СтруктураКолонок.Загрузить(Документы.ЗагрузкаДанныхOracleSmithInvoice.ПолучитьСтруктуруКолонокТаблицыДанных());
