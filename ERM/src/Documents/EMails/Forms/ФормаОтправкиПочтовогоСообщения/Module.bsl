...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Body") Тогда
		Если Параметры.Свойство("ДобавитьТаблицуИнвойсовВТелоПисьма") Тогда
			 Параметры.Body = Параметры.Body + Символы.ПС + Символы.ПС + Символы.ПС + "#List of Invoices#";
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ОпределитьЗаголовокCopy();
	
КонецПроцедуры

&НаСервере
Процедура ОпределитьЗаголовокCopy()
	
	Если Copies.Количество() > 0 Тогда
		
		МассивEmails = Новый Массив();
		
		Для каждого СтрокаДанных Из Copies Цикл
			МассивEmails.Добавить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаДанных.Recipient, "Mail"));
		КонецЦикла;
		
		ПолучателиCopy = "Copies: " + СтрСоединить(МассивEmails, ", ");
		Элементы.ГруппаКопии.ЗаголовокСвернутогоОтображения = ПолучателиCopy;
		
	Иначе
		
		Элементы.ГруппаКопии.ЗаголовокСвернутогоОтображения = Элементы.ГруппаКопии.Заголовок;
...
&НаКлиенте
Процедура CopiesПриИзменении(Элемент)
	ОпределитьЗаголовокCopy();
КонецПроцедуры
