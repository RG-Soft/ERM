...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ПользовательLDAP) Тогда
		...
		Если ЗначениеЗаполнено(ПочтаПользователя) Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не найдена учетная запись электронной почты с адресом - " + ПочтаПользователя, ЭтотОбъект, , , Отказ);
			КонецЕсли;
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не указан адрес электронной почты для пользователя LDAP - " + ПользовательLDAP, ЭтотОбъект, , , Отказ);
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не привязана учетная запись LDAP к текущему пользователю - " + ТекПользователь, ЭтотОбъект, , , Отказ);
	КонецЕсли;
	//}RGS AArsentev S-E-0000019 17.01.2017
