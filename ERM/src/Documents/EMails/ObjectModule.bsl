...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Recipients") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Copies") Тогда
			Для Каждого СтрокаRecipient Из ДанныеЗаполнения.Copies Цикл
				НоваяСтрокаТЧ = Copies.Добавить();
				НоваяСтрокаТЧ.Recipient = СтрокаRecipient.Recipient;
			КонецЦикла;
		КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ТипПисьма = Перечисления.EmailType.Incoming Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("You can't send an incoming email.", , , , Отказ);
		Возврат;
	КонецЕсли;
	...
	// { RGS TAlmazova 26.04.2019 17:45:24 - доработка для нотификации
	//ПользовательLDAP = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекПользователь, "ПользовательLDAP");
	Если НЕ ЗначениеЗаполнено(Sender) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("The sender is not specified in the e-mail.", ЭтотОбъект, , , Отказ);
	КонецЕсли;
	AutoUser = rgsНастройкаКонфигурации.ЗначениеНастройки("AutoUser");
	Если ТекПользователь = Sender ИЛИ ТекПользователь = AutoUser Тогда
		 ПользовательLDAP = Sender;
	Иначе
		 ОбщегоНазначенияКлиентСервер.СообщитьПользователю("You can not send a letter from another sender.", ЭтотОбъект, , , Отказ);
	КонецЕсли;
	// } RGS TAlmazova 26.04.2019 17:45:33 - доработка для нотификации
	...
	МассивСтруктурКопии = Новый Массив;
	Для Каждого СтрокаТЧ Из Copies Цикл
		//{RGS AArsentev 26.01.2017 S-E-0000048
		Если ТипЗнч(СтрокаТЧ.Recipient) <> Тип("СправочникСсылка.LDAPUsers") Тогда
			МассивСтруктурКопии.Добавить(Новый Структура("Адрес, Представление", СтрокаТЧ.Recipient, , СтрокаТЧ.Recipient));
		Иначе
		//}RGS AArsentev 26.01.2017 S-E-0000048
			СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаТЧ.Recipient, "Mail, DisplayName");
			МассивСтруктурКопии.Добавить(Новый Структура("Адрес, Представление", СтруктураРеквизитов.Mail, , СтруктураРеквизитов.DisplayName));
		КонецЕсли;
	КонецЦикла; 
	ПараметрыПисьма.Вставить("Копии", МассивСтруктурКопии);
	...
	Пока НЕ ПисьмоОтправлено И КоличествоПопыток < 3 Цикл
		...
		Попытка
			...
			//}RGS AArsentev S-E-0000019 17.01.2017
			ИдентификаторСообщения = ПараметрыПисьма.ИдентификаторСообщения;
