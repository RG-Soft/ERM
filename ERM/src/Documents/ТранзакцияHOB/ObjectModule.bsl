...
//		КонецЕсли;
		...
		НЗ_DIRИзГИС = РегистрыСведений.ДатыDIRИзГИС.СоздатьНаборЗаписей();
		НЗ_DIRИзГИС.Отбор.Company.Установить(Реквизиты.Company);
		НЗ_DIRИзГИС.Прочитать();
		...
		Если (Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.АктОбОказанииПроизводственныхУслуг ИЛИ
			...
			Если Реквизиты.TransactionType = Перечисления.HOBTransactionType.Accrual И ЗначениеЗаполнено(Реквизиты.SalesOrderID) И НЗ_DIRИзГИС.Количество() > 0 Тогда
