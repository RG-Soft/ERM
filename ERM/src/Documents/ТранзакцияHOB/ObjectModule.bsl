...
//		КонецЕсли;
		...
		Если Реквизиты.TransactionType = Перечисления.HOBTransactionType.Accrual И
			ЗначениеЗаполнено(Реквизиты.SalesOrderID) И
			...
			Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.ПередачаОС) Тогда
			
			ВыполнитьДействияПодсистемыDIR(Реквизиты, РежимИзмененияСвязанногоЗначения, Движения, СуммыДляПроводки.Amount, СуммыДляПроводки.BaseAmount, Отказ);
...
Процедура ВыполнитьДействияПодсистемыDIR(Реквизиты, РежимИзмененияСвязанногоЗначения, Движения, СуммыДляПроводки.Amount, СуммыДляПроводки.BaseAmount, Отказ)
	
	НЗ = РегистрыСведений.ДатыDIRИзГИС.СоздатьНаборЗаписей();
	НЗ.Отбор.Company.Установить(Реквизиты.Company);
	НЗ.Прочитать();
	Если НЗ.Количество() > 0 Тогда
		Документ = Реквизиты.SalesOrder;
		ЗаписатьДанныеDIR(Реквизиты, Документ);
	Иначе	
		Документ = Реквизиты.Invoice;
	КонецЕсли;
