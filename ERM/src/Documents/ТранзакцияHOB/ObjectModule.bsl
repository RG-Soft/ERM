...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ПометкаУдаления ИЛИ РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Тогда
		...
		НЗ = РегистрыСведений.ДатыDIRИзГИС.СоздатьНаборЗаписей();
		...
		Если ФормироватьДвижениеInvoicedDebtsПоSO(Реквизиты) Тогда
			Документ = ПараметрыПроведения.СвязанныеДокументы.SalesOrder;
		Иначе
			Документ = ПараметрыПроведения.СвязанныеДокументы.Invoice;
		КонецЕсли;
		РегистрыСведений.ДатыПервыхПроводокПоInvoicedDebts.ОчиститьЗаписьПоПроводке(Документ, Ссылка);
...
//		КонецЕсли;
		...
		Если Реквизиты.TransactionType = Перечисления.HOBTransactionType.Accrual И 
			(Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.АктОбОказанииПроизводственныхУслуг ИЛИ
			Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.РеализацияТоваровУслуг ИЛИ
			Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.РеализацияУслугПоПереработке ИЛИ
			Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.ПередачаОС)
			И ЗначениеЗаполнено(Реквизиты.SalesOrderID)
			И ФормироватьДвижениеInvoicedDebtsПоSO(Реквизиты)
			И НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Client, "Intercompany")
			И СуммыДляПроводки.Amount < 0 Тогда
			
				ВыполнитьДвижениеInvoicedDebts(Реквизиты, ПараметрыПроведения.СвязанныеДокументы.SalesOrder, Движения, -СуммыДляПроводки.Amount, -СуммыДляПроводки.BaseAmount, Отказ);
				
			//ИначеЕсли Реквизиты.TransactionType = Перечисления.HOBTransactionType.Receivables И ЗначениеЗаполнено(Реквизиты.InvoiceID) И НЗ_DIRИзГИС.Количество() = 0 Тогда
			//	
			//	ВыполнитьДвижениеInvoicedDebts(Реквизиты, ПараметрыПроведения.СвязанныеДокументы.Invoice, Движения, СуммыДляПроводки.Amount, СуммыДляПроводки.BaseAmount, Отказ);
			//	ЗаписатьДанныеDIR(Реквизиты, ПараметрыПроведения.СвязанныеДокументы.Invoice);
			//	
			//КонецЕсли;
	...
	Иначе
		...
		ИначеЕсли Реквизиты.TransactionType = Перечисления.HOBTransactionType.Receivables Тогда
			...
			Если (Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.АктОбОказанииПроизводственныхУслуг ИЛИ
				Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.РеализацияТоваровУслуг ИЛИ
				Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.РеализацияУслугПоПереработке ИЛИ
				Реквизиты.HOBDocumentType = Перечисления.HOBDocumentTypes.ПередачаОС)
				И ЗначениеЗаполнено(Реквизиты.InvoiceID)
				И НЕ ФормироватьДвижениеInvoicedDebtsПоSO(Реквизиты)
				И НЕ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Client, "Intercompany")
				И Реквизиты.Client <> Справочники.Контрагенты.NonTrade
				И Лев(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Реквизиты.Account, "Код"),3) = "120" 
				И СуммыДляПроводки.Amount > 0 Тогда
...
Процедура ВыполнитьДвижениеInvoicedDebts(Реквизиты, Документ, Движения, Сумма, СуммаUSD, Отказ)
	
	Если Отказ Тогда
		Возврат;
	КонецЕсли;

	ДатаПервойПроводки = РегистрыСведений.ДатыПервыхПроводокПоInvoicedDebts.ДатаПервойПроводкиПоДокументу(Документ);
	Если ДатаПервойПроводки = Неопределено Или Реквизиты.Дата < ДатаПервойПроводки Тогда
		...
		ЗаписьНабора.Invoice = Документ;
		ЗаписьНабора.ДатаПроводки = Реквизиты.Дата;
		ЗаписьНабора.Проводка = Ссылка;
		НЗ.Записать();
	ИначеЕсли НачалоМесяца(Реквизиты.Дата) > НачалоМесяца(ДатаПервойПроводки) Тогда
...
Процедура _ВыполнитьДействияПодсистемыDIR(Реквизиты, СвязанныеДокументы, Движения, Сумма, СуммаUSD, Отказ)
	
	НЗ = РегистрыСведений.ДатыDIRИзГИС.СоздатьНаборЗаписей();
	НЗ.Отбор.Company.Установить(Реквизиты.Company);
	НЗ.Прочитать();
	Если НЗ.Количество() > 0 Тогда
		Документ = СвязанныеДокументы.SalesOrder;
	Иначе
		Документ = СвязанныеДокументы.Invoice;
		ЗаписатьДанныеDIR(Реквизиты, Документ);
	КонецЕсли;
	
	ВыполнитьДвижениеInvoicedDebts(Реквизиты, Документ, Движения, Сумма, СуммаUSD, Отказ);
	
КонецПроцедуры

Функция ФормироватьДвижениеInvoicedDebtsПоSO(Реквизиты)
...
Процедура ЗаписатьДанныеDIR(Реквизиты, Документ)
	
	Даты = Новый Соответствие();
	Даты.Вставить("InvoiceFlagDate", Реквизиты.PostingDate);
	РегистрыСведений.DIR.ЗаписатьДаты(Документ, Даты);
