...
Функция CreditNotesCreditNoteПриИзмененииНаСервере(CreditNote)
	...
	Если ТипЗнч(CreditNote) = Тип("ДокументСсылка.Memo") Тогда
		...
		ЗначенияРеквизитов.Amount = ПолучитьСуммуМемо(CreditNote);
...
Функция ПолучитьСуммуМемо(CreditNote)
	...
	Запрос = Новый Запрос;
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			 Возврат ВыборкаДетальныеЗаписи.AmountОстаток;
		КонецЦикла;
...
Процедура ДатаПриИзмененииНаСервере()
	...
	Если Объект.ВидОперацииNoteAllocation = Перечисления.ВидыОперацийNoteAllocation.DebitMemo Тогда
		Для каждого СтрокаТЗ Из Объект.CreditNotes Цикл
			СтрокаТЗ.Amount = ПолучитьСуммуМемо(СтрокаТЗ.CreditNote);
		КонецЦикла;
