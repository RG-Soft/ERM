...
Функция ВыгрузитьДанныеДляPowerBI(Ссылка, Операция, ПараметрыЛогирования) Экспорт
	...
	ТекОбъект = PowerBIРегламенты.ПолучитьОбъектИзВнешнегоИсточника("dbo_JournalEntry", Операция, Ссылка, ПараметрыЛогирования);
	...
	Если Операция = Перечисления.ОперацииPowerBI.Create Тогда
		PowerBIРегламенты.СформироватьЗаписиЖурналаДокуменовPowerBI(Ссылка, "JV", ПараметрыЛогирования, СтруктураРезультата);
	КонецЕсли;
...
Процедура ЗаполнитьОбъектPowerBI(Объект, Ссылка)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	РучнаяКорректировка.ПометкаУдаления,
		|	РучнаяКорректировка.Номер,
		|	РучнаяКорректировка.Дата,
		|	ПРЕДСТАВЛЕНИЕ(РучнаяКорректировка.Source) КАК Source,
		|	РучнаяКорректировка.Company.БазовыйЭлемент КАК Company,
		|	РучнаяКорректировка.Client,
		|	РучнаяКорректировка.Currency,
		|	РучнаяКорректировка.Amount,
		|	РучнаяКорректировка.Agreement
		|ИЗ
		|	Документ.РучнаяКорректировка КАК РучнаяКорректировка
		|ГДЕ
		|	РучнаяКорректировка.Ссылка = &Ссылка";
