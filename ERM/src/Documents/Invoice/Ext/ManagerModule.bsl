...
Процедура ОбновитьСтатусОплатыДокументаПоПроводкам(Документ, Комментарий = "")
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ВЫРАЗИТЬ(ЕСТЬNULL(PaymentsОбороты.AmountОборот, 0) / ЕСТЬNULL(ВнутренниеКурсыВалютСрезПоследних.Курс, 1) * ЕСТЬNULL(ВнутренниеКурсыВалютСрезПоследних.Кратность, 1) КАК ЧИСЛО(15, 2))) КАК AmountОборот,
		|	МАКСИМУМ(ВЫБОР
	...
	ИначеЕсли Не ЕстьДвиженияПоИнвойсу И СуммаОплаты > 0 И ТекущийСтатус <> СтатусОплачен И ТекущийСтатус <> СтатусЧастичноОплачен Тогда
		...
		УстановитьСтатусИнвойса(Документ, СтатусЧастичноОплачен, ВыборкаСуммаОплаты.ДатаПоследнейОплаты, AutoUser, Комментарий);
	...
	ИначеЕсли ЕстьДвиженияПоИнвойсу И ТекущийОстаток <= 0 И СуммаОплаты <> 0 И ТекущийСтатус <> СтатусОплачен Тогда
		...
		УстановитьСтатусИнвойса(Документ, СтатусОплачен, ВыборкаСуммаОплаты.ДатаПоследнейОплаты, AutoUser, Комментарий);
...
Процедура ОбновитьСтатусОплатыДокументаПоРегиструОплат(Документ, Комментарий = "")
	...
	ИначеЕсли Не ЕстьДвиженияПоИнвойсу И СуммаОплаты > 0 И ТекущийСтатус <> СтатусОплачен И ТекущийСтатус <> СтатусЧастичноОплачен Тогда
		...
		УстановитьСтатусИнвойса(Документ, СтатусЧастичноОплачен, ВыборкаСуммаОплаты.PaymentDate, AutoUser, Комментарий);
	...
	ИначеЕсли ЕстьДвиженияПоИнвойсу И ТекущийОстаток <= 0 И СуммаОплаты <> 0 И ТекущийСтатус <> СтатусОплачен Тогда
		...
		// по инвойсу закрыт баланс и была оплата, значит считаем оплаченным
		УстановитьСтатусИнвойса(Документ, СтатусОплачен, ВыборкаСуммаОплаты.PaymentDate, AutoUser, Комментарий);
