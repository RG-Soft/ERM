...
Процедура ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища) Экспорт
	...
	//ТаблицаДанных.Колонки.Добавить("Amount", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15, 3)));
	//СтрокаФайла = 1;
	//
	//Пока rs.EOF() = 0 Цикл
...
Процедура ЗаписатьДанныеВТаблицу(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.УстановитьПараметр("ДатаКонец", КонецМесяца(СтруктураПараметров.Дата));
