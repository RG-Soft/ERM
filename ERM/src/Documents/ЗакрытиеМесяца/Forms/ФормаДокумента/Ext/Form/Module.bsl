...
Процедура ТаблицаДеревоВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтруктураОтбора.Вставить("КонецПериода", КонецМесяца(Объект.Дата));
	СтруктураОтбора.Вставить("НачалоПериода", НачалоМесяца(Объект.Дата));
	...
	ОткрытьФорму("Отчет.РасшифровкаСтрокиСверкиСHFM.Форма.ФормаОтчета", СтруктураПараметров);
