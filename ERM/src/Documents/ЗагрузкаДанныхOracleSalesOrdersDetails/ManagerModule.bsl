...
Процедура ОбновитьРеквизитыSalesOrders(СтруктураПараметров, АдресХранилища) Экспорт
	...
	|	OracleSalesOrdersDetailsSourceData.ShipmentDate КАК ShipmentDate,
	...
	|	Исходники.ShipmentDate КАК ShipmentDate,
	|	Исходники.InvoiceAmount КАК InvoiceAmount,
	|	Исходники.WellData КАК WellData
	...
	|			И Исходники.CompanyCode = ДокInvoice.Company.Код
	|			И (ДокInvoice.Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.OracleMI))
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИсходникиСсылки.CustomerNumber КАК CustomerNumber
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ИсходникиСсылки.Currency КАК Currency
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВалютаБезДублей.Currency КАК Currency,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КлиентыБезДублей.CustomerNumber КАК CustomerNumber,
	|	НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних.ОбъектПриемника КАК ОбъектПриемника
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ИсходникиСсылки.ДокументЗагрузки КАК ДокументЗагрузки,
	|	ИсходникиСсылки.НомерSO КАК НомерSO,
	|	ИсходникиСсылки.CustomerNumber КАК CustomerNumber,
	|	ИсходникиСсылки.Agreement КАК Agreement,
	|	ИсходникиСсылки.CompanyCode КАК CompanyCode,
	|	ИсходникиСсылки.CustomerRepresentative КАК CustomerRepresentative,
	|	ИсходникиСсылки.ApprovedBy КАК ApprovedBy,
	|	ИсходникиСсылки.CreatedBy КАК CreatedBy,
	|	ИсходникиСсылки.Currency КАК Currency,
	|	ИсходникиСсылки.FTLCreatedBy КАК FTLCreatedBy,
	|	МИНИМУМ(ИсходникиСсылки.ERPStatus) КАК ERPStatus,
	|	ИсходникиСсылки.JobEndDate КАК JobEndDate,
	|	ИсходникиСсылки.CreationDate КАК CreationDate,
	|	ИсходникиСсылки.FTLSubmissionDate КАК FTLSubmissionDate,
	|	ИсходникиСсылки.FirstSubmissionDate КАК FirstSubmissionDate,
	|	ИсходникиСсылки.ApprovalDate КАК ApprovalDate,
	|	ИсходникиСсылки.InvoiceFlagDate КАК InvoiceFlagDate,
	|	ИсходникиСсылки.FieldTicket КАК FieldTicket,
	|	ИсходникиСсылки.DOC_ID КАК DOC_ID,
	|	ИсходникиСсылки.СсылкаInvoice КАК СсылкаInvoice,
	|	ИсходникиСсылки.СсылкаSalesOrder КАК СсылкаSalesOrder,
	|	ИсходникиСсылки.СсылкаОрганизация КАК СсылкаОрганизация,
	|	ВалютыСсылка.ОбъектПриемника КАК СсылкаВалюта,
	|	КлиентыСсылка.ОбъектПриемника КАК СсылкаКлиент,
	|	ИсходникиСсылки.ShipmentDate КАК ShipmentDate,
	|	ИсходникиСсылки.InvoiceAmount КАК InvoiceAmount,
	|	ИсходникиСсылки.WellData КАК WellData
	|ИЗ
	|	ИсходникиСсылки КАК ИсходникиСсылки
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	SalesOrder.Ссылка КАК SAM_SO,
	|	SalesOrder.Основание КАК ST_SO,
	|	ИсходникиСсылки.WellData КАК WellData
	|ИЗ
	|	Документ.SalesOrder КАК SalesOrder
	|		ЛЕВОЕ СОЕДИНЕНИЕ ИсходникиСсылки КАК ИсходникиСсылки
	|		ПО SalesOrder.Основание = ИсходникиСсылки.СсылкаSalesOrder
	|ГДЕ
	|	SalesOrder.Основание В
	|			(ВЫБРАТЬ
	|				ИсходникиСсылки.СсылкаSalesOrder
	|			ИЗ
	|				ИсходникиСсылки КАК ИсходникиСсылки)";
	...
	SAM_ST_WellData = РезультатЗапроса[7].Выгрузить();
	...
	Пока Выборка.Следующий() Цикл
		...
		// { RGS TAlmazova 31.07.2018 15:55:03 - S-E-0000619 заполнение InvFlagDate в инвойсе регистре DIR
		//Если ЗначениеЗаполнено(Выборка.СсылкаInvoice) И Выборка.InvoiceFlagDate <> ПустаяДата Тогда
		//	Даты = Новый Соответствие();
		//	Даты.Вставить("InvoiceFlagDate", Выборка.InvoiceFlagDate);
		//	РегистрыСведений.DIR.ЗаписатьДаты(Выборка.СсылкаInvoice, Даты);
		//	СтрокаТЗ = ОбновленныеДатыDIR.Добавить();
		//	СтрокаТЗ.Invoice = Выборка.СсылкаInvoice;
		//КонецЕсли;
		// } RGS TAlmazova 31.07.2018 15:55:03 - 
		...
		//// { RGS TAlmazova 23.01.2017 15:00:33 - обновление комментария для СО Статус меняется из оборотов
	...
	Если SAM_ST_WellData.Количество() > 0 Тогда
		
		Для каждого СтрокаДанных Из SAM_ST_WellData Цикл
		
			SAM_Объект = СтрокаДанных.SAM_SO.ПолучитьОбъект();
			РГСофтКлиентСервер.УстановитьЗначение(SAM_Объект.WellData, СтрокаДанных.WellData);
			Если SAM_Объект.Модифицированность() Тогда
				SAM_Объект.ОбменДанными.Загрузка = Истина;
				SAM_Объект.Записать();
			КонецЕсли;
