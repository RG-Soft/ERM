...
Процедура ЗагрузитьИЗаписатьДвижения(СтруктураПараметров, ТаблицаДанных)
	...
	// { RGS AArsentev 07.02.2017 - чистим LE для организаций кроме 391 и 460
	Для каждого СтрокаДанных Из ТаблицаДанных Цикл
		Если СтрокаДанных.CompanyCode <> "460" И СтрокаДанных.CompanyCode <> "391" Тогда
			 СтрокаДанных.LEgalEntity = "";
		КонецЕсли;
	КонецЦикла;
	// } RGS AArsentev 07.02.2017 - чистим LE для организаций кроме 391 и 460
...
Процедура СформироватьПроводкиDSSAccruals(СтруктураПараметров, АдресХранилища)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ВыборкаДетальныеЗаписи, , ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках);
...
Процедура СформироватьПроводкиDSSReceivables(СтруктураПараметров, АдресХранилища)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ВыборкаДетальныеЗаписи, КэшПредставленийТипов, ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках);
...
Процедура ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ДанныеДляЗаполнения, КэшПредставленийТипов, ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках)
...
Процедура ДозаполнитьSalesOrderИзПроводки(SalesOrder, ТранзакцияHOBОбъект, ДанныеДляЗаполнения)
	...
	РеквизитыSalesOrder = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(SalesOrder, "Client,Company,AU,Account,Location,SubSubSegment,ClientID,CREW");
	...
	Если НЕ ЗначениеЗаполнено(РеквизитыSalesOrder.Client) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыSalesOrder.Company) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыSalesOrder.AU) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыSalesOrder.Account) ИЛИ
		...
		Для каждого Реквизит из РеквизитыSalesOrder Цикл
			Если Реквизит.Ключ <> "CREW" И Реквизит.Ключ <> "ClientID" Тогда
