...
Процедура СформироватьПроводкиDSSJV(СтруктураПараметров, АдресХранилища)
	...
	// { RGS TAlmazova 25.07.2017 17:28:41 - Пометка на удаление корректировок, если данные в транзакции поменялись
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|	ТаблицаСуществующихТранзакций.TrID,
		...
		|			ИЛИ ВТ_ДанныеТранзакцийДоЗагрузки.TrID <> DSSСформированныеПриЗагрузке.ПроводкаDSS.TrID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ТранзакцияДоЗагрузкиClient = Справочники.Контрагенты.GLNoName И ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.СсылкаКорректировка) Тогда
		...
		Если ВыборкаДетальныеЗаписи.ТранзакцияПослеЗагрузкиClient = Справочники.Контрагенты.GLNoName Тогда
