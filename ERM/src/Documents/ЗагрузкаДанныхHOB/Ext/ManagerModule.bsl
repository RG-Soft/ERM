	...
	Если ТранзакцияHOBОбъект.TransactionType = Перечисления.HOBTransactionType.Accrual Тогда
		...
		Если Не ПустаяСтрока(ДанныеДляЗаполнения.InvoiceID) Тогда
			...
			Иначе
				...
				ДозаполнитьИнвойсИзПроводки(ТекИнвойс, ТранзакцияHOBОбъект, Отказ, ТекстСообщенияОбОшибках);
...
Процедура ДозаполнитьИнвойсИзПроводки(Инвойс, ТранзакцияHOBОбъект, Отказ, ТекстСообщенияОбОшибках)
	...
	Если НЕ Отказ Тогда
		...
		Для каждого Реквизит из РеквизитыИнвойса Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если НЕ ЗначениеЗаполнено(Инвойс["DocID"]) Тогда
