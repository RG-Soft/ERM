...
Процедура СформироватьПроводкиDSSAccruals(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|		ПО КонтрольныеСуммыПриЗагрузке.ИдентификаторСистемы = ДанныеТранзакции.ПроводкаDSSAU
...
Процедура СформироватьПроводкиDSSJV(СтруктураПараметров, АдресХранилища)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ВыборкаДетальныеЗаписи, , ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует);
		...
		Если КлиентОтсутствует Тогда
			...
			Задача.РольИсполнителя = Справочники.РолиИсполнителей.ОтветственныйHOB;
...
Процедура ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ДанныеДляЗаполнения, КэшПредставленийТипов, ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует = Ложь)
