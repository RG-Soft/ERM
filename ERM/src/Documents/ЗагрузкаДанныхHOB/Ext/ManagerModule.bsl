...
Процедура ВыполнитьПроверкуНастроекСинхронизации(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВТ_HOBSourceData.LocationCode
		|ИЗ
		|	ВТ_HOBSourceData КАК ВТ_HOBSourceData
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПодразделенияОрганизаций КАК ПодразделенияОрганизаций
		|		ПО (НЕ ПодразделенияОрганизаций.ПометкаУдаления)
		|			И ВТ_HOBSourceData.LocationCode = ПодразделенияОрганизаций.Код
...
Функция СформироватьСвязанныеОбъекты(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшРучныхКоррерктировок, КэшИнвойсов, КэшCashBatch, КэшBatchAllocation)
	...
	ИначеЕсли ТранзакцияOracleОбъект.DocType = "DEP" ИЛИ ТранзакцияOracleОбъект.DocType = "GUAR"
		...
		ТекИнвойс = ПолучитьИнвойс(ТранзакцияOracleОбъект.DocID);
	...
	ИначеЕсли ТранзакцияOracleОбъект.DocType = "CASH" Тогда
		...
		//КонецЕсли;
		// TODO RGS AGorlenko 21.07.2016: переделать поиск
		ТекCashBatch = ПолучитьCashBatch(ТранзакцияOracleОбъект.DocID);
		...
		Если ТранзакцияOracleОбъект.TransType = "CCURR_REC" ИЛИ ТранзакцияOracleОбъект.TransType = "TRADE_REC" Тогда
			...
			ДокументРасчетов = ПолучитьИнвойс(ТранзакцияOracleОбъект.LineID);
...
Функция ПолучитьCashBatch(DocID)
