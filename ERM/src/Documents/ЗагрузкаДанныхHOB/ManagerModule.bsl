...
Процедура ПолучитьСтруктуруКолонокТаблицыДанныхJV(СтруктураКолонок)
	...
	// JobPeriod
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "JobPeriod";
	СтрокаТЗ.ИмяКолонки = "JobPeriod";
	СтрокаТЗ.Обязательная = Истина;
	
	// JVTransactionNumber
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "JVTransactionNumber";
	СтрокаТЗ.ИмяКолонки = "JVTransactionNumber";
	СтрокаТЗ.Обязательная = Истина;
...
Процедура ВыполнитьПроверкуНастроекСинхронизацииAccruals(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ВТ_HOBAccrualsSourceData.AUType <> ""Lawson""
		|	И КостЦентры.ПодразделениеОрганизации = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ВыполнитьПроверкуНастроекСинхронизацииJV(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|				ИЛИ HOBJVSourceData.Document = ""Операция международная МПГ00000283 от 01.03.2020 00:00:00""
...
Процедура СформироватьПроводкиDSSAccruals(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|	Lawson.Ссылка,
		//|	ПодразделенияОрганизаций.Ссылка,
		|	ЕСТЬNULL(КостЦентры.ПодразделениеОрганизации, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)),
		//|	Сегменты.Ссылка,
		|	ЕСТЬNULL(КостЦентры.Сегмент, ЗНАЧЕНИЕ(Справочник.Сегменты.ПустаяСсылка)),
		...
		|	КостЦентры.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.Родитель,
...
Процедура СформироватьПроводкиDSSJV(СтруктураПараметров, АдресХранилища)
	...
	// { RGS AGorlenko 15.04.2020 11:11:59 - S-E-0001313- Для перехода на новый алгоритм загрузки JV HOBs: не трогаем счета Bad Debts
	//СписокСчетов = Новый СписокЗначений;
	//СписокСчетов.Добавить(ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12001"));
	//СписокСчетов.Добавить(ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12002"));
	// } RGS AGorlenko 15.04.2020 11:15:12 - S-E-0001313- Для перехода на новый алгоритм загрузки JV HOBs: не трогаем счета Bad Debts
	...
	//	|	И НЕ ТранзакцияHOB.ПометкаУдаления
	//	|	И ТранзакцияHOB.TransactionType = ЗНАЧЕНИЕ(Перечисление.HOBTransactionType.JV)
	//	|	И ТранзакцияHOB.Account.БазовыйЭлемент В (&СписокСчетов)";
	...
	Запрос.Текст = 
		...
		|	HOBJVSourceData.JVTransactionNumber КАК JVTransactionNumber,
		|	HOBJVSourceData.JobPeriod КАК JobPeriod
		|ПОМЕСТИТЬ ВТ_ДанныеФайла
		|ИЗ
		|	РегистрСведений.HOBJVSourceData КАК HOBJVSourceData
		|ГДЕ
		...
		|	ВТ_ДанныеФайла.EndClientINN КАК EndCustomerNumber,
		|	ВТ_ДанныеФайла.PostingDate КАК PostingDate,
		|	ВТ_ДанныеФайла.JVTransactionNumber КАК JVTransactionNumber,
		|	ВТ_ДанныеФайла.JobPeriod КАК JobPeriod
		|ИЗ
		|	ВТ_ДанныеФайла КАК ВТ_ДанныеФайла
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
		|		ПО (НЕ Организации.ПометкаУдаления)
		...
		|	ВТ_ДанныеФайла.EndClientINN,
		|	ВТ_ДанныеФайла.PostingDate,
		|	ВТ_ДанныеФайла.JVTransactionNumber,
		|	ВТ_ДанныеФайла.JobPeriod
		|ИЗ
		|	ВТ_ДанныеФайла КАК ВТ_ДанныеФайла
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
		|		ПО (НЕ Организации.ПометкаУдаления)
	...
	КэшДетальныхРучныхКорректировок = РезультатЗапроса[3].Выгрузить();
	КэшДетальныхРучныхКорректировок.Индексы.Добавить("Source, Company, Journal, LineNbr, Date");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.DocumentPresentation = "Операция международная МПГ00000150 от 01.02.2020 00:00:00" 
				ИЛИ ВыборкаДетальныеЗаписи.DocumentPresentation = "Операция международная МПГ00000283 от 01.03.2020 00:00:00"
		...
		// { RGS AGorlenko1 13.04.2020 13:17:39 - S-E-0001313-Доработка для перехода на новый алгоритм загрузки по JV
		//Если НачалоМесяца(ВыборкаДетальныеЗаписи.Period) = Дата("20200301000000") И НачалоМесяца(ВыборкаДетальныеЗаписи.JobPeriod) <> Дата("20200301000000") Тогда
		//	
		//	Продолжить;
		//	
		//КонецЕсли;
		// } RGS AGorlenko1 13.04.2020 13:19:45 - S-E-0001313-Доработка для перехода на новый алгоритм загрузки по JV
		...
		НайденаОшибкаПриПоискеСвязанныхОбъектов = СформироватьСвязанныеОбъекты(ТранзакцияHOBОбъект, ВыборкаДетальныеЗаписи, , , КэшДетальныхРучныхКорректировок, Неопределено, Неопределено, Неопределено, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, Отказ, ТекстСообщенияОбОшибках);
...
Процедура ЗаполнитьТранзакцию(СтруктураПараметров, ТранзакцияHOBОбъект, ДанныеДляЗаполнения, КэшПредставленийТипов, ТаблицаИсключенийАналитики, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует = Ложь, КэшAU)
	...
	Если ТранзакцияHOBОбъект.TransactionType = Перечисления.HOBTransactionType.JV Тогда
		...
		ТранзакцияHOBОбъект.JVTransactionNumber = ДанныеДляЗаполнения.JVTransactionNumber;
...
Функция СформироватьСвязанныеОбъекты(ТранзакцияHOBОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшИнвойсов, КэшДетальныхРучныхКорректировок, КэшCashBatch, КэшBatchAllocation, КэшMemo, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, Отказ, ТекстСообщенияОбОшибках)
	...
	СтруктураПоискаДетальнойРучнойКорректировки = Новый Структура("Source, Company, Journal, LineNbr, Date");
	...
	ИначеЕсли ТранзакцияHOBОбъект.TransactionType = Перечисления.HOBTransactionType.JV Тогда
		...
		СтруктураПоискаДетальнойРучнойКорректировки.Source = Перечисления.ТипыСоответствий.HOBs;
		СтруктураПоискаДетальнойРучнойКорректировки.Company = ТранзакцияHOBОбъект.Company;
		СтруктураПоискаДетальнойРучнойКорректировки.Journal = ТранзакцияHOBОбъект.DocumentID;
		СтруктураПоискаДетальнойРучнойКорректировки.LineNbr = ТранзакцияHOBОбъект.JVTransactionNumber;
		//СтруктураПоискаДетальнойРучнойКорректировки.Date = ?(ЗначениеЗаполнено(ДанныеДляЗаполнения.JobPeriod), ДанныеДляЗаполнения.JobPeriod, ТранзакцияHOBОбъект.Дата);
		
		СтрокиДетальнойРучнойКорректировки = КэшДетальныхРучныхКорректировок.НайтиСтроки(СтруктураПоискаДетальнойРучнойКорректировки);
		
		Если СтрокиДетальнойРучнойКорректировки.Количество() = 0 Тогда
			
			ТекРучнаяКорректировка = СоздатьРучнуюКорректировку(ТранзакцияHOBОбъект, ДанныеДляЗаполнения);
			НоваяСтрокаКэша = КэшДетальныхРучныхКорректировок.Добавить();
			//НоваяСтрокаКэша.Source = Перечисления.ТипыСоответствий.HOBs;
			//НоваяСтрокаКэша.Company = ТранзакцияHOBОбъект.Company;
			//НоваяСтрокаКэша.Journal = ТранзакцияHOBОбъект.DocumentID;
			//НоваяСтрокаКэша.LineNbr = ТранзакцияHOBОбъект.НомерПроводки;
			//НоваяСтрокаКэша.Date = ДанныеДляЗаполнения.ПериодРабот;
			ЗаполнитьЗначенияСвойств(НоваяСтрокаКэша, СтруктураПоискаДетальнойРучнойКорректировки);
			НоваяСтрокаКэша.Date = ?(ЗначениеЗаполнено(ДанныеДляЗаполнения.JobPeriod), ДанныеДляЗаполнения.JobPeriod, ТранзакцияHOBОбъект.Дата);
			НоваяСтрокаКэша.РучнаяКорректировка = ТекРучнаяКорректировка;
			
		Иначе
			
			ДобавитьСвязанныйОбъект(ТранзакцияHOBОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.РучнаяКорректировка, СтрокиДетальнойРучнойКорректировки[0].РучнаяКорректировка);
...
Процедура ДозаполнитьИнвойсИзПроводки(Инвойс, ТранзакцияHOBОбъект, ДанныеДляЗаполнения,ЭтоПроводкаПоСчетуВыручки)
	...
	РеквизитыИнвойса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Инвойс, "Company,DocID,Client,Location,SubSubSegment,AU,Account,Currency,LegalEntity,ClientID,Amount");
	
	Если ЭтоПроводкаПоСчетуВыручки И НЕ ЗначениеЗаполнено(РеквизитыИнвойса.ClientID) И НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Client) Тогда
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Company) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.DocID) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Client) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Location) ИЛИ
			...
			ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.LegalEntity) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.ClientID) ИЛИ РеквизитыИнвойса.Amount <> ДанныеДляЗаполнения.InvoiceAmount Тогда
			ИнвойсОбъект = Инвойс.ПолучитьОбъект();
...
Функция СоздатьРучнуюКорректировку(ТранзакцияHOBОбъект, ДанныеДляЗаполнения)
	...
	РучнаяКорректировкаОбъект.Дата = ?(ЗначениеЗаполнено(ДанныеДляЗаполнения.JobPeriod), ДанныеДляЗаполнения.JobPeriod, ТранзакцияHOBОбъект.Дата);
	РучнаяКорректировкаОбъект.JournalLawson = ТранзакцияHOBОбъект.DocumentID;
	РучнаяКорректировкаОбъект.LineNbrLawson = ТранзакцияHOBОбъект.JVTransactionNumber;
