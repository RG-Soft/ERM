...
Процедура СоздатьНовыеОбъекты(Отказ)
	...
	НЗ_LawsonClients.ДополнительныеСвойства.Вставить("РазрешитьСозданиеЗаписи", Истина);
	...
	НЗ_MIClients.ДополнительныеСвойства.Вставить("РазрешитьСозданиеЗаписи", Истина);
	...
	НЗ_SmithClients.ДополнительныеСвойства.Вставить("РазрешитьСозданиеЗаписи", Истина);
...
Процедура ОбработатьИзменныеРеквизиты(Отказ)
	...
	// { RGS TAlmazova 16.07.2018 15:08:31 - S-E-0000859
	...
	ДанныеЗаполенияКИ.Колонки.Добавить("Объект");
	ДанныеЗаполенияКИ.Колонки.Добавить("Вид");
	ДанныеЗаполенияКИ.Колонки.Добавить("Тип");
	ДанныеЗаполенияКИ.Колонки.Добавить("Значение");
	ДанныеЗаполенияКИ.Колонки.Добавить("ЗначенияПолей");
	ДанныеЗаполенияКИ.Колонки.Добавить("Представление");
	ДанныеЗаполенияКИ.Колонки.Добавить("Дата");
	// } RGS TAlmazova 16.07.2018 15:08:33 - S-E-0000859
	...
	Для каждого СтрокаТЧ Из ИзмененныеРеквизиты Цикл
		...
		// { RGS TAlmazova 16.07.2018 15:09:00 - S-E-0000859
		...
		СтрокаКИ.Объект = СтрокаТЧ.Client;
		СтрокаКИ.Вид = Справочники.ВидыКонтактнойИнформации.CRMАдресКонтрагента;
		СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес;
		//СтрокаКИ.ЗначенияПолей = УправлениеКонтактнойИнформациейСлужебный.ПривестиКонтактнуюИнформациюXML(Новый Структура("Страна, Индекс, Город, Улица", СтрокаТЧ.NewCountry, СтрокаТЧ.NewPostalCode, СтрокаТЧ.NewCity, СтрокаТЧ.NewStreetAddress));
		СтрокаКИ.Представление = СтрШаблон("%1, %2, %3", ?(ЗначениеЗаполнено(СтрокаТЧ.NewPostalCode), СтрокаТЧ.NewPostalCode, СтрокаТЧ.NewCountry) , СтрокаТЧ.NewCity, СтрокаТЧ.NewStreetAddress);
		СтрокаКИ.Дата = ТекущаяДата();
		// } RGS TAlmazova 16.07.2018 15:09:00 - S-E-0000859
		
	КонецЦикла;
	
	// { RGS TAlmazova 16.07.2018 15:09:57 - S-E-0000859
	//УправлениеКонтактнойИнформацией.ЗаполнитьКонтактнуюИнформациюОбъектов(ДанныеЗаполенияКИ);
	УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъектов(ДанныеЗаполенияКИ);
	// } RGS TAlmazova 16.07.2018 15:09:58 - S-E-0000859
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не РольДоступна("Администрирование") И Проведен Тогда
		Отказ = Истина;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ДополнительныеСвойства.Свойство("АдресФайлаВХранилище") Тогда
		...
		Отказ = РГСофт.СоздатьПрисоединенныйФайлЗагрузки(ИмяФайла, Ссылка, АдресФайлаВХранилище, "Clients");
