...
Процедура УдалитьШапкуФайла(ПутьКФайлу, СтруктураПараметров)
	...
	// в качестве ориентиров будем использовать колонки WellData, OrderNumber
	НомерСтрокиЗаголовка = 1;
	
	Для ТекНомерСтроки = 1 По 100 Цикл
		
		НайденаOrderNumber = Ложь;
		НайденаWellData = Ложь;
		
		Для ТекНомерСтолбца = 1 По 100 Цикл
			
			Если Лист.Cells(ТекНомерСтроки, ТекНомерСтолбца).Value = "invoice_nbr" Тогда
				НайденаOrderNumber = Истина;
			КонецЕсли;
			Если Лист.Cells(ТекНомерСтроки, ТекНомерСтолбца).Value = "customer_po_nbr" Тогда
				НайденаWellData = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
		Если НайденаOrderNumber И НайденаWellData Тогда
