...
Процедура ТаблицаКоллизийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.ТаблицаКоллизий.ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Элементы.ТаблицаКоллизий.ТекущиеДанные.ТипОбъектаВнешнейСистемы = ПредопределенноеЗначение("Перечисление.ТипыОбъектовВнешнихСистем.Client") Тогда
...
Процедура ЗаполнитьПоДаннымФайлаНаСервере()
	...
	Запрос.Текст = 
		...
		|	ВТ_СоответствиеParentClient.ОбъектПриемника КАК ParentClient,
		|	ВТ_СоответствиеКлиентов.ОбъектПриемника КАК Client,
