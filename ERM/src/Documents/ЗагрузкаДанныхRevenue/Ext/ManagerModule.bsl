...
Процедура ВыполнитьПроверкуНастроекСинхронизации(СтруктураПараметров, АдресХранилища) Экспорт
	...
	// { RGS PMatkov 16.12.2015 16:11:30 - 
	Если СтруктураПараметров.ТипВнешнейСистемы = Перечисления.ТипыСоответствий.OracleMI Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, ПолучитьИзменениеТекстаЗапроса(Перечисления.ТипыСоответствий.OracleMI),"");
	КонецЕсли;
	// } RGS PMatkov 16.12.2015 16:11:30 - 
...
// { RGS PMatkov 16.12.2015 16:19:16 - 
Функция ПолучитьИзменениеТекстаЗапроса(ИспользуемыйТипыСоответствий)
	Если ИспользуемыйТипыСоответствий = Перечисления.ТипыСоответствий.OracleMI Тогда
	Текст = "
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
	...
	Возврат Текст;
КонецФункции
// } RGS PMatkov 16.12.2015 16:19:17 - 
