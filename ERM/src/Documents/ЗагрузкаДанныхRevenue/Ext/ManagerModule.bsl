...
Процедура ВыполнитьПроверкуНастроекСинхронизации(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.Lawson) КАК ТипСоответствия,
		...
		|			И (RevenueSourceData.Регистратор = &Регистратор)
		|ГДЕ
		...
		|		ПО (RevenueSourceData.Регистратор = &Регистратор)
