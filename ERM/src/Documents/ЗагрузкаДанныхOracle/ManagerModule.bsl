...
Процедура ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища) Экспорт
	...
	//	
		...
		Если СтруктураПараметров.ИмяРегистра = "OracleSourceData" Тогда
			УдалитьШапкуФайлаMI(ПутьКФайлу, СтруктураПараметров);
		Иначе
			УдалитьШапкуФайлаSmith(ПутьКФайлу, СтруктураПараметров);
		КонецЕсли;
...
Процедура УдалитьШапкуФайлаMI(ПутьКФайлу, СтруктураПараметров)
...
Процедура УдалитьШапкуФайлаSmith(ПутьКФайлу, СтруктураПараметров)
	...
	Для ТекНомерСтроки = 1 По 100 Цикл
		...
		НайденаSOURCE = Ложь;
		НайденаCATEGORY = Ложь;
		НайденаBATCH_NAME = Ложь;
		
		Для ТекНомерСтолбца = 1 По 100 Цикл
			
			Если Лист.Cells(ТекНомерСтроки, ТекНомерСтолбца).Value = "SOURCE" Тогда
				НайденаSOURCE = Истина;
			КонецЕсли;
			Если Лист.Cells(ТекНомерСтроки, ТекНомерСтолбца).Value = "CATEGORY" Тогда
				НайденаCATEGORY = Истина;
			КонецЕсли;
			Если Лист.Cells(ТекНомерСтроки, ТекНомерСтолбца).Value = "BATCH_NAME" Тогда
				НайденаBATCH_NAME = Истина;
			КонецЕсли;
			
		КонецЦикла;
		
		Если НайденаSOURCE И НайденаCATEGORY И НайденаBATCH_NAME Тогда
			НомерСтрокиЗаголовка = ТекНомерСтроки;
			Прервать;
		КонецЕсли;
	...
	ПоследняяСтрока = Лист.Cells(1,1).SpecialCells(11).Row;
	Если Лист.Cells(ПоследняяСтрока, 1).Value = "Total" Тогда
		Лист.Rows(ПоследняяСтрока).Delete();
	КонецЕсли;
...
Функция ПолучитьСтруктуруКолонокТаблицыДанныхSmith(СтруктураКолонок)
	...
	СтрокаТЗ.ИмяКолонки = "CostCenter";
	СтрокаТЗ.Обязательная = Истина;
...
Процедура ЗагрузитьИЗаписатьДвижения(Ссылка, ДатаДокумента, ТаблицаДанных, ИмяРегистра)
	...
	Если ИмяРегистра = "OracleSmithSourceData" Тогда
		Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			...
			ИначеЕсли СтрокаТаблицы.company_code = "E030" Тогда
				СтрокаТаблицы.company_code = "30";
			ИначеЕсли СтрокаТаблицы.company_code = "E260" Тогда
				СтрокаТаблицы.company_code = "260";
...
Процедура ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Source, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует, КэшAU)
	...
	Если НЕ ЗначениеЗаполнено(ТранзакцияOracleОбъект.Client) Тогда
		...
		// } RGS TAlmazova 03.10.2016 14:44:47 - поиск по OracleGlSourceType
			...
			ТранзакцияOracleОбъект.Client = Справочники.Контрагенты.Unreconciled;
