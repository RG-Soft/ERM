...
// не используется // { RGS AIvanov 05.07.2019 
...
// не используется // { RGS AIvanov 05.07.2019 
Процедура _ЗаполнитьДатыDIR(ДокументЗагрузки)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	DSSСвязанныеДокументы.СвязанныйОбъект КАК Invoice,
	|	DSSСформированныеПриЗагрузке.ПроводкаDSS.CreationDate КАК CreationDate,
	|	DSSСформированныеПриЗагрузке.ПроводкаDSS.ShipDateActual КАК ShipDateActual
	|ИЗ
	...
	|	И ПОДСТРОКА(DSSСформированныеПриЗагрузке.ПроводкаDSS.Account.Код, 0, 4) = ""1101""
	|	И DSSСформированныеПриЗагрузке.ПроводкаDSS.TransType = ""INV_REC""
	|	И DSSСформированныеПриЗагрузке.ПроводкаDSS.DocType = ""INV""
	|	И DSSСформированныеПриЗагрузке.ПроводкаDSS.GlSourceType = ЗНАЧЕНИЕ(Перечисление.OracleGlSourceType.Receivables)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Даты.Вставить("JobEndDate", Выборка.ShipDateActual);
		Даты.Вставить("CreationDate", Выборка.CreationDate);
...
///////////////////////////////////////////////////////////////////////////////
// Структура колонок таблицы данных (MI, Smith)
///////////////////////////////////////////////////////////////////////////////
#Область Структура_колонок
...
Функция ПолучитьСтруктуруКолонокТаблицыДанныхMI(СтруктураКолонок)
	...
	//SAM_STATUS
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "SAM_STATUS";
	СтрокаТЗ.ИмяКолонки = "SAM_STATUS";
	СтрокаТЗ.Обязательная = Истина;
	
	//ACCRUAL_ORG_ID
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "ACCRUAL_ORG_ID";
	СтрокаТЗ.ИмяКолонки = "ACCRUAL_ORG_ID";
	СтрокаТЗ.Обязательная = Истина;
	
	//ANALYTICS_3
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "ANALYTICS_3";
	СтрокаТЗ.ИмяКолонки = "ANALYTICS_3";
	СтрокаТЗ.Обязательная = Истина;
...
Функция ПолучитьСтруктуруКолонокТаблицыДанныхSmith(СтруктураКолонок)
	...
	// { RGS AGorlenko 06.05.2020 15:22:53 - Добавление колонок для загрузки JV
	СтрокаТЗ = СтруктураКолонок.Добавить();
	СтрокаТЗ.ИмяПоля = "RECEIPT_DATE";
	СтрокаТЗ.ИмяКолонки = "RECEIPT_DATE";
	СтрокаТЗ.Обязательная = Истина;
	...
	СтрокаТЗ.ИмяКолонки = "ACCRUAL_ORG_ID";
	СтрокаТЗ.Обязательная = Истина;
	// } RGS AGorlenko 06.05.2020 15:23:18 - Добавление колонок для загрузки JV
	
КонецФункции
#КонецОбласти

///////////////////////////////////////////////////////////////////////////////
// Заполнение регистров промежуточных данных (Load_file)
///////////////////////////////////////////////////////////////////////////////
#Область Load_file
Процедура ЗагрузитьДанныеИзФайла(СтруктураПараметров, АдресХранилища) Экспорт
	...
	//Если Истина Тогда
		...
		ФайлЭксель = РаботаСФайлами.ДвоичныеДанныеФайла(СтруктураПараметров.ИсточникДанных);
...
Процедура ЗагрузитьИЗаписатьДвижения(Ссылка, ДатаДокумента, ТаблицаДанных, ИмяРегистра)
	...
	Если ИмяРегистра = "OracleSmithSourceData" Тогда
		Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			...
			ИначеЕсли СтрокаТаблицы.company_code = "E040" Тогда
				СтрокаТаблицы.company_code = "40";
	...
	Иначе
		Для каждого СтрокаТаблицы Из ТаблицаДанных Цикл
			Если СтрокаТаблицы.OU = "99W0" Тогда
				СтрокаТаблицы.OU = "9900";
			КонецЕсли;
			СтрокаТаблицы.OU = Число(СтрокаТаблицы.OU);
		КонецЦикла;
	КонецЕсли;
...
///////////////////////////////////////////////////////////////////////////////
// Настройки_синхронизации (Check conflicts)
///////////////////////////////////////////////////////////////////////////////
#Область Check_conflicts
...
Функция ПолучитьТекстЗапросаДляПроверкиНастроекСинхронизацииSmith()
	...
	ТекстЗапроса = 
		...
		|			ИЛИ OracleSmithSourceData.hfm_account ПОДОБНО ""122%""
		|			ИЛИ OracleSmithSourceData.hfm_account ПОДОБНО ""209%""
...
///////////////////////////////////////////////////////////////////////////////
// Создание OracleTransactions (Fill in according to file)
///////////////////////////////////////////////////////////////////////////////
#Область Fill_in_according_to_file
...
Процедура СформироватьПроводкиDSSMI(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|	OracleSourceData.SAM_STATUS КАК SAM_STATUS,
		...
		|	OracleSourceData.ACCRUAL_ORG_ID КАК ACCRUAL_ORG_ID,
		|	OracleSourceData.ANALYTICS_3 КАК ANALYTICS_3
		|ПОМЕСТИТЬ ВТ_ДанныеФайла
		|ИЗ
		|	РегистрСведений.OracleSourceData КАК OracleSourceData
		|ГДЕ
		|	OracleSourceData.ДокументЗагрузки = &ДокументЗагрузки
		|	И (ПОДСТРОКА(OracleSourceData.GL_ACCOUNT, 17, 1) = ""4""
		|				ИЛИ OracleSourceData.ENTERED <> 0)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	НомерТранзакции
		|;
		...
		|	КлючиДетальныхРучныхКорректировок.Date КАК Date,
		...
		|ГДЕ
		|	КлючиДетальныхРучныхКорректировок.Source = &ТипВнешнейСистемы
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КлючиBatchAllocation.Source КАК Source,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_ДанныеФайла.GL_DATE КАК gl_date,
		...
		|	ВТ_ДанныеФайла.END_CUSTOMER_NUMBER КАК EndCustomerNumber,
		|	ВТ_ДанныеФайла.SAM_STATUS КАК SAM_STATUS,
		...
		|	ВТ_ДанныеФайла.ACCRUAL_ORG_ID КАК AccrualOrgID,
		|	ВТ_ДанныеФайла.ANALYTICS_3 КАК Analytics3
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ИтоговаяБезКорректировок.gl_date КАК gl_date,
		...
		|	ЕСТЬNULL(ИтоговаяБезКорректировок.EndClient, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК EndClient,
		...
		|	ИтоговаяБезКорректировок.EndCustomerNumber КАК EndCustomerNumber,
		|	ИтоговаяБезКорректировок.SAM_STATUS КАК SAM_STATUS,
		|	ПРЕДСТАВЛЕНИЕ(ИтоговаяБезКорректировок.DocTransID) КАК DocTransID,
		...
		|	ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.OracleMI) КАК Source,
		|	ИтоговаяБезКорректировок.AccrualOrgID КАК AccrualOrgID,
		|	ИтоговаяБезКорректировок.Analytics3 КАК Analytics3
		|ИЗ
	...
	КэшДетальныхРучныхКорректировок = РезультатЗапроса[5].Выгрузить();
	КэшДетальныхРучныхКорректировок.Индексы.Добавить("Source, Company, Journal");
	...
	СписокКолонокВыборки = РезультатЗапроса[10].Колонки;
	СписокКолонокДляЗаполнения = ПолучитьСписокКолонокДляЗаполнения(СписокКолонокВыборки
		, Метаданные.НайтиПоТипу(Тип("ДокументОбъект.ТранзакцияOracle")));
	...
	// { RGS AGorlenko1 19.04.2020 13:14:09 - Временная доработка для перехода на новый алгоритм загрузки JV
	//Счет12001 = ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12001");
	//Счет12002 = ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12002");
	//НезагружаемыеКомпании = Новый Структура("Компания8180, Компания9850", Справочники.Организации.НайтиПоКоду("8180"), Справочники.Организации.НайтиПоКоду("9850"));
	//Компания9900 = Справочники.Организации.НайтиПоКоду("9900");
	// } RGS AGorlenko1 19.04.2020 13:14:37 - Временная доработка для перехода на новый алгоритм загрузки JV
	
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		
		КоличествоТранзакцийВФайле = КоличествоТранзакцийВФайле + 1;
		
		// { RGS TAlmazova 24.09.2019 11:05:53 - S-E-0001365 - игнорируем строки по проводкам старше 1 сент 2019 года, так как в них неверно были заполнены поля EndClient.
		// колонки код конечного клиента и наименование конечного клиента были перепутаны в файле загрузки. Файл поправили с сентября. Если подгрузить данные по проводкам 
		// до сентября, то ручные корректировки транзакций отменятся, чего допустить нельзя. Поэтому добавляем игнорирование проводок до сентября 2019 года.
		Если ВыборкаДетальныеЗаписи.gl_date < Дата("20190901000000") Тогда
			Продолжить;
		КонецЕсли;
		...
		//ВРЕМЕННАЯ ДОРАБОТКА ДЛЯ ПЕРЕХОДА НА НОВЫЙ АЛГОРИТМ ЗАГРУЗКИ JV
		//БазовыйСчет = ВыборкаДетальныеЗаписи.Account.БазовыйЭлемент;
		//
		//Если ВыборкаДетальныеЗаписи.GlSourceType = "Sales Accrual" ИЛИ 
		//		ВыборкаДетальныеЗаписи.GlSourceType = "SAM Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "Receivables"
		//		ИЛИ (БазовыйСчет <> Счет12001 И БазовыйСчет <> Счет12002) Тогда
		//		
		//		Продолжить;
		//		
		//КонецЕсли;
		//
		//Если (НачалоМесяца(ВыборкаДетальныеЗаписи.gl_date) >= Дата("20200401000000")
		//		ИЛИ (НачалоМесяца(ВыборкаДетальныеЗаписи.gl_date) = Дата("20200301000000") И ВыборкаДетальныеЗаписи.TransType = "R"
		//				И НЕ ВыборкаДетальныеЗаписи.Company = НезагружаемыеКомпании.Компания8180
		//				И НЕ (ВыборкаДетальныеЗаписи.Company = НезагружаемыеКомпании.Компания9850 И БазовыйСчет = Счет12001)))
		//	И НЕ (ВыборкаДетальныеЗаписи.GlSourceType = "Sales Accrual" ИЛИ 
		//		ВыборкаДетальныеЗаписи.GlSourceType = "SAM Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "Receivables") Тогда
		//		
		//		Продолжить;
		//	
		//КонецЕсли;
		
		Если НачалоМесяца(ВыборкаДетальныеЗаписи.gl_date) < Дата("20200401000000") Тогда
		...
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Транзакция) Тогда
			...
			//Тогда
				...
				Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КорректировкаТранзакции) Тогда
					...
					ПроверитьАктуальностьКорректировки(ВыборкаДетальныеЗаписи, СтруктураПараметров.Ссылка);
				КонецЕсли;
			//Иначе
			//Продолжить;
			//КонецЕсли;
		...
		Если ВыборкаДетальныеЗаписи.gl_date < ДатаНачалаЗагрузки Тогда
		...
		ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ВыборкаДетальныеЗаписи, СтруктураПараметров.ТипВнешнейСистемы, Отказ
			, ТекстСообщенияОбОшибках, КлиентОтсутствует, КэшAU, СписокКолонокДляЗаполнения);
			
		ТранзакцияОбнулена = Ложь;
		
		Если НЕ ТранзакцияOracleОбъект.Модифицированность() Тогда
			...
			ЗаписьНабора.ПропускатьПроведение = Истина;
...
// { RGS AIvanov 05.07.2019 
// устарела с 01.07.2019
Процедура СформироватьПроводкиDSSSmith___Устарела(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|			OracleSmithSourceData.major ПОДОБНО ""4%"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ТЗ_ИспользованныеКлючи = НЗ_КлючиТранзакцийOracleSmith.ВыгрузитьКолонки("gl_date, psob_id, source, category, batch_name, je_name, je_line_num");
	ТЗ_ИспользованныеКлючи.Индексы.Добавить("gl_date, psob_id, source, category, batch_name, je_name, je_line_num");
	СтруктураПоискаТранзакций = Новый Структура("gl_date, psob_id, source, category, batch_name, je_name, je_line_num");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ЭтоНоваяТранзакция Тогда
			...
			ЗаписьКлючейТранзакции.gl_date = ВыборкаДетальныеЗаписи.gl_date;
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.ТранзакцияПослеЗагрузкиClient = Справочники.Контрагенты.Unreconciled И ВыборкаДетальныеЗаписи.ТранзакцияDocType <> "Revaluation" Тогда
...
Процедура СформироватьПроводкиDSSSmith(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|	OracleSmithSourceData.account_type КАК TRANS_TYPE,
		|	OracleSmithSourceData.sub_ledger_source + ""."" + OracleSmithSourceData.cust_no + ""."" + OracleSmithSourceData.company_code_str КАК DOC_ID,
		|	OracleSmithSourceData.batch_name КАК batch_name,
		|	OracleSmithSourceData.je_name КАК je_name,
		|	OracleSmithSourceData.je_line_num КАК je_line_num,
		|	OracleSmithSourceData.company_code КАК OU,
		|	OracleSmithSourceData.business_line КАК SubSubSegment,
		|	OracleSmithSourceData.cost_center КАК Location,
		|	OracleSmithSourceData.major + ""."" + OracleSmithSourceData.minor + ""."" + OracleSmithSourceData.intercompany + ""."" + OracleSmithSourceData.business_line КАК Account,
		|	OracleSmithSourceData.intercompany КАК intercompany,
		|	OracleSmithSourceData.local КАК local,
		|	OracleSmithSourceData.gl_date КАК GL_DATE,
		|	OracleSmithSourceData.entered_currency КАК CURRENCY_CODE,
		|	OracleSmithSourceData.exch_rate КАК EXCHANGE_RATE,
		|	OracleSmithSourceData.cust_no КАК CUSTOMER_NUMBER,
		|	OracleSmithSourceData.line_desc КАК CUSTOMER_NUMBER_JV,
		|	OracleSmithSourceData.customer_name КАК customer_name,
		|	OracleSmithSourceData.entered_amt КАК ENTERED,
		|	OracleSmithSourceData.rsob_amt КАК AMOUNT_USD,
		|	OracleSmithSourceData.sub_ledger_source КАК DOC_NUMBER,
		|	OracleSmithSourceData.master_source_document_number КАК master_source_document_number,
		|	OracleSmithSourceData.master_source_document_number + ""."" + OracleSmithSourceData.cust_no + ""."" + OracleSmithSourceData.company_code_str КАК LINE_ID,
		|	OracleSmithSourceData.GL_ACCOUNT КАК GL_ACCOUNT,
		|	OracleSmithSourceData.СтрокаФайла КАК СтрокаФайла,
		|	OracleSmithSourceData.source КАК source,
		|	OracleSmithSourceData.category КАК category,
		|	OracleSmithSourceData.cost_center + ""."" + OracleSmithSourceData.business_line КАК AU,
		|	OracleSmithSourceData.posting_date КАК posting_date,
		|	OracleSmithSourceData.ID_ORIG КАК ID_ORIG,
		|	&Префикс + OracleSmithSourceData.ID_ORIG КАК НомерТранзакции,
		|	OracleSmithSourceData.ACCRUAL_ORG_ID КАК ACCRUAL_ORG_ID
		|ПОМЕСТИТЬ ВТ_ДанныеФайла
		|ИЗ
		|	РегистрСведений.OracleSmithSourceData КАК OracleSmithSourceData
		|ГДЕ
		...
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК Priority,
		|	ВТ_ДанныеФайла.psob_id КАК psob_id,
		|	ВТ_ДанныеФайла.batch_name КАК batch_name,
		|	ВТ_ДанныеФайла.je_name КАК je_name,
		|	ВТ_ДанныеФайла.je_line_num КАК je_line_num,
		|	ВЫБОР
		|		КОГДА ВТ_ДанныеФайла.source = ""FMS""
		|			ТОГДА ""SO Accrual""
		|		ИНАЧЕ ВТ_ДанныеФайла.source
		|	КОНЕЦ КАК source,
		|	ВТ_ДанныеФайла.category КАК category,
		|	КостЦентры.Ссылка КАК AU,
		|	ВТ_ДанныеФайла.AU КАК КодAU,
		|	ВТ_ДанныеФайла.posting_date КАК PostingDate,
		|	ВТ_ДанныеФайла.ID_ORIG КАК ID_ORIG,
		|	ЕСТЬNULL(ТранзакцияOracle.Ссылка, ЗНАЧЕНИЕ(Документ.ТранзакцияOracle.ПустаяСсылка)) КАК Транзакция,
		|	ВТ_ДанныеФайла.ACCRUAL_ORG_ID КАК ACCRUAL_ORG_ID
		|ПОМЕСТИТЬ ИтоговаяБезКорректировок
		|ИЗ
		|	ВТ_ДанныеФайла КАК ВТ_ДанныеФайла
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Организации КАК Организации
		...
		|	ИтоговаяБезКорректировок.LineID КАК LineID,
		|	ИтоговаяБезКорректировок.CustomerNumber КАК CustomerNumber,
		|	ИтоговаяБезКорректировок.CUSTOMER_NUMBER_JV КАК CUSTOMER_NUMBER_JV,
		|	ИтоговаяБезКорректировок.DocNumber КАК DocNumber,
		|	ИтоговаяБезКорректировок.master_source_document_number КАК master_source_document_number,
		|	ИтоговаяБезКорректировок.ПорядокAccruals КАК ПорядокAccruals,
		|	ИтоговаяБезКорректировок.GeoMarketHFM КАК GeoMarketHFM,
		|	ИтоговаяБезКорректировок.Priority КАК Priority,
		|	ИтоговаяБезКорректировок.psob_id КАК psob_id,
		|	ИтоговаяБезКорректировок.batch_name КАК batch_name,
		|	ИтоговаяБезКорректировок.je_name КАК je_name,
		|	ИтоговаяБезКорректировок.je_line_num КАК je_line_num,
		|	ИтоговаяБезКорректировок.source КАК source,
		|	ИтоговаяБезКорректировок.category КАК category,
		|	ИтоговаяБезКорректировок.AU КАК AU,
		|	ИтоговаяБезКорректировок.КодAU КАК КодAU,
		|	ИтоговаяБезКорректировок.PostingDate КАК PostingDate,
		|	ИтоговаяБезКорректировок.ID_ORIG КАК ID_ORIG,
		|	ИтоговаяБезКорректировок.Транзакция КАК Транзакция,
		|	КорректировкаТранзакции.Ссылка КАК КорректировкаТранзакции,
		|	ВЫБОР
		|		КОГДА ИтоговаяБезКорректировок.je_name ПОДОБНО ""%Reverse%""
		|				И НЕ ИтоговаяБезКорректировок.je_name ПОДОБНО ""%Reverse%Reverse%""
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК ReverseJV,
		|	ИтоговаяБезКорректировок.ACCRUAL_ORG_ID КАК AccrualOrgID
		|ИЗ
		|	ИтоговаяБезКорректировок КАК ИтоговаяБезКорректировок
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаТранзакции КАК КорректировкаТранзакции
		|		ПО ИтоговаяБезКорректировок.Транзакция = КорректировкаТранзакции.ДокументОснование
	...
	КэшДетальныхРучныхКорректировок = РезультатЗапроса[4].Выгрузить();
	КэшДетальныхРучныхКорректировок.Индексы.Добавить("Source, Company, Journal, Date");
	...
	СписокКолонокВыборки = РезультатЗапроса[11].Колонки;
	СписокКолонокДляЗаполнения = ПолучитьСписокКолонокДляЗаполнения(СписокКолонокВыборки
		, Метаданные.НайтиПоТипу(Тип("ДокументОбъект.ТранзакцияOracle")));
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НачалоМесяца(ВыборкаДетальныеЗаписи.gl_date) < НачалоМесяца(Дата(2020,02,01)) И ВыборкаДетальныеЗаписи.Company = Справочники.Организации.НайтиПоКоду(40) Тогда
			
			Продолжить;
			
		КонецЕсли;
		
		//Если ВыборкаДетальныеЗаписи.GlSourceType = "Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "SO Accrual"
		//		ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "SAM Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "Receivables"
		//		ИЛИ (БазовыйСчет <> Счет12001 И БазовыйСчет <> Счет12002) Тогда
		//	
		//	Продолжить;
		//	
		//КонецЕсли;
		//
		//Если НачалоМесяца(ВыборкаДетальныеЗаписи.gl_date) = НачалоМесяца(Дата(2020, 04, 01)) И ВыборкаДетальныеЗаписи.ReverseJV = Истина 
		//	И НЕ (ВыборкаДетальныеЗаписи.GlSourceType = "Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "SO Accrual"
		//		ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "SAM Sales Accrual" ИЛИ ВыборкаДетальныеЗаписи.GlSourceType = "Receivables") Тогда
		//	
		//	Продолжить;
		//	
		//КонецЕсли;
		
		// { RGS TAlmazova 07.07.2019 14:06:01 - с июля 2019 транзакции грузятся по новому алгоритму, грузить транзакции более ранних периодов запрещено!!!
		Если ВыборкаДетальныеЗаписи.gl_date < Дата(2019,07,01) Тогда
			ТекОшибка = "from July 2019 transactions are loaded using a new algorithm! you cannot download transactions from earlier periods ";
			Отказ = Истина;
			ВызватьИсключение ТекОшибка;
		КонецЕсли;
		// } RGS TAlmazova 07.07.2019 14:06:31 - с июля 2019 транзакции грузятся по новому алгоритму, грузить транзакции более ранних периодов запрещено!!!
		...
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Транзакция) Тогда
			...
			ТранзакцияOracleОбъект = ВыборкаДетальныеЗаписи.Транзакция.ПолучитьОбъект();
			// TODO сделать ПроверитьАктуальностьКорректировки_Smith !!!
			Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КорректировкаТранзакции) Тогда
				//ПроверитьАктуальностьКорректировки_MI(ВыборкаДетальныеЗаписи, СтруктураПараметров.Ссылка);
				ПроверитьАктуальностьКорректировки(ВыборкаДетальныеЗаписи, СтруктураПараметров.Ссылка);
			КонецЕсли;
		...
		ТранзакцияОбнулена = Ложь;
		НайденаОшибкаПриПоискеСвязанныхОбъектов = СформироватьСвязанныеОбъектыSmith(ТранзакцияOracleОбъект, ВыборкаДетальныеЗаписи, КэшSalesOrders, КэшДетальныхРучныхКорректировок, КэшИнвойсов, КэшМемо, КэшCashBatch, КэшBatchAllocation, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, ТранзакцияОбнулена, Отказ, ТекстСообщенияОбОшибках);
...
Функция ПолучитьМассивТранзакцийOracleSmith(СтруктураПараметров)
	...
	Запрос.Текст = 
		...
		|	И КлючиТранзакцийOracleSmith.gl_date = &gl_date
	...
	Запрос.УстановитьПараметр("gl_date", СтруктураПараметров.gl_date);
...
Процедура ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Source, Отказ, ТекстСообщенияОбОшибках,
	КлиентОтсутствует, КэшAU, СписокКолонок)
	
	//ЗаполнитьЗначенияСвойств(ТранзакцияOracleОбъект, ДанныеДляЗаполнения);
	Для Каждого КолонкаДанныхЗаполнения Из СписокКолонок Цикл
		
		Если НЕ (Source =  Перечисления.ТипыСоответствий.OracleMI И (КолонкаДанныхЗаполнения = "GlSourceType" ИЛИ КолонкаДанныхЗаполнения = "Client")) Тогда
		
			РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект[КолонкаДанныхЗаполнения]
				, ДанныеДляЗаполнения[КолонкаДанныхЗаполнения]);	
				
			КонецЕсли; 

	КонецЦикла;
	
	GlSourceType = ПолучитьGlSourceType(ДанныеДляЗаполнения);
	
	Если GlSourceType = Перечисления.OracleGlSourceType.JV Тогда
		
		РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.DateFromDescription, 
				ПолучитьДатуИзDescription(?(Source = Перечисления.ТипыСоответствий.OracleMI, ДанныеДляЗаполнения.Description, ДанныеДляЗаполнения.CUSTOMER_NUMBER_JV)));
				
		Если Source =  Перечисления.ТипыСоответствий.OracleSmith Тогда
			ТранзакцияOracleОбъект.Reverse = ДанныеДляЗаполнения.ReverseJV;
		КонецЕсли;
			
	КонецЕсли;
	
	КлиентПоДаннымДляЗаполнения = ДанныеДляЗаполнения.Client;
	
	Если Source = Перечисления.ТипыСоответствий.OracleMI Тогда
		
		РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Номер, "MI-" + ДанныеДляЗаполнения.ID_ORIG);
		
	ИначеЕсли Source = Перечисления.ТипыСоответствий.OracleSmith Тогда
	// { RGS AIvanov 05.07.2019 
		РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Номер, "SM-" + ДанныеДляЗаполнения.ID_ORIG);
	//	ТранзакцияOracleОбъект.УстановитьНовыйНомер("SM-");
	// } RGS AIvanov 05.07.2019 
	КонецЕсли;
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Дата, ДанныеДляЗаполнения.gl_date); 
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Source, Source);
	
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.GlSourceType, GlSourceType);
	...
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.AU, AU);
	...
	Если НЕ ЗначениеЗаполнено(ТранзакцияOracleОбъект.Client) И ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.JV И Source = Перечисления.ТипыСоответствий.OracleSmith Тогда
		...
		Если Индекс <> 0 Тогда
			...
			Пока ВыборкаКлиенты.Следующий() Цикл
				РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Client, ВыборкаКлиенты.ОбъектПриемника);
			КонецЦикла;
	...
	//Далее ТранзакцияOracleОбъект.Client менять на КлиентПоДаннымДляЗаполнения 
	//Если НЕ ЗначениеЗаполнено(ТранзакцияOracleОбъект.Client) Тогда
	Если НЕ ЗначениеЗаполнено(КлиентПоДаннымДляЗаполнения) Тогда 
		...
		// } RGS TAlmazova 03.10.2016 14:44:47 - поиск по OracleGlSourceType
			...
			//РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Client, Справочники.Контрагенты.Unreconciled);
			РГСофтКлиентСервер.УстановитьЗначение(КлиентПоДаннымДляЗаполнения, Справочники.Контрагенты.Unreconciled);
			КлиентОтсутствует = Истина;
		ИначеЕсли ДанныеДляЗаполнения.CustomerNumber = "" Тогда
			//РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Client, Справочники.Контрагенты.Undefined);
			РГСофтКлиентСервер.УстановитьЗначение(КлиентПоДаннымДляЗаполнения, Справочники.Контрагенты.Undefined);
		Иначе
	...
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Client, КлиентПоДаннымДляЗаполнения);
	
КонецПроцедуры
Процедура ПреобразованиеСокрЛП(ТранзакцияOracleОбъект)
	
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.Contract, СокрЛП(ТранзакцияOracleОбъект.Contract));
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.DocType, СокрЛП(ТранзакцияOracleОбъект.DocType));
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.TransType, СокрЛП(ТранзакцияOracleОбъект.TransType));
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.SONum, СокрЛП(ТранзакцияOracleОбъект.SONum));
	//РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.CreatedBy, СокрЛП(ТранзакцияOracleОбъект.CreatedBy));
	РГСофтКлиентСервер.УстановитьЗначение(ТранзакцияOracleОбъект.CreatedByName, СокрЛП(ТранзакцияOracleОбъект.CreatedByName));
	
КонецПроцедуры

Функция СформироватьСвязанныеОбъекты(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшДетальныхРучныхКорректировок, КэшРучныхКоррерктировок, КэшИнвойсов, КэшCashBatch, КэшBatchAllocation, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, ТранзакцияОбнулена, Отказ, ТекстСообщенияОбОшибках)
	...
	СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
	...
	СтруктураПоискаРучнойКорректировки = Новый Структура("Source, Company, Journal");
	...
	Если ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.SAMAccrual ИЛИ ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.SOAccrual Тогда
		...
		Иначе
			...
			Если НЕ ПустаяСтрока(ДанныеДляЗаполнения.SONum) Тогда
				...
				Если СтрокиSalesOrder.Количество() = 0 Тогда
					...
					SOОснование = СоздатьSalesOrder(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, , InvoiceSAM);
		...
		Если СтрокиSalesOrder.Количество() = 0 Тогда
			...
			Если ТранзакцияOracleОбъект.TransType = "A" Тогда
				...
				ТекSalesOrder = СоздатьSalesOrder(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, SOОснование, InvoiceSAM);
				НоваяСтрокаКэша = КэшSalesOrders.Добавить();
		...
		Иначе
			...
			Если НЕ ЭтоПроводкаПоСчетуВыручки Тогда 
				ДозаполнитьSalesOrderИзПроводки(ТекSalesOrder, ТранзакцияOracleОбъект, ДанныеДляЗаполнения);
			КонецЕсли;
	...
	ИначеЕсли ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.Receivables Тогда
		Если ТранзакцияOracleОбъект.DocType = "DEP" ИЛИ ТранзакцияOracleОбъект.DocType = "GUAR"
			...
			ТекИнвойс = ПолучитьИнвойсПриСоздании(ТранзакцияOracleОбъект.DocID, ТранзакцияOracleОбъект.Source, ДанныеДляЗаполнения.InvoiceAmount);
			...
			Если НЕ ПустаяСтрока(ДанныеДляЗаполнения.InvoiceNumberSAM) ИЛИ НЕ ПустаяСтрока(ТранзакцияOracleОбъект.SONum) Тогда
				...
				Если НЕ ПустаяСтрока(ДанныеДляЗаполнения.InvoiceNumberSAM) Тогда
					СтруктураПоискаSO.Номер =  "SAM-" + ДанныеДляЗаполнения.InvoiceNumberSAM;
					СтруктураПоискаSO.Company = ТранзакцияOracleОбъект.Company;
					СтрокиSalesOrder = КэшSalesOrders.НайтиСтроки(СтруктураПоискаSO);
					СО_Найден = СтрокиSalesOrder.Количество() > 0;
				КонецЕсли;
				
				Если НЕ ПустаяСтрока(ДанныеДляЗаполнения.SONum) И НЕ СО_Найден Тогда
				...
				Если СО_Найден Тогда
					...
					КонецЕсли;
					ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.SalesOrder, ТекSalesOrder);
					ДобавитьСоответствиеВРегистр(ТекSalesOrder, ТекИнвойс);
				КонецЕсли;
		...
		ИначеЕсли ТранзакцияOracleОбъект.DocType = "DM" ИЛИ ТранзакцияOracleОбъект.DocType = "CM" Тогда
			...
			Иначе
				...
				Если ЗначениеЗаполнено(ТекИнвойс) Тогда
					...
					ДозаполнитьИнвойсИзПроводки(ТекИнвойс, ТранзакцияOracleОбъект, ДанныеДляЗаполнения,ЭтоПроводкаПоСчетуВыручки);
					ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.Invoice, ТекИнвойс);
				// { RGS AGorlenko 03.04.2020 15:20:25 - Для проводок по счетам выручки с TransType = CM_REV создаем недостающий инвойс
				ИначеЕсли ЭтоПроводкаПоСчетуВыручки И ТранзакцияOracleОбъект.TransType = "CM_REV" Тогда
					
					ТекИнвойс = СоздатьИнвойс(ТранзакцияOracleОбъект, ДанныеДляЗаполнения);
					ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.Invoice, ТекИнвойс);
					
				// } RGS AGorlenko 03.04.2020 15:21:05 - Для проводок по счетам выручки с TransType = CM_REV создаем недостающий инвойс
				Иначе
		...
		ИначеЕсли ТранзакцияOracleОбъект.DocType = "CASH" Тогда
			...
			Если ТранзакцияOracleОбъект.TransType = "CCURR_REC" ИЛИ ТранзакцияOracleОбъект.TransType = "TRADE_REC" Тогда
				...
				// { RGS TAlmazova 4/9/2018 12:09:53 AM - Искать и инвойс и мемо всегда
				...
				//	Иначе
				ДокументРасчетовИнвойс = ПолучитьИнвойс(ТранзакцияOracleОбъект.LineID, ТранзакцияOracleОбъект.DocNumber, ТранзакцияOracleОбъект.Source);
				ДокументРасчетовМемо = ПолучитьMemo(ТранзакцияOracleОбъект.LineID, ТранзакцияOracleОбъект.DocNumber, ТранзакцияOracleОбъект.Source);
				Если ТипЗнч(ДокументРасчетовИнвойс) = Тип("ДокументСсылка.Invoice") Тогда
					ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.Invoice, ДокументРасчетовИнвойс);
				КонецЕсли;
				Если ТипЗнч(ДокументРасчетовМемо) = Тип("ДокументСсылка.Memo") Тогда
					ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.Memo, ДокументРасчетовМемо);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(ДокументРасчетовИнвойс) И НЕ ЗначениеЗаполнено(ДокументРасчетовМемо) Тогда
				// } RGS TAlmazova 4/9/2018 12:10:04 AM - Искать и инвойс и мемо всегда
						//НайденаОшибка = Истина;
					Если ТранзакцияOracleОбъект.Дата >= ДатаВыверенныхОстатков Тогда
						ТекОшибка = "Failed to find Invoice for transaction " + ТранзакцияOracleОбъект.Номер;
						Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
	...
	Иначе
		...
		//СтрокиРучнойКорректировки = КэшРучныхКоррерктировок.НайтиСтроки(СтруктураПоискаРучнойКорректировки);
		//Если СтрокиРучнойКорректировки.Количество() = 0 Тогда
		//	ТекРучнаяКорректировка = СоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения);
		//	НоваяСтрокаКэша = КэшРучныхКоррерктировок.Добавить();
		//	НоваяСтрокаКэша.Source = ТранзакцияOracleОбъект.Source;
		//	НоваяСтрокаКэша.Company = ТранзакцияOracleОбъект.Company;
	...
	// } RGS TAlmazova 03.10.2016 14:58:21 - изменение порядка условий
		//ДатаРучнойКорректировки = ?(ЗначениеЗаполнено(ТранзакцияOracleОбъект.DateFromDescription), ТранзакцияOracleОбъект.DateFromDescription, ДанныеДляЗаполнения.gl_date);
	
		//СтруктураПоискаКорректировки = Новый Структура("Source, Company, Journal, Date", ТранзакцияOracleОбъект.Source, ТранзакцияOracleОбъект.Company, ?(ТранзакцияOracleОбъект.TransType = "A", ДанныеДляЗаполнения.ID_ORIG, ДанныеДляЗаполнения.AccrualOrgID), ДатаРучнойКорректировки);
		//СтруктураПоискаРучнойКорректировки.Source = ТранзакцияOracleОбъект.Source;
		//СтруктураПоискаРучнойКорректировки.Company = ТранзакцияOracleОбъект.Company;
		//СтруктураПоискаРучнойКорректировки.Journal = ?(ТранзакцияOracleОбъект.TransType = "A", ДанныеДляЗаполнения.ID_ORIG, ДанныеДляЗаполнения.AccrualOrgID);
		//
		//НайденныеРучныеКорректировки = КэшДетальныхРучныхКорректировок.НайтиСтроки(СтруктураПоискаРучнойКорректировки);
		//
		//Если ТранзакцияOracleОбъект.TransType = "A" Тогда
		//	
		//	Если НайденныеРучныеКорректировки.Количество() = 0 Тогда
		//	
		//		ТекКорректировка = СоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения);
		//		НоваяСтрокаКэша = КэшДетальныхРучныхКорректировок.Добавить();
		//		ЗаполнитьЗначенияСвойств(НоваяСтрокаКэша, СтруктураПоискаРучнойКорректировки);
		//		НоваяСтрокаКэша.РучнаяКорректировка = ТекКорректировка;
		//	
		//	КонецЕсли;
		//		
		//ИначеЕсли ТранзакцияOracleОбъект.TransType = "R" Тогда
		//		
		//		Если НайденныеРучныеКорректировки.Количество() = 0 Тогда
		//		
		//			ТекОшибка = "Failed to find Journal Entry for transaction " + ТранзакцияOracleОбъект.Номер;
		//			Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
		//				ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		//			КонецЕсли;
		//			Отказ = Истина;
		...
		//		Иначе
		//			
		//			ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.РучнаяКорректировка, НайденныеРучныеКорректировки[0].РучнаяКорректировка);
		//			
		//		КонецЕсли;
		//		
		//		
		//КонецЕсли;
		НайтиСоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, СтруктураПоискаРучнойКорректировки, КэшДетальныхРучныхКорректировок, Отказ, ТекстСообщенияОбОшибках);
...
Функция СформироватьСвязанныеОбъектыSmith(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшДетальныхРучныхКорректировок, КэшИнвойсов, КэшМемо, КэшCashBatch, КэшBatchAllocation, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, ТранзакцияОбнулена,  Отказ, ТекстСообщенияОбОшибках)
			...
			Иначе
				...
				Если НЕ ЭтоПроводкаПоСчетуВыручки И ТранзакцияOracleОбъект.Дата >= ДатаВыверенныхОстатков Тогда
					ТекОшибка = "Failed to find Sales Order for transaction psob_id = " + ДанныеДляЗаполнения.psob_id
						+ ", gl_date = " + ДанныеДляЗаполнения.gl_date
	...
	Иначе
		...
		//СтруктураПоискаРучнойКорректировки.Source = ТранзакцияOracleОбъект.Source;
		//СтруктураПоискаРучнойКорректировки.Company = ТранзакцияOracleОбъект.Company;
		//СтруктураПоискаРучнойКорректировки.Journal = ?(ДанныеДляЗаполнения.Reverse = Ложь, ДанныеДляЗаполнения.ID_ORIG, ДанныеДляЗаполнения.AccrualOrgID);
		//
		//НайденныеРучныеКорректировки = КэшДетальныхРучныхКорректировок.НайтиСтроки(СтруктураПоискаРучнойКорректировки);
		//
		//Если ТранзакцияOracleОбъект.TransType = "A" Тогда
		...
		НайтиСоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, СтруктураПоискаРучнойКорректировки, КэшДетальныхРучныхКорректировок, Отказ, ТекстСообщенияОбОшибках);
		//КэшДетальныхРучныхКорректировок = СтруктураРезультат.КэшДетальныхРучныхКорректировок;
		//ТекстСообщенияОбОшибках = СтруктураРезультат.ТекстСообщенияОбОшибках;
...
Процедура НайтиСоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, СтруктураПоискаРучнойКорректировки, КэшДетальныхРучныхКорректировок, Отказ, ТекстСообщенияОбОшибках)
	
	Если ТранзакцияOracleОбъект.Source = Перечисления.ТипыСоответствий.OracleMI Тогда
		
		ЭтоНачисление = ?(ТранзакцияOracleОбъект.TransType = "A", Истина, Ложь);
		//ЭтоНачисление = ТранзакцияOracleОбъект.TransType = "A";
		
	Иначе
		
		ЭтоНачисление = НЕ ДанныеДляЗаполнения.ReverseJV;
		
	КонецЕсли;
	
	СтруктураПоискаРучнойКорректировки.Source = ТранзакцияOracleОбъект.Source;
	СтруктураПоискаРучнойКорректировки.Company = ТранзакцияOracleОбъект.Company;
	СтруктураПоискаРучнойКорректировки.Journal = ?(ЭтоНачисление, ДанныеДляЗаполнения.ID_ORIG, ДанныеДляЗаполнения.AccrualOrgID);
	
	НайденныеРучныеКорректировки = КэшДетальныхРучныхКорректировок.НайтиСтроки(СтруктураПоискаРучнойКорректировки);
	
	Счет12001 = ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12001");
	Счет12002 = ПланыСчетов.HFM_GL_Accounts.НайтиПоКоду("12002");
	БазовыйСчет = ТранзакцияOracleОбъект.Account.БазовыйЭлемент;
	
	Если НайденныеРучныеКорректировки.Количество() = 0 Тогда
		
		Если ЭтоНачисление ИЛИ (БазовыйСчет <> Счет12001 И БазовыйСчет <> Счет12002) Тогда
	...
	//СтруктураВозврат = Новый Структура("КэшДетальныхРучныхКорректировок, ТекстСообщенияОбОшибках", КэшДетальныхРучныхКорректировок, ТекстСообщенияОбОшибках);
	//	
...
Функция СоздатьSalesOrder(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Основание = Неопределено, InvoiceSAM = Ложь)
	...
	Если ТранзакцияOracleОбъект.Source = Перечисления.ТипыСоответствий.OracleMI Тогда
		...
		Если InvoiceSAM Тогда
			ДокОбъект.Номер = ДанныеДляЗаполнения.InvoiceNumberSAM;
			ДокОбъект.ArInvoice = "SAM-" + ДанныеДляЗаполнения.InvoiceNumberSAM;
		Иначе
		...
		//{ RGS Alm 21.06.2018 18:11:57 S-E-0000619
		ДокОбъект.JobEndDate = ДанныеДляЗаполнения.ShipDateActual;
		ДокОбъект.CreationDate = ДанныеДляЗаполнения.CreationDate;
		// } RGS Alm 21.06.2018 18:11:57 S-E-0000619
	Иначе
	...
	// { RGS Alm 14.06.2018 15:55:05 S-E-0000795
	//ДокОбъект.Client = ТранзакцияOracleОбъект.Client;
	СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
	ДокОбъект.Client = СтруктураКлиент.Клиент;
	// } RGS Alm 14.06.2018 15:55:05 S-E-0000795
	...
	// { RGS Alm 14.06.2018 17:01:58 S-E-0000795
	//Если НЕ ТранзакцияOracleОбъект.Client.Предопределенный Тогда
	//	ДокОбъект.ClientID = ДанныеДляЗаполнения.CustomerNumber;
	//КонецЕсли;
	Если НЕ СтруктураКлиент.Клиент.Предопределенный Тогда
		ДокОбъект.ClientID = СтруктураКлиент.КодКлиента;
	КонецЕсли;
	// } RGS Alm 14.06.2018 17:01:58 S-E-0000795
...
Функция СоздатьИнвойс(ТранзакцияOracleОбъект, ДанныеДляЗаполнения)
	...
	Если ТранзакцияOracleОбъект.Source = Перечисления.ТипыСоответствий.OracleMI Тогда
		...
		ДокОбъект.ShipDateActual = ТранзакцияOracleОбъект.ShipDateActual;
	Иначе
		ДокОбъект.Дата = ТекущаяДата();
	КонецЕсли;
	// { RGS Alm 14.06.2018 17:04:30 S-E-0000795
	//ДокОбъект.Client = ТранзакцияOracleОбъект.Client;
	СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
	ДокОбъект.Client = СтруктураКлиент.Клиент;
	// } RGS Alm 14.06.2018 17:04:30 S-E-0000795
...
Процедура ДозаполнитьИнвойсИзПроводки(Инвойс, ТранзакцияOracleОбъект, ДанныеДляЗаполнения,ЭтоПроводкаПоСчетуВыручки)
	...
	РеквизитыИнвойса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Инвойс, "Source,Company,DocID,Дата,Client,AU,Location,SubSubSegment,Account,Currency,OriginalAnalytics,ClientID,DocNumber,ShipDateActual");
	...
	Если ЭтоПроводкаПоСчетуВыручки И (НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Client) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Currency)) Тогда 
		...
		Если НЕ РеквизитыИнвойса.Client.Предопределенный Тогда
			// { RGS Alm 14.06.2018 17:48:04 S-E-0000795
			СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
			ИнвойсОбъект.ClientID = СтруктураКлиент.КодКлиента;
			// } RGS Alm 14.06.2018 17:48:04 S-E-0000795
		КонецЕсли;
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Source) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Company) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.DocID) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Дата) ИЛИ 
			...
			Или НЕ ЗначениеЗаполнено(РеквизитыИнвойса.ShipDateActual)
			...
			Если НЕ ЗначениеЗаполнено(РеквизитыИнвойса.ClientID) И НЕ РеквизитыИнвойса.Client.Предопределенный Тогда
				// { RGS Alm 14.06.2018 17:48:56 S-E-0000795
				СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
				ИнвойсОбъект.ClientID = СтруктураКлиент.КодКлиента;
				// } RGS Alm 14.06.2018 17:48:56 S-E-0000795
			КонецЕсли;
...
Функция СоздатьРучнуюКорректировку(ТранзакцияOracleОбъект, ДанныеДляЗаполнения)
	
	РучнаяКорректировкаОбъект = Документы.РучнаяКорректировка.СоздатьДокумент();
	
	//Если ТранзакцияOracleОбъект.Source = Перечисления.ТипыСоответствий.OracleMI Тогда 
		
	ДатаКорректировки = ?(ЗначениеЗаполнено(ТранзакцияOracleОбъект.DateFromDescription), ТранзакцияOracleОбъект.DateFromDescription, ДанныеДляЗаполнения.gl_date);
	
	Если ТранзакцияOracleОбъект.Source = Перечисления.ТипыСоответствий.OracleMI И ЗначениеЗаполнено(ДанныеДляЗаполнения.Analytics3) Тогда
		
		СтрокаAnalytics3 = ДанныеДляЗаполнения.Analytics3;
		
		ОбъектRegEx = РГСофт.СоздатьRegEx();
		
		//РегулярноеВыражение = "20\d{2,2}.\d{2,2}.\d{2,2}";
		РегулярноеВыражение = "\d{2,2}-[a-zA-Z]{3}-\d{2,2}";
		ОбъектRegEx.Pattern = РегулярноеВыражение;
		
		Совпадения = ОбъектRegEx.Execute(СтрокаAnalytics3);
		...
		Если ЧислоВхождений <> 0 Тогда
			...
			Пока ИндексСовпадения < ЧислоВхождений Цикл
				
				Если Совпадения.Item(ИндексСовпадения).Length = СтрДлина(СтрокаAnalytics3) Тогда
				
					ДатаКорректировки = РаботаСФайламиСлужебный.ВозвратДатыRFC1123ДляВнешнегоИспользования(" " + Лев(СтрокаAnalytics3, 2) + " " + Сред(СтрокаAnalytics3, 4,3) + " 20" + Прав(СтрокаAnalytics3, 2));
					Прервать;
				
				КонецЕсли;
				
				ИндексСовпадения = ИндексСовпадения + 1;
	...
	РучнаяКорректировкаОбъект.JournalLawson = ДанныеДляЗаполнения.ID_ORIG;
	...
	СтруктураКлиент = Документы.ТранзакцияOracle.ПолучитьКлиента(ТранзакцияOracleОбъект);
	РучнаяКорректировкаОбъект.Client = СтруктураКлиент.Клиент;
	// } RGS Alm 14.06.2018 17:04:30 S-E-0000795
	...
	// { RGS Alm 14.06.2018 17:06:46 S-E-0000795
	//Если НЕ ТранзакцияOracleОбъект.Client.Предопределенный Тогда
	//	РучнаяКорректировкаОбъект.ClientID = ТранзакцияOracleОбъект.CustomerNumber;
	//КонецЕсли;
	Если НЕ СтруктураКлиент.Клиент.Предопределенный Тогда
		РучнаяКорректировкаОбъект.ClientID = СтруктураКлиент.КодКлиента;
	КонецЕсли;
	// } RGS Alm 14.06.2018 17:06:46 S-E-0000795
...
Процедура ДобавитьСоответствиеВРегистр(ТекSalesOrder, ТекИнвойс)
	
	НЗ = РегистрыСведений.СоответствиеInvoiceSO.СоздатьНаборЗаписей();
	НЗ.Отбор.Invoice.Установить(ТекИнвойс);
	НЗ.Отбор.SalesOrder.Установить(ТекSalesOrder);
	НЗ.Прочитать();
	Если НЗ.Количество() = 0 Тогда
		ЗаписьНабора = НЗ.Добавить();
		ЗаписьНабора.Invoice = ТекИнвойс;
		ЗаписьНабора.SalesOrder = ТекSalesOrder;
		НЗ.Записать();
	КонецЕсли;
...
// { RGS TAlmazova 01.12.2016 17:24:47 - возврат алгоритма поиска инвойса при создании
Функция ПолучитьИнвойсПриСоздании(DocID, Source, InvoiceAmount)
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Invoice.Ссылка КАК Ссылка,
	...
	Инвойс = ВыборкаДетальныеЗаписи.Ссылка;
	
	Если Source = Перечисления.ТипыСоответствий.OracleMI И ВыборкаДетальныеЗаписи.Amount = 0 Тогда
		ИнвойсОбъект =  Инвойс.ПолучитьОбъект();
		ИнвойсОбъект.Amount = InvoiceAmount;
		ИнвойсОбъект.ОбменДанными.Загрузка = Истина;
		ИнвойсОбъект.Записать();
		Инвойс = ИнвойсОбъект.Ссылка;
	КонецЕсли;
...
//Процедура ПроверитьАктуальностьКорректировки_MI(ДанныеЗагрузки, ДокументОснование)- адаптация процедуры под оба источника (MI и Smith)
Процедура ПроверитьАктуальностьКорректировки(ДанныеЗагрузки, ДокументОснование)
	
	РеквизитыТранзакции = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗагрузки.Транзакция, "Source, Company, AU, Account, Currency, Client, CustomerNumber, EndClient, EndCustomerNumber");
	ДанныеКлиентаТранзакции = Документы.ТранзакцияOracle.ПолучитьКлиента(РеквизитыТранзакции);
	
	СтруктураДанныхЗагрузки = Новый Структура("Source, Company, AU, Account, Currency, Client, CustomerNumber, EndClient, EndCustomerNumber, GlSourceType, Транзакция,TransType,DocType, КорректировкаТранзакции");
	ЗаполнитьЗначенияСвойств(СтруктураДанныхЗагрузки,ДанныеЗагрузки);
	//не актально, т.к. клиент определяется через Документы.ТранзакцияOracle.ПолучитьКлиента()
	//Если НЕ ЗначениеЗаполнено(СтруктураДанныхЗагрузки.Client) Тогда
	//	GlSourceType = ПолучитьGlSourceType(СтруктураДанныхЗагрузки);
	//	Если GlSourceType = Перечисления.OracleGlSourceType.JV  
	//		ИЛИ СтруктураДанныхЗагрузки.DocType = "CASH" И СтруктураДанныхЗагрузки.TransType = "TRADE_UNID" Тогда
	//		СтруктураДанныхЗагрузки.Client = Справочники.Контрагенты.Unreconciled;
	//	ИначеЕсли СтруктураДанныхЗагрузки.CustomerNumber = "" Тогда
	//		СтруктураДанныхЗагрузки.Client = Справочники.Контрагенты.Undefined;
	//	КонецЕсли;
	//КонецЕсли;

	ДанныеКлиентаЗагрузки = Документы.ТранзакцияOracle.ПолучитьКлиента(СтруктураДанныхЗагрузки);
	Если НЕ ЗначениеЗаполнено(ДанныеКлиентаЗагрузки.Клиент) Тогда
		ДанныеКлиентаЗагрузки.Клиент = Справочники.Контрагенты.Unreconciled;
	КонецЕсли;
	Если СтруктураДанныхЗагрузки.Company <> РеквизитыТранзакции.Company
		ИЛИ СтруктураДанныхЗагрузки.Account <> РеквизитыТранзакции.Account
		ИЛИ СтруктураДанныхЗагрузки.Currency <> РеквизитыТранзакции.Currency
		// { RGS TAlmazova 05.06.2018 19:42:01 - клиент может быть пустым
		//ИЛИ ДанныеЗагрузки.Client <> РеквизитыТранзакции.Client
		ИЛИ ДанныеКлиентаЗагрузки.Клиент <> ДанныеКлиентаТранзакции.Клиент
		// } RGS TAlmazova 05.06.2018 19:42:14 - клиент может быть пустым
		ИЛИ СтруктураДанныхЗагрузки.AU <> РеквизитыТранзакции.AU Тогда
			
			КорректировкаОбъект = СтруктураДанныхЗагрузки.КорректировкаТранзакции.ПолучитьОбъект();
			КорректировкаОбъект.УстановитьПометкуУдаления(Истина);
			
			//Если СтруктураДанныхЗагрузки.Client = Справочники.Контрагенты.Unreconciled Тогда
			Если ДанныеКлиентаЗагрузки.Клиент = Справочники.Контрагенты.Unreconciled Тогда
				
				НЗ_Проблемы = РегистрыСведений.ПроблемыПриЗагрузке.СоздатьНаборЗаписей();
				НЗ_Проблемы.Отбор.Транзакция.Установить(СтруктураДанныхЗагрузки.Транзакция);
				НЗ_Проблемы.Отбор.ДокументЗагрузки.Установить(ДокументОснование);
				НЗ_Проблемы.Отбор.ОписаниеПроблемы.Установить("Missing client code");
				НоваяЗапись = НЗ_Проблемы.Добавить();
				НоваяЗапись.Транзакция = СтруктураДанныхЗагрузки.Транзакция;
				...
				Задача = Задачи.ЗадачаИсполнителя.СоздатьЗадачу();
				Задача.Дата = ТекущаяДата();
				Задача.Наименование = "Fill the client in the transaction " + СтруктураДанныхЗагрузки.Транзакция;
				//Задача.ОсновнойОбъектАдресации = Перечисления.ТипыСоответствий.OracleMI;
				Задача.ОсновнойОбъектАдресации = СтруктураДанныхЗагрузки.Source;
				Задача.РольИсполнителя = Справочники.РолиИсполнителей.ОтветственныйЗаЗагрузкуПроводок;
				//Задача.СрокИсполнения = ТекущаяДата + 24*60*60;
				Задача.Автор = Пользователи.ТекущийПользователь();
				Задача.Предмет = СтруктураДанныхЗагрузки.Транзакция;
				Задача.Записать();
...
Функция ПолучитьСписокКолонокДляЗаполнения(КолонкиВыборки, МетаданныеДокумента)
	СписокКолонокДляЗаполнения = Новый Массив;
	Для Каждого КолонкаВыборки Из КолонкиВыборки Цикл
		Если МетаданныеДокумента.Реквизиты.Найти(КолонкаВыборки.Имя) <> Неопределено Тогда
			СписокКолонокДляЗаполнения.Добавить(КолонкаВыборки.Имя);
		КонецЕсли;
	КонецЦикла;
	Возврат СписокКолонокДляЗаполнения;
КонецФункции
...
Функция ПолучитьДатуИзDescription(Описание)
	
	ПриведенноеОписание = СокрЛП(Описание);
	ЛевГод = Сред(ПриведенноеОписание, 3, 2);
	ЛевМесяц = Лев(ПриведенноеОписание, 2);
	...
	Если Дата(ДатаИзDescription) = Дата(1,1,1) Тогда
		ПравГод = Прав(ПриведенноеОписание, 2);
		ПравМесяц = Лев(Прав(ПриведенноеОписание, 4),2);
...
///////////////////////////////////////////////////////////////////////////////
// Провести загруженные документы (Post transactions)
///////////////////////////////////////////////////////////////////////////////
#Область Post_transactions
Процедура ПровестиЗагруженныеДокументы(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Запрос.Текст = 
		...
		|	И НЕ DSSСформированныеПриЗагрузке.ПропускатьПроведение";
	...
	//Иначе
	// { RGS agorlenko 14.06.2018 18:34:31 - а теперь и по MI здесь
	...
	Иначе
		...
		ОбновитьСтатусыSalesOrderMI(СтруктураПараметров.Ссылка);
		ОбновитьСтатусыCashBatchOrderMI_1091(СтруктураПараметров.Ссылка);
	...
	//заполнение дат перенесено в модуль объекта проводки
	//ЗаполнитьДатыDIR(СтруктураПараметров.Ссылка);
...
Процедура ОбновитьСтатусыCashBatchOrderMI_1091(ДокументЗагрузки)
	...
	|		ПО (DSSСформированныеПриЗагрузке.ДокументЗагрузки = &ДокументЗагрузки)
	|		И DSSСформированныеПриЗагрузке.ПроводкаDSS = DSSСвязанныеДокументы.ПроводкаDSS
	|		И (DSSСвязанныеДокументы.ТипСвязанногоОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовСвязанныхСпроводкойDSS.CashBatch))
	|ГДЕ
	|	DSSСвязанныеДокументы.СвязанныйОбъект <> ЗНАЧЕНИЕ(Документ.CashBatch.ПустаяСсылка)
	|	И DSSСформированныеПриЗагрузке.ПроводкаDSS.Account.Код ПОДОБНО ""1091%""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СвязанныйОбъект
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_CashBatch.Ссылка КАК Ссылка,
	|	SalesOrdersCommentsСрезПоследних.Problem КАК Problem,
	...
	|ИЗ
	|	ВТ_CashBatch КАК ВТ_CashBatch
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.UnallocatedCash.Остатки(, CashBatch В
	|			(ВЫБРАТЬ
	|				ВТ_CashBatch.Ссылка
	|			ИЗ
	|				ВТ_CashBatch КАК ВТ_CashBatch)) КАК UnallocatedCashОстатки
	|		ПО ВТ_CashBatch.Ссылка = UnallocatedCashОстатки.CashBatch
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.SalesOrdersComments.СрезПоследних КАК SalesOrdersCommentsСрезПоследних
	|		ПО ВТ_CashBatch.Ссылка = SalesOrdersCommentsСрезПоследних.SalesOrder
	|ГДЕ
	|	ЕСТЬNULL(UnallocatedCashОстатки.AmountОстаток, 0) = 0
...
//					СообщениеОбОшибке = "For the selected Cash Batch is not filled Responsible";
//					Прервать;
//				КонецЕсли;
...
Процедура ОбновитьСтатусыSalesOrderMI(ДокументЗагрузки)
	ОбновитьСтатусыSalesOrderMI_Billed(ДокументЗагрузки);
	ОбновитьСтатусыSalesOrderMI_Canceled(ДокументЗагрузки);
КонецПроцедуры

Процедура ОбновитьСтатусыSalesOrderMI_Billed(ДокументЗагрузки)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СоответствиеInvoiceSO.SalesOrder КАК Ссылка
	|ПОМЕСТИТЬ ВТ_SO
	|ИЗ
	|	РегистрСведений.СоответствиеInvoiceSO КАК СоответствиеInvoiceSO
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ИнвойсыСОборотами КАК ВТ_ИнвойсыСОборотами
	|		ПО СоответствиеInvoiceSO.Invoice = ВТ_ИнвойсыСОборотами.Invoice
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Problem) И Выборка.Период >= НачалоМесТекДата Тогда
			...
			Если ЗначениеЗаполнено(Выборка.EscalateTo) Тогда
				...
				МассивОтветственных = Документы.SalesOrder.ПолучитьОтветственныхПоSO(Выборка.Ссылка, Выборка.EscalateTo);
				Если МассивОтветственных.Количество() = 0 Тогда
					СообщениеОбОшибке = СообщениеОбОшибке + "For " + Выборка.Ссылка + " is not filled Responsible" + Символы.ПС;
					////ОтменитьТранзакцию();
					Отказ = Истина;
					Продолжить;
					//МассивОтветственных.Добавить(AutoUser);
				КонецЕсли;
				Проблема.Responsibles.Очистить();
				Для каждого ТекОтветственный Из МассивОтветственных Цикл
					НоваяСтрока = Проблема.Responsibles.Добавить();
...
Процедура ОбновитьСтатусыSalesOrderMI_Canceled(ДокументЗагрузки)
	...
	|		И DSSСформированныеПриЗагрузке.ПроводкаDSS = DSSСвязанныеДокументы.ПроводкаDSS
	|		И
	|			(DSSСвязанныеДокументы.ТипСвязанногоОбъекта = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовСвязанныхСпроводкойDSS.SalesOrder))
	|		И DSSСвязанныеДокументы.СвязанныйОбъект <> ЗНАЧЕНИЕ(Документ.SalesOrder.ПустаяСсылка)
	|		И
	|		НЕ DSSСформированныеПриЗагрузке.ПроводкаDSS.Account.БазовыйЭлемент В ИЕРАРХИИ (&СчетВыручкиВерхнегоУровня)
	|		И DSSСформированныеПриЗагрузке.ПроводкаDSS.SAM_STATUS = ""Deleted""
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СвязанныйОбъект
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТ_SO.СвязанныйОбъект КАК Ссылка,
	|	SalesOrdersCommentsСрезПоследних.Problem КАК Problem,
	|	SalesOrdersCommentsСрезПоследних.Problem.Reason КАК Reason,
	|	SalesOrdersCommentsСрезПоследних.Problem.ExpectedDateForInvoice КАК ExpectedDateForInvoice,
	|	SalesOrdersCommentsСрезПоследних.Problem.EscalateTo КАК EscalateTo,
	...
	|	ВТ_SO КАК ВТ_SO
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.SalesOrdersComments.СрезПоследних КАК SalesOrdersCommentsСрезПоследних
	|		ПО ВТ_SO.СвязанныйОбъект = SalesOrdersCommentsСрезПоследних.SalesOrder
	|ГДЕ
	|	(SalesOrdersCommentsСрезПоследних.Problem.Billed <> ЗНАЧЕНИЕ(Перечисление.SalesOrderBilledStatus.Canceled)
	|	ИЛИ SalesOrdersCommentsСрезПоследних.Problem ЕСТЬ NULL)";
	...
	Пока Выборка.Следующий() Цикл
		...
		Проблема.SalesOrder = Выборка.Ссылка;
		Проблема.User = AutoUser;
		Проблема.Billed = Перечисления.SalesOrderBilledStatus.Canceled;
		Если ЗначениеЗаполнено(Выборка.Problem) И Выборка.Период >= НачалоМесТекДата Тогда
			Проблема.Reason = Выборка.Reason;
			Проблема.ExpectedDateForInvoice = Выборка.ExpectedDateForInvoice;
			Проблема.Details = Выборка.Details;
