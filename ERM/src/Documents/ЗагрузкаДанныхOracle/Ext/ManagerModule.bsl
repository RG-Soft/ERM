...
Процедура СформироватьПроводкиDSS(СтруктураПараметров, АдресХранилища) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ВыборкаДетальныеЗаписи, Отказ, ТекстСообщенияОбОшибках);
		
		ТранзакцияOracleОбъект.Записать(РежимЗаписиДокумента.Запись);
		
		НайденаОшибкаПриПоискеСвязанныхОбъектов = СформироватьСвязанныеОбъекты(ТранзакцияOracleОбъект, ВыборкаДетальныеЗаписи, КэшSalesOrders, КэшРучныхКоррерктировок, КэшИнвойсов, КэшCashBatch, КэшBatchAllocation);
...
Процедура ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Отказ, ТекстСообщенияОбОшибках)
...
Функция СформироватьСвязанныеОбъекты(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшРучныхКоррерктировок, КэшИнвойсов, КэшCashBatch, КэшBatchAllocation)
	...
	Если ТранзакцияOracleОбъект.DocType = "JV" Тогда
		...
		СтруктураПоискаРучнойКорректировки.Source = Перечисления.ТипыСоответствий.OracleMI;
		...
		Если СтрокиРучнойКорректировки.Количество() = 0 Тогда
			...
			НоваяСтрокаКэша.Source = Перечисления.ТипыСоответствий.OracleMI;
	...
	ИначеЕсли ТранзакцияOracleОбъект.DocType = "DEP" ИЛИ ТранзакцияOracleОбъект.DocType = "DM"
		ИЛИ ТранзакцияOracleОбъект.DocType = "GUAR" ИЛИ ТранзакцияOracleОбъект.DocType = "INV"
		ИЛИ ТранзакцияOracleОбъект.DocType = "PMT" ИЛИ ТранзакцияOracleОбъект.DocType = "CB"
		ИЛИ ТранзакцияOracleОбъект.DocType = "CM" Тогда
	...
	ИначеЕсли ТранзакцияOracleОбъект.DocType = "CASH" Тогда
		...
		Если ТранзакцияOracleОбъект.TransType = "CCURR_REC" ИЛИ ТранзакцияOracleОбъект.TransType = "TRADE_REC" Тогда
			...
			// Batch allocation
			СтруктураПоискаBatch.Source = Перечисления.ТипыСоответствий.OracleMI;
...
Функция СоздатьРучнуюКорректировку(ТранзакцияOracleОбъект)
	...
	РучнаяКорректировкаОбъект.Source = Перечисления.ТипыСоответствий.OracleMI;
...
Функция СоздатьИнвойс(ТранзакцияOracleОбъект)
	...
	ДокОбъект.Source = Перечисления.ТипыСоответствий.OracleMI;
	ДокОбъект.Company = ТранзакцияOracleОбъект.Company;
	ДокОбъект.Номер = ?(ПустаяСтрока(ТранзакцияOracleОбъект.DocNumber2), ТранзакцияOracleОбъект.DocNumber, ТранзакцияOracleОбъект.DocNumber2);
