...
Функция ПолучитьСтруктуруКолонокТаблицыДанных(Source) Экспорт
...
Функция ПолучитьТекстЗапросаДляПроверкиНастроекСинхронизацииSmith()
	...
	ТекстЗапроса = 
		...
		|	OracleSmithSourceData.currency,
...
Процедура СформироватьПроводкиDSSSmith(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|	OracleSmithSourceData.currency КАК CURRENCY_CODE,
		...
		|	ВТ_ДанныеФайла.DOC_ID КАК DocID,
		|	ВТ_ДанныеФайла.LINE_ID КАК LineID,
		|	ЕСТЬNULL(КлючиТранзакцийOracleSmith.Транзакция, ЗНАЧЕНИЕ(Документ.ТранзакцияOracle.ПустаяСсылка)) КАК Транзакция,
		|	ВТ_ДанныеФайла.CUSTOMER_NUMBER КАК CustomerNumber,
		|	ВТ_ДанныеФайла.DOC_NUMBER КАК DocNumber,
		|	ВЫБОР
		|		КОГДА ВТ_ДанныеФайла.TRANS_TYPE = ""A""
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СоответствиеКлиентовCustomerNumber КАК ВТ_СоответствиеКлиентовCustomerNumber
		|		ПО ВТ_ДанныеФайла.CUSTOMER_NUMBER = ВТ_СоответствиеКлиентовCustomerNumber.Идентификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КлючиТранзакцийOracleSmith КАК КлючиТранзакцийOracleSmith
		|		ПО ВТ_ДанныеФайла.psob_id = КлючиТранзакцийOracleSmith.psob_id
		|			И ВТ_ДанныеФайла.GL_SOURCE = КлючиТранзакцийOracleSmith.source
		|			И ВТ_ДанныеФайла.DOC_TYPE = КлючиТранзакцийOracleSmith.category
		|			И ВТ_ДанныеФайла.batch_name = КлючиТранзакцийOracleSmith.batch_name
		|			И ВТ_ДанныеФайла.je_name = КлючиТранзакцийOracleSmith.je_name
		|			И ВТ_ДанныеФайла.je_line_num = КлючиТранзакцийOracleSmith.je_line_num
		|
		|УПОРЯДОЧИТЬ ПО
		|	Priority,
		|	GlDate,
...
Функция СформироватьСвязанныеОбъектыSmith(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, КэшSalesOrders, КэшРучныхКоррерктировок, КэшИнвойсов, КэшМемо, КэшCashBatch, КэшBatchAllocation, ДатаНачалаЗагрузки, ДатаВыверенныхОстатков, Отказ, ТекстСообщенияОбОшибках)
	...
	Если ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.SAMAccrual ИЛИ ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.SOAccrual Тогда
		...
		Иначе
			...
			ТекSalesOrder = СтрокаSalesOrder.SalesOrder;
			ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.SalesOrder, ТекSalesOrder);
	...
	ИначеЕсли ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.Receivables Тогда
		...
		ИначеЕсли ТранзакцияOracleОбъект.DocType = "Credit Memo Applications" Тогда
			...
			Если СтрокаМемо = Неопределено Тогда
				...
				Если ТранзакцияOracleОбъект.Дата >= ДатаВыверенныхОстатков Тогда
					ТекОшибка = "Failed to Memo for transaction " + ТранзакцияOracleОбъект.Номер;
					Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
						ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
					КонецЕсли;
					Отказ = Истина;
			...
			Если СтрокаИнвойса = Неопределено Тогда
				...
				Если ТранзакцияOracleОбъект.Дата >= ДатаВыверенныхОстатков Тогда
					ТекОшибка = "Failed to Invoice for transaction " + ТранзакцияOracleОбъект.Номер;
					Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
						ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
					КонецЕсли;
					Отказ = Истина;
		...
		ИначеЕсли ТранзакцияOracleОбъект.DocType = "Trade Receipts" Тогда
...
Процедура ДозаполнитьИнвойсИзПроводки(Инвойс, ТранзакцияOracleОбъект, ДанныеДляЗаполнения)
	...
	РеквизитыИнвойса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Инвойс, "Source,Company,DocID,Дата,Client,Location,SubSubSegment,Account,Currency,OriginalAnalytics,ClientID");
	
	//Для каждого Реквизит из РеквизитыИнвойса Цикл
	//	Если Реквизит.Ключ <> "OriginalAnalytics" И Реквизит.Ключ <> "Дата" Тогда
	//		Если ЗначениеЗаполнено(Реквизит.Значение) И Реквизит.Значение <> ТранзакцияOracleОбъект[Реквизит.Ключ] Тогда
	//			ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
	//			Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
	//				ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
	//			КонецЕсли;
	//			Отказ = Истина;
	...
	//	КонецЕсли;
	//КонецЦикла;
	//Если ЗначениеЗаполнено(РеквизитыИнвойса.OriginalAnalytics) И РеквизитыИнвойса.OriginalAnalytics <> ТранзакцияOracleОбъект.GL_Account Тогда
	//	ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
	//	Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
	//		ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
	//	КонецЕсли;
	//	Отказ = Истина;
	//КонецЕсли;
	//Если ЗначениеЗаполнено(РеквизитыИнвойса.Дата) И РеквизитыИнвойса.Дата <> ТранзакцияOracleОбъект.InvDate Тогда
	//	ТекОшибка = "Different analytics for invoice " + Инвойс.Номер + " in the database and the uploaded document.";
	//	Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
	//		ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
	//	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Source) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Company) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.DocID) ИЛИ НЕ ЗначениеЗаполнено(РеквизитыИнвойса.Дата) ИЛИ 
		...
		Для каждого Реквизит из РеквизитыИнвойса Цикл
			Если Реквизит.Ключ <> "OriginalAnalytics" И Реквизит.Ключ <> "Дата" И Реквизит.Ключ <> "ClientID" Тогда
