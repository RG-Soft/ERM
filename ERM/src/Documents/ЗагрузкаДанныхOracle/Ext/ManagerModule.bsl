...
Процедура СформироватьПроводкиDSSMI(СтруктураПараметров, АдресХранилища)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ВыборкаДетальныеЗаписи, СтруктураПараметров.ТипВнешнейСистемы, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует);
		...
		Если КлиентОтсутствует Тогда
			...
			Задача.РольИсполнителя = Справочники.РолиИсполнителей.ОтветственныйOracle;
	...
	Запрос.Текст = 
		...
		|	DSSСформированныеПриЗагрузке.ПроводкаDSS.Location,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КонтрольныеСуммыПриЗагрузке.ИдентификаторСистемы КАК Location,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДанныеТранзакции КАК ДанныеТранзакции
		|		ПО КонтрольныеСуммыПриЗагрузке.ИдентификаторСистемы = ДанныеТранзакции.ПроводкаDSSLocation
		|			И КонтрольныеСуммыПриЗагрузке.Account = ДанныеТранзакции.ПроводкаDSSAccount
		...
		|	И (ЕСТЬNULL(ДанныеТранзакции.ПроводкаDSSBaseAmount, 0) - ЕСТЬNULL(КонтрольныеСуммыПриЗагрузке.BaseAmount, 0) <> 0
		|			ИЛИ ЕСТЬNULL(ДанныеТранзакции.ПроводкаDSSAmount, 0) - ЕСТЬNULL(КонтрольныеСуммыПриЗагрузке.Amount, 0) <> 0)";
	...
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ТекОшибка = "Checksums do not match for Location " + ВыборкаДетальныеЗаписи.AU;
			Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
				ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
...
Процедура СформироватьПроводкиDSSSmith(СтруктураПараметров, АдресХранилища)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	КонтрольныеСуммыПриЗагрузке.ДокументЗагрузки = &ДокументЗагрузки
		|	И ЕСТЬNULL(ДанныеТранзакции.ПроводкаDSSAmount, 0) - ЕСТЬNULL(КонтрольныеСуммыПриЗагрузке.Amount, 0) <> 0";
	...
	Если ВыборкаДетальныеЗаписи.Количество() > 0 Тогда
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ТекОшибка = СтрШаблон("Checksums do not match for Location = %1, Account = %2, Currency = %3.", ВыборкаДетальныеЗаписи.Location, ВыборкаДетальныеЗаписи.Account, ВыборкаДетальныеЗаписи.Currency);
			Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
				ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Source, Отказ, ТекстСообщенияОбОшибках, КлиентОтсутствует)
...
Процедура ЗаполнитьКонтрольныеСуммы(Ссылка, ТаблицаДанныхКонтрольныеСуммы)
	...
	ТаблицаДанныхКонтрольныеСуммы.Колонки.Location.Имя = "ИдентификаторСистемы";
