...
Процедура ЗаполнитьТранзакцию(ТранзакцияOracleОбъект, ДанныеДляЗаполнения, Source, Отказ, ТекстСообщенияОбОшибках)
	...
	ТранзакцияOracleОбъект.Номер = ?(Source = Перечисления.ТипыСоответствий.OracleMI, "MI-", "SM-") + ДанныеДляЗаполнения.ID_ORIG; 
	...
	ИначеЕсли ТранзакцияOracleОбъект.GlSourceType = Перечисления.OracleGlSourceType.Receivables Тогда
		Если ТранзакцияOracleОбъект.DocType = "DEP" ИЛИ ТранзакцияOracleОбъект.DocType = "GUAR"
			...
			Если Не ЗначениеЗаполнено(ТекИнвойс) Тогда
				ТекИнвойс = СоздатьИнвойс(ТранзакцияOracleОбъект);
			Иначе
				ДозаполнитьИнвойс(ТекИнвойс, ТранзакцияOracleОбъект, Отказ, ТекстСообщенияОбОшибках);
				ДобавитьСвязанныйОбъект(ТранзакцияOracleОбъект, Перечисления.ТипыОбъектовСвязанныхСПроводкойDSS.Invoice, ТекИнвойс);
			КонецЕсли;
...
Процедура ДозаполнитьИнвойс(Инвойс, ТранзакцияOracleОбъект, Отказ, ТекстСообщенияОбОшибках)
	РеквизитыИнвойса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Инвойс, "Source,Company,DocID,Дата,Client,Location,SubSubSegment,Account,Currency,OriginalAnalytics");
	Для каждого Реквизит из РеквизитыИнвойса Цикл
		Если Реквизит.Ключ <> "OriginalAnalytics" И Реквизит.Ключ <> "Дата" Тогда
			Если ЗначениеЗаполнено(Реквизит.Значение) И Реквизит.Значение <> ТранзакцияOracleОбъект[Реквизит.Ключ] Тогда
				ТекОшибка = "Different analyst on the invoice " + Инвойс["Номер"] + " in the database and the uploaded document.";
				Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
					ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
				КонецЕсли;
				Отказ = Истина;
		...
		КонецЕсли;
	КонецЦикла;
	Если ЗначениеЗаполнено(Инвойс.OriginalAnalytics) И Инвойс.OriginalAnalytics <> ТранзакцияOracleОбъект.GL_Account Тогда
		ТекОшибка = "Different analyst on the invoice " + Инвойс.Номер + " in the database and the uploaded document ";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
		Отказ = Истина;
	КонецЕсли;
	Если ЗначениеЗаполнено(Инвойс.Дата) И Инвойс.Дата <> ТранзакцияOracleОбъект.InvDate Тогда
		ТекОшибка = "Different analyst on the invoice " + Инвойс.Номер + " in the database and the uploaded document.";
		Если СтрНайти(ТекстСообщенияОбОшибках, ТекОшибка) = 0 Тогда
			ТекстСообщенияОбОшибках = ТекстСообщенияОбОшибках + ТекОшибка + Символы.ПС;
		КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Для каждого Реквизит из РеквизитыИнвойса Цикл
			...
			КонецЕсли;
		КонецЦикла;
		Если НЕ ЗначениеЗаполнено(Инвойс.OriginalAnalytics) Тогда
			Инвойс.OriginalAnalytics = ТранзакцияOracleОбъект.GL_Account;
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(Инвойс.Дата) Тогда
			Инвойс.Дата = ТранзакцияOracleОбъект.InvDate;
		КонецЕсли;
