...
Функция ПолучитьОтветственныхПоSO(SalesOrder, Уровень) Экспорт
	...
	Если ЗначениеЗаполнено(Уровень) Тогда
		...
		СтруктураОтбора.Вставить("Настройка", ПланыВидовХарактеристик.НастройкиКонфигурации.HOBARSupervisor);
		SupervisorHOB = РегистрыСведений.СохраненныеНастройкиКонфигурации.Получить(СтруктураОтбора).Значение;
		Если ЗначениеЗаполнено(SupervisorHOB) И SalesOrder.Source = Перечисления.ТипыСоответствий.HOBs Тогда
