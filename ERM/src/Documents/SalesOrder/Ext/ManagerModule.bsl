...
Функция ПолучитьОтветственныхПоSO(SalesOrder, Уровень) Экспорт
	...
	Если ЗначениеЗаполнено(Уровень) Тогда
		...
		СтруктураОтбора.Вставить("Настройка", ПланыВидовХарактеристик.rgsНастройкиКонфигурации.HOBARSupervisor);
		Supervisor = РегистрыСведений.rgsСохраненныеНастройкиКонфигурации.Получить(СтруктураОтбора).Значение;
		Если ЗначениеЗаполнено(Supervisor) Тогда
			МассивОтветственных.Добавить(Supervisor);
		КонецЕсли;
