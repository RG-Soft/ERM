...
Процедура ЗаполнитьДанныеПоБиллингу(СтруктураПараметров,
	...
	Запрос.Текст = "ВЫБРАТЬ
		...
		|	ВложенныйЗапросАктуальныеОстаткиБезОплат.Invoice.Contract.ПроцентЗадолженности КАК Percent,
		...
		|	ВТ_ОплатыЗаПериодРасчетаСДатамиОплаты.Invoice.Contract.ПроцентЗадолженности,
	...
	Пока ВыборкаИнвойс.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если ВыборкаДетальныеЗаписи.ПриоритетОбработки = 2
				...
				Продолжить;
			КонецЕсли;
			КонецПериодаРасчета = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДатаОплаты), КонецДня(ВыборкаДетальныеЗаписи.ДатаОплаты), КонецМесяца(СтруктураПараметров.ПериодРасчета));
			ЗаполнитьСтрокуДанныхПоШтрафам(ДанныеПоШтрафам, ВыборкаДетальныеЗаписи, НачалоПериодаРасчета, КонецПериодаРасчета, ТекущийБаланс, КэшКурсов);
			НачалоПериодаРасчета = КонецПериодаРасчета + 1;
...
Процедура ЗаполнитьСтрокуДанныхПоШтрафам(ДанныеПоШтрафам, Параметры, НачалоПериодаРасчета, КонецПериодаРасчета, Баланс, КэшКурсов)
	...
	ЗаполнитьЗначенияСвойств(СтрокаШтрафа, Параметры, "Company, LegalEntity, AU, Client, Contract, Invoice, DueDate, Currnecy, Percent");
	...
	СтрокаШтрафа.RemainingAmountUSD = СтрокаШтрафа.RemainingAmount / ПолучитьКурсИзКэша(
		КэшКурсов, Параметры.Currnecy, КонецМесяца(КонецПериодаРасчета));
	СтрокаШтрафа.PenaltyAmount = СтрокаШтрафа.RemainingAmountUSD
		* СтрокаШтрафа.Percent * (ДеньГода(КонецПериодаРасчета) - ДеньГода(НачалоПериодаРасчета));
	СтрокаШтрафа.IsProblem = Параметры.ЕстьПроблемаРасчета;
