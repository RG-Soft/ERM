...
Процедура ЗаполнитьДанныеПоШтрафамЗадолженности(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И Invoice.DueDateTo < &КонецПериодаРасчета
	               |				И НЕ Client.Intercompany
	               |				И НЕ Invoice.Contract.NonTrade
	               |				И Client <> ЗНАЧЕНИЕ(Справочник.Контрагенты.NonTrade)) КАК BilledARОстатки
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               ...
	               |				И НЕ Client.Intercompany
	               |				И НЕ Invoice.Contract.NonTrade
	               |				И Client <> ЗНАЧЕНИЕ(Справочник.Контрагенты.NonTrade)) КАК PaymentsОбороты
	               |ГДЕ
	               |	ВЫБОР
...
Процедура ЗаполнитьДанныеПоBenefit(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice КАК Invoice,
	               |	ВТ_ОплатыЗаПериодРасчета.Currency КАК Currency,
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.Contract КАК Contract,
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.Contract.СпособРасчетаШтрафаПоЗадолженности КАК СпособРасчетаШтрафаПоЗадолженности,
	               |	ЕСТЬNULL(ВТ_ОплатыЗаПериодРасчета.Invoice.Contract.ПроцентЗадолженности, 0) КАК Percent,
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.DueDateTo КАК DueDate,
	               |	СУММА(ВТ_ОплатыЗаПериодРасчета.AmountОборот) КАК BaseAmount,
	               |	ВЫБОР
	               ...
	               |		ИНАЧЕ ЕСТЬNULL(ВТ_LawsonDepositDates.DepositDate, ВТ_ОплатыЗаПериодРасчета.Период)
	               |	КОНЕЦ,
	               |	ЕСТЬNULL(ВТ_ОплатыЗаПериодРасчета.Invoice.Contract.ПроцентЗадолженности, 0),
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.Contract,
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.Contract.СпособРасчетаШтрафаПоЗадолженности,
	               |	ВТ_ОплатыЗаПериодРасчета.Invoice.DueDateTo";
...
//	ДанныеПоСтавкеРефинансирования =  ПолучитьДанныеПоСтавкеРефинансирования(СтруктураПараметров.ПериодРасчета);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.BaseAmount = 0 ИЛИ ВыборкаДетальныеЗаписи.PenaltyDate >= ВыборкаДетальныеЗаписи.DueDate Тогда
			Продолжить;
		КонецЕсли;
		
		ЕстьПроблема = ВыборкаДетальныеЗаписи.BaseAmount < 0;
		ОписаниеПроблемы = "";
		...
		ИначеЕсли ВыборкаДетальныеЗаписи.СпособРасчетаШтрафаПоЗадолженности = Справочники.СпособыРасчетаШтрафаПоЗадолженности.СогласноГК Тогда
			...
			ЗапросСтавка.Текст = 
			"ВЫБРАТЬ
			|	ВЫРАЗИТЬ(СтавкаРефинансированияЦБСрезПоследних.Размер / ДЕНЬГОДА(КОНЕЦПЕРИОДА(&НачалоПериода, ГОД)) КАК ЧИСЛО(15,
			|		2)) КАК Размер
			|ИЗ
			|	РегистрСведений.СтавкаРефинансированияЦБ.СрезПоследних(&НачалоПериода,) КАК СтавкаРефинансированияЦБСрезПоследних
			|";
			ЗапросСтавка.УстановитьПараметр("НачалоПериода", ВыборкаДетальныеЗаписи.PenaltyDate);
		...
		СтрокаПоBenefit.OverDueDays = (НачалоДня(ВыборкаДетальныеЗаписи.DueDate) - НачалоДня(ВыборкаДетальныеЗаписи.PenaltyDate)) / (60*60*24) + 1;
...
Процедура ЗаполнитьДанныеПоOnTime(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_ОплатыЗаПериодРасчетаСДатамиОплаты.Invoice.Contract.СпособРасчетаШтрафаПоЗадолженности КАК СпособРасчетаШтрафаПоЗадолженности,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтрокаOnTime.OverDueDays = 0;
		СтрокаOnTime.BaseAmountUSD = СтрокаOnTime.BaseAmount / ПолучитьКурсИзКэша(
			КэшКурсов, ВыборкаДетальныеЗаписи.Currency, КонецМесяца(ВыборкаДетальныеЗаписи.PenaltyDate));
		СтрокаOnTime.PenaltyAmount = 0;
		СтрокаOnTime.PenaltyAmountUSD  = 0;
...
Процедура ЗаполнитьСтрокуДанныхПоШтрафам(ДанныеПоШтрафам, Параметры, НачалоПериодаРасчета, КонецПериодаРасчета, Баланс, 
	...
	СтрокаШтрафа.OverDueDays = (НачалоДня(КонецПериодаРасчета) - НачалоДня(НачалоПериодаРасчета)) / (60*60*24) + 1;
