...
Процедура ЗаполнитьДанныеПоШтрафам(СтруктураПараметров, АдресХранилища) Экспорт
	...
	ВыбокаClient = Запрос.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
						...
						Пока ВыборкаAU.Следующий() Цикл
							...
							Пока ВыборкаLegalEntity.Следующий() Цикл
								...
								Пока ВыборкаClientID.Следующий() Цикл
									...
									ВыборкаИнвойс = ВыборкаClientID.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура ЗаполнитьСтрокуДанныхПоШтрафам(ДанныеПоШтрафам, Параметры, НачалоПериодаРасчета, КонецПериодаРасчета, Баланс, КэшКурсов)
	...
	СтрокаШтрафа.OverDueDays = (КонецПериодаРасчета - НачалоПериодаРасчета)/(60*60*24);
	...
	СтрокаШтрафа.PenaltyAmount = СтрокаШтрафа.RemainingAmountUSD
		* СтрокаШтрафа.Percent/100 * (ДеньГода(КонецПериодаРасчета) - ДеньГода(НачалоПериодаРасчета) + 1);
