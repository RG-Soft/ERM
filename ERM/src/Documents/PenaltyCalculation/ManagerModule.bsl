...
Процедура ЗаполнитьДанныеПоBenefit(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_ОплатыЗаПериодРасчетаСДатамиОплаты.AmountОборот КАК Amount,
	               |	ВТ_ОплатыЗаПериодРасчетаСДатамиОплаты.ДатаОплаты КАК ДатаОплаты,
	...
	//ДанныеПоШтрафам.Колонки.Добавить("PenaltyDate");
	ДанныеПоBenefit.Колонки.Добавить("RemainingAmount");
	ДанныеПоBenefit.Колонки.Добавить("RemainingAmountUSD");
							...
							Пока ВыборкаAU.Следующий() Цикл
								...
								Пока ВыборкаLegalEntity.Следующий() Цикл
									...
									Пока ВыборкаClientID.Следующий() Цикл
										...
										//ТекущийБаланс = ?(ВыборкаДетальныеЗаписи.ПриоритетОбработки = 1, 
										//	ВыборкаДетальныеЗаписи.Amount, -ВыборкаДетальныеЗаписи.Amount);
										СуммаОплаты = ВыборкаДетальныеЗаписи.Amount;
										НачалоПериодаРасчета =НачалоМесяца(СтруктураПараметров.ПериодРасчета);
										КонецПериодаРасчета = Мин(КонецМесяца(СтруктураПараметров.ПериодРасчета), КонецДня(DueDate));
										Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
											Если ВыборкаДетальныеЗаписи.ДатаОплаты >= КонецДня(DueDate) Тогда
												СуммаОплаты = СуммаОплаты + ВыборкаДетальныеЗаписи.Amount;
												Продолжить;
											КонецЕсли;
											ТекКонецПериодаРасчета = ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДатаОплаты), 
												КонецДня(ВыборкаДетальныеЗаписи.ДатаОплаты), КонецМесяца(СтруктураПараметров.ПериодРасчета));
											Если ВыборкаДетальныеЗаписи.ЕстьПроблемаРасчета Тогда
												ОписаниеПроблемы = "По инвойсу не загружена дата оплаты";
											КонецЕсли;
											ЗаполнитьСтрокиДанныхПоШтрафам(ДанныеПоBenefit, ВыборкаДетальныеЗаписи, НачалоПериодаРасчета, 
												ТекКонецПериодаРасчета, СуммаОплаты, ДанныеПоСтавкеРефинансирования, КэшКурсов, 
												ЕстьПроблема ИЛИ ВыборкаДетальныеЗаписи.ЕстьПроблемаРасчета, ОписаниеПроблемы);
											НачалоПериодаРасчета = ТекКонецПериодаРасчета + 1;
											СуммаОплаты = СуммаОплаты + ВыборкаДетальныеЗаписи.Amount;
										КонецЦикла;
										...
										Если НачалоПериодаРасчета < КонецПериодаРасчета Тогда
											ЗаполнитьСтрокиДанныхПоШтрафам(ДанныеПоBenefit, ВыборкаДетальныеЗаписи, НачалоПериодаРасчета, 
												КонецПериодаРасчета, СуммаОплаты, ДанныеПоСтавкеРефинансирования, КэшКурсов,
												ЕстьПроблема ИЛИ ВыборкаДетальныеЗаписи.ЕстьПроблемаРасчета, ОписаниеПроблемы);
										КонецЕсли;
...
Процедура ЗаполнитьДанныеПоOnTime(СтруктураПараметров, ДанныеДляЗаполнения)
	...
	ДанныеПоOnTime.Колонки.Добавить("RemainingAmount");
	ДанныеПоOnTime.Колонки.Добавить("RemainingAmountUSD");
							...
							Пока ВыборкаAU.Следующий() Цикл
								...
								Пока ВыборкаLegalEntity.Следующий() Цикл
									...
									Пока ВыборкаClientID.Следующий() Цикл
										...
										ВыборкаДетальныеЗаписи.Следующий();
										СуммаОплаты = ВыборкаДетальныеЗаписи.Amount;
										//НачалоПериодаРасчета = КонецДня(DueDate);
										//КонецПериодаРасчета = Мин(КонецМесяца(СтруктураПараметров.ПериодРасчета), КонецДня(DueDate));
										Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
											Если ВыборкаДетальныеЗаписи.ДатаОплаты >= КонецДня(DueDate) Тогда
												СуммаОплаты = СуммаОплаты + ВыборкаДетальныеЗаписи.Amount;
											...
											Если ВыборкаДетальныеЗаписи.ЕстьПроблемаРасчета Тогда
												ОписаниеПроблемы = "По инвойсу не загружена дата оплаты";
											КонецЕсли;
											ЗаполнитьСтрокиДанныхПоШтрафам(ДанныеПоOnTime, ВыборкаДетальныеЗаписи, КонецДня(DueDate), 
												КонецДня(DueDate), СуммаОплаты, ДанныеПоСтавкеРефинансирования, КэшКурсов, 
												ЕстьПроблема ИЛИ ВыборкаДетальныеЗаписи.ЕстьПроблемаРасчета, ОписаниеПроблемы);
											//НачалоПериодаРасчета = ТекКонецПериодаРасчета + 1;
											СуммаОплаты = СуммаОплаты + ВыборкаДетальныеЗаписи.Amount;
										КонецЦикла;
	...
	Для каждого СтрокаДанных Из ДанныеПоOnTime Цикл
		СтрокаДанных.OverdueDays = 0;
		СтрокаДанных.PenaltyAmount = 0;
		СтрокаДанных.PenaltyAmountUSD = 0;
	КонецЦикла;
