...
Процедура ПриПолученииДанныхОтГлавногоВНачале(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Иначе
		...
		Если Отправитель.Ссылка = rgsОбменНачальноеЗаполнениеERMПовтИспСеанс.ПолучитьУзелBilling()
			И ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.Контрагенты") И ЭлементДанных.ЭтоНовый() Тогда
			ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
		КонецЕсли;
		// } RGS AGorlenko 11.08.2016 5:04:38 - поддержка логики обмена с лоджелкой
		// { RGS AArsentev 17.08.2016 13:30:44
		Биллинг = rgsОбменДокументамиLogelcoERMПовтИспСеанс.ПолучитьУзелBilling();
		Если Биллинг <> Неопределено И НЕ ЭлементДанных.ЭтоНовый() Тогда		
			...
			Если Отправитель.Ссылка <> Биллинг.Ссылка И ЕстьЗапись Тогда				
				ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
			КонецЕсли;
		КонецЕсли;
		// } RGS AArsentev 17.08.2016 13:30:44
