...
Процедура ПриПолученииДанныхОтГлавногоВНачале(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Иначе
		...
		Билинг = rgsОбменДокументамиLogelcoERMПовтИспСеанс.ПолучитьУзелBilling();
		Если Билинг <> Неопределено И НЕ ЭлементДанных.ЭтоНовый() Тогда		
			ДанныеОбмена = Новый Структура;
			ДанныеОбмена.Вставить("Ссылка",ЭлементДанных.Ссылка);
			ДанныеОбмена.Вставить("УзелИнформационнойБазы",Билинг);
			ЕстьЗапись = РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(ДанныеОбмена);
			Если Отправитель.Ссылка <> Билинг.Ссылка Тогда
