...
//  Отправитель    - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена от которого принимается сообщение обмена.
...
//
//  УзелПланаОбмена     - ПланыОбменаОбъект - УзелИнформационнойБазы,
...
Процедура ПриПолученииДанныхОтГлавногоВНачале(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Иначе
		...
		// { RGS AGorlenko 11.08.2016 5:04:22 - поддержка логики обмена с лоджелкой
		//Если Отправитель.Ссылка = rgsОбменДокументамиLogelcoERMПовтИспСеанс.ПолучитьУзелLogelco() И ЭлементДанных.ЭтоНовый() Тогда
		//	ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
		//КонецЕсли;
		// при начальном заполнении данных клиентов игнорируем тех, которые пытаются создаться
		Если (ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.Контрагенты") ИЛИ ТипЗнч(ЭлементДанных) = Тип("СправочникОбъект.Организации")) И ЭлементДанных.ЭтоНовый() Тогда
			ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
		КонецЕсли;
		// } RGS AGorlenko 11.08.2016 5:04:38 - поддержка логики обмена с лоджелкой
		//// { RGS AArsentev 17.08.2016 13:30:44
		//Биллинг = rgsОбменДокументамиERMПовтИспСеанс.ПолучитьУзелBilling();
		//Если Биллинг <> Неопределено И НЕ ЭлементДанных.ЭтоНовый() Тогда		
		//	ДанныеОбмена = Новый Структура;
		//	ДанныеОбмена.Вставить("Ссылка",ЭлементДанных.Ссылка);
		//	ДанныеОбмена.Вставить("УзелИнформационнойБазы", Биллинг);
		//	ЕстьЗапись = РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(ДанныеОбмена);
		//	Если Отправитель.Ссылка <> Биллинг И ЕстьЗапись Тогда				
		//		ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
		//	КонецЕсли;
		//КонецЕсли;
		//// } RGS AArsentev 17.08.2016 13:30:44
...
//
// Параметры:
//     Получатель - ПланОбменаСсылка          - УзелИнформационнойБазы плана обмена, для которого выполняется регистрация изменений
//                                              данных;
...
Процедура СохранитьРазрешенныеКВыгрузкеОбъекты(УзелОбъект)
	...
	|ГДЕ
	|	ТаблицаИзменений.УзелИнформационнойБазы = &УзелИнформационнойБазы";
	Запрос.УстановитьПараметр("УзелИнформационнойБазы", УзелОбъект.Ссылка);
...
Функция УзлыДляРегистрацииПоУсловиюВыгружатьВсегда(Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага) Экспорт
	...
	ТекстЗапроса = "
	|ВЫБРАТЬ
	|	ШапкаПланаОбмена.Ссылка КАК УзелИнформационнойБазы
	|ИЗ
	...
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("УзелИнформационнойБазы");
КонецФункции
...
Функция УзлыДляРегистрацииПоУсловиюВыгружатьПриНеобходимости(Ссылка, Знач ИмяПланаОбмена, Знач ИмяРеквизитаФлага) Экспорт
	...
	Иначе
		...
		МассивУзлов = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("УзелИнформационнойБазы");
...
//    * АвторВерсии - Пользователь или УзелИнформационнойБазы плана обмена - Источник версии.
//        Необязательный, значение по умолчанию Неопределено.
