...
Процедура СозданиеКорректировокПоНовымКлиентамИзCRM() Экспорт
	...
	Запрос.Текст = 
		...
		|					И ОбъектПриемника <> &Client) КАК НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних
		...
		|ИЗ
		|	Транзакции_Клиент КАК Транзакции_Клиент
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Корректировка) Тогда
			...
			ЗаполнитьЗначенияСвойств(КорректировкаТразакции, ВыборкаДетальныеЗаписи.Транзакция, "Company,Account,Currency,AU,LegalEntity");
		...
		Иначе
			...
			Если НазначитьЗадачу И НЕ ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.Задача) Тогда
				...
				ИначеЕсли ВыборкаДетальныеЗаписи.Source = Перечисления.ТипыСоответствий.OracleMI Тогда
					Задача.РольИсполнителя = Справочники.РолиИсполнителей.ОтветственныйOracleMI;
				Иначе
					Задача.РольИсполнителя = Справочники.РолиИсполнителей.ОтветственныйOracleSmith;
				КонецЕсли;
