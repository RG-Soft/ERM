...
Процедура ОтправитьУведомление() Экспорт
	...
	ДанныеSalesOrders = ПолучитьДанныеПоSalesOrders();
	
	Если ДанныеSalesOrders.Количество() > 0 Тогда
		
		Получатели_Ордера = СобратьПолучателейОрдеров(ДанныеSalesOrders);
...
Функция ПолучитьДанныеПоSalesOrders()
...
Процедура АктуализироватьРегистрОчередьУведомлений()
	...
	|	ОчередьУведомлений.Уровень,
	|	ОчередьУведомлений.Проблема.SalesOrder КАК SalesOrder,
	|	ОчередьУведомлений.Проблема.Дата КАК ПроблемаДата
	|ИЗ
	|	РегистрСведений.ОчередьУведомлений КАК ОчередьУведомлений
	|
	|УПОРЯДОЧИТЬ ПО
