...
Процедура ОтправитьUnbilledNotification() Экспорт
	...
	ДанныеSalesOrders = ПолучитьДанныеПоSalesOrdersДляUnbilledNotification();
	...
	Если ДанныеSalesOrders.Количество() > 0 Тогда
		...
		Для Каждого Получатель из УникальныеПолучатели Цикл 
			...
			Attach = ПолучитьAttachДляUnbilledNotification(Ордера, ид);
...
Функция ПолучитьДанныеПоSalesOrdersДляUnbilledNotification()
...
Функция ПолучитьAttachДляUnbilledNotification(Таблица, ид)
...
Процедура АктуализироватьРегистрОчередьУведомлений()
	...
	Пока Результат.Следующий() Цикл
		...
		ЗапросПроблемы = Новый Запрос;
		ЗапросПроблемы.Текст =  "ВЫБРАТЬ
		|	ОчередьУведомлений.Проблема,
		|	ОчередьУведомлений.Уровень
		|ИЗ
		|	РегистрСведений.ОчередьУведомлений КАК ОчередьУведомлений
		|ГДЕ
		|	ОчередьУведомлений.Проблема <> &Проблема
		|	И ОчередьУведомлений.Проблема.SalesOrder = &SalesOrder";
		ЗапросПроблемы.УстановитьПараметр("Проблема",Результат.Problem);
		ЗапросПроблемы.УстановитьПараметр("SalesOrder",Результат.SalesOrder);
		
		РезультатПроблемы = ЗапросПроблемы.Выполнить().Выбрать();
		
		Пока РезультатПроблемы.Следующий() Цикл
			
			НаборЗаписей = РегистрыСведений.ОчередьУведомлений.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Проблема.Установить(РезультатПроблемы.Проблема);
			НаборЗаписей.Отбор.Уровень.Установить(РезультатПроблемы.Уровень);
			НаборЗаписей.Записать();
