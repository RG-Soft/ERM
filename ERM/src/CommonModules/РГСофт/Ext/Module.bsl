...
Процедура ОтправитьУведомление() Экспорт
	...
	НачатьТранзакцию();
	АктуализироватьРегистр();
	Данные = СобратьТаблицу();
	
	Если Данные.Количество() > 0 Тогда
		
		Получатели_Ордера = СобратьПолучателейОрдеров(Данные);
		...
		Для Каждого Получатель из УникальныеПолучатели Цикл 
			...
			Для Каждого Ордер из Ордера Цикл
				...
				ПроблемыПоОрдеру = Данные.Скопировать(Отбор);
			...
			Attach = ПолучитьAttach(Ордера, ид);
...
Функция ПолучитьAttach(Таблица, ид)
	
	ТабДок = Новый ТабличныйДокумент;
	
	Построитель = Новый ПостроительОтчета();
	
	ТаблицаДляAttach = ПолучитьДанныеДляAttach(Таблица.ВыгрузитьКолонку("SalesOrder"));
...
Функция ПолучитьДанныеДляAttach(МассивОрдеров)
...
Функция СобратьПолучателейОрдеров(Данные)
...
Процедура АктуализироватьРегистр()
	...
	Ордера = Результат.Скопировать();
	Ордера.Свернуть("SalesOrder");
	
	Для Каждого Строка Из Ордера Цикл
		
		Отбор = Новый Структура;
		Отбор.Вставить("SalesOrder", Строка.SalesOrder);
		
		ПроблемыПоОрдеру = Результат.Скопировать(Отбор);
		ПроблемыПоОрдеру.Сортировать("ДатаУведомления Убыв");
		
		КолВоПроблем = ПроблемыПоОрдеру.Количество();
		ПервыйЭлемент = Истина;
		ПерваяПроблема = Документы.SalesOrderProblem.ПустаяСсылка();
		Если КолВоПроблем > 1 Тогда
			
			Для каждого Проблема из ПроблемыПоОрдеру Цикл
				Если ПервыйЭлемент Тогда
					ПервыйЭлемент = Ложь;
					ПерваяПроблема = Проблема.Проблема;
					Продолжить
				Иначе
					НаборЗаписей = РегистрыСведений.ОчередьУведомлений.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Проблема.Установить(Проблема.Проблема);
