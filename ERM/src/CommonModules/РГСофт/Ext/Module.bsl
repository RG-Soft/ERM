...
Процедура СозданиеКорректировокПоНовымКлиентамИзCRM() Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(ВЫБОР
		|			КОГДА КорректировкаТранзакции.Ссылка ЕСТЬ NULL
		|					ИЛИ КорректировкаТранзакции.ПометкаУдаления
		|				ТОГДА 0
		|			ИНАЧЕ 1
		|		КОНЕЦ) КАК НаличиеКорректировки
		|ИЗ
		...
		|	Транзакции_Клиент.Транзакция.Amount,
		|	Транзакции_Клиент.Транзакция.BaseAmount
		|
		|ИМЕЮЩИЕ
		|	СУММА(ВЫБОР
		|			КОГДА КорректировкаТранзакции.Ссылка ЕСТЬ NULL
