...
Функция ПолучитьДанныеПоSalesOrdersДляUnbilledNotification(Период)
	...
	Запрос.Текст =  "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |	РегистрСведений.ОчередьУведомлений КАК ОчередьУведомлений
	                |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПолучателиУведомленийUnbilled КАК ПолучателиУведомленийUnbilled
	                |		ПО (ОчередьУведомлений.Проблема.SalesOrder.Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU))
	                |			И (ПолучателиУведомленийUnbilled.Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU))
	                |			И ОчередьУведомлений.Уровень = ПолучателиУведомленийUnbilled.Уровень
	                |			И ОчередьУведомлений.Проблема.SalesOrder.Company = ПолучателиУведомленийUnbilled.Идентификатор1
	                |ГДЕ
	                |	ОчередьУведомлений.ДатаУведомления <= &ДатаУведомления
	                |	И ОчередьУведомлений.Проблема.SalesOrder.Source = ЗНАЧЕНИЕ(Перечисление.ТипыСоответствий.HOBsKZU)
	                |
	                |ОБЪЕДИНИТЬ ВСЕ
	                |
	                |ВЫБРАТЬ РАЗЛИЧНЫЕ
...
/////////////////////////////////////////////////////////////////////////////////
//РАССЫЛКА НОТИФИКАЦИЙ КОЛЛЕКТОРАМ
Процедура НотификацияПоПросроченнымКомментариям() Экспорт
	
	CollectionAnalyst = rgsНастройкаКонфигурацииПовтИсп.ПолучитьЗначениеНастройки("CollectionAnalyst");
	
	СписокСтатусовDispute = Перечисления.InvoiceStatus.ПолучитьDisputeStatuses();
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	Collectors.Collector КАК Collector,
	               |	Collectors.Collector.ПользовательLDAP КАК CollectorПользовательLDAP
	               |ПОМЕСТИТЬ ВТ_ДанныеРегистраОстатков
	               |ИЗ
	               |	РегистрНакопления.BilledAR.Остатки(&ТекущаяДата, ) КАК BilledARОстатки
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Collectors КАК Collectors
	               |		ПО BilledARОстатки.Client = Collectors.Client
	               |			И (BilledARОстатки.AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.ManagementGeomarket = Collectors.ManagementGeomarket
	               |				ИЛИ Collectors.ManagementGeomarket = ЗНАЧЕНИЕ(Справочник.ManagementGeography.ПустаяСсылка))
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               ...
	               |	ВЫБОР
	               |		КОГДА InvoiceCommentsСрезПоследних.Problem.FactualCommentDate = ДАТАВРЕМЯ(1, 1, 1)
	               |			ТОГДА InvoiceCommentsСрезПоследних.Период
	               |		ИНАЧЕ InvoiceCommentsСрезПоследних.Problem.FactualCommentDate
	               |	КОНЕЦ КАК Период,
	               |	InvoiceCommentsСрезПоследних.Problem.CustomerInputDetails КАК Comment,
	               |	ВТ_ДанныеРегистраОстатков.Client КАК Client,
	               |	ВТ_ДанныеРегистраОстатков.Collector КАК Collector,
	               |	ЕСТЬNULL(ВТ_ДанныеРегистраОстатков.CollectorПользовательLDAP, &CollectionAnalyst) КАК ПользовательLDAP
	               |ИЗ
	               |	РегистрСведений.InvoiceComments.СрезПоследних КАК InvoiceCommentsСрезПоследних
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеРегистраОстатков КАК ВТ_ДанныеРегистраОстатков
	               |		ПО InvoiceCommentsСрезПоследних.Invoice = ВТ_ДанныеРегистраОстатков.Invoice
	               |ГДЕ
	               |	InvoiceCommentsСрезПоследних.Problem.Status В(&DisputeStatusesList)
	               |	И ВЫБОР
	               |			КОГДА InvoiceCommentsСрезПоследних.Problem.FactualCommentDate = ДАТАВРЕМЯ(1, 1, 1)
	               |				ТОГДА РАЗНОСТЬДАТ(InvoiceCommentsСрезПоследних.Период, &ТекущаяДата, ДЕНЬ) > 10
	               |			ИНАЧЕ РАЗНОСТЬДАТ(InvoiceCommentsСрезПоследних.Problem.FactualCommentDate, &ТекущаяДата, ДЕНЬ) > 10
	               |		КОНЕЦ";
	Запрос.УстановитьПараметр("ТекущаяДата", ТекущаяДата());
	Запрос.УстановитьПараметр("DisputeStatusesList", СписокСтатусовDispute);
	Запрос.УстановитьПараметр("CollectionAnalyst", CollectionAnalyst);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ДанныеInvoiceComments = РезультатЗапроса.Выгрузить();
	МассивПолучателей = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДанныеInvoiceComments.ВыгрузитьКолонку("ПользовательLDAP"));
	
	СтруктураОтбора = Новый Структура("ПользовательLDAP");
	...
	Для Каждого ПолучательLDAP из МассивПолучателей Цикл 
		...
		Если НЕ ЗначениеЗаполнено(ПолучательEmail) Тогда
			...
			СтрокаСообщенияОбОшибке = "Email not filled for LDAP user: " + ПолучательLDAP.DisplayName;
			ЗаписьЖурналаРегистрации("Overdue invoice invoice comments notification error", УровеньЖурналаРегистрации.Ошибка, , , СтрокаСообщенияОбОшибке);
			ОтменитьТранзакцию();
			
		КонецЕсли;
		
		СтруктураОтбора.ПользовательLDAP = ПолучательLDAP;
		Тема = "List of overdue invoice comments as of " + Формат(ТекущаяДата(), "Л=en_US; ДФ='MMMM yyyy'");
		ТелоHTML = "<HTML><HEAD>
		|<META content=""text/html; charset=utf-8"" http-equiv=Content-Type><LINK rel=stylesheet type=text/css href=""v8help://service_book/service_style""><BASE href=""v8config://d349bc7e-06e3-4fc1-b24f-9709087cc83c/mdobject/id44c3769f-050d-4f0a-ae52-7c2a9e753714/038b5c85-fb1c-4082-9c4c-e69f8928bf3a"">
		|<META name=GENERATOR content=""MSHTML 11.00.9600.18525""></HEAD>
		|<BODY>
		|<br>
		|<P style=""FONT-SIZE: 15px; FONT-FAMILY: Arial, sans-serif; WHITE-SPACE: normal; WORD-SPACING: 0px; TEXT-TRANSFORM: none; FONT-WEIGHT: normal; COLOR: rgb(0,0,0); FONT-STYLE: normal; ORPHANS: 2; WIDOWS: 2; LETTER-SPACING: normal; TEXT-INDENT: 0px; font-variant-ligatures: normal; font-variant-caps: normal; -webkit-text-stroke-width: 0px"">Please be kindly informed that there are some overdue invoice comments that have been assigned to you.</P>
		|<BR>
		|<P FONT-SIZE: 15px; FONT-FAMILY: Arial, sans-serif; WHITE-SPACE: WORD-SPACING: TEXT-TRANSFORM: none; FONT-WEIGHT: COLOR: rgb(0,0,0); FONT-STYLE: ORPHANS: WIDOWS: 2; TEXT-INDENT: 0px; font-variant-ligatures: font-variant-caps: normal; -webkit-text-stroke-width: 0px??><STRONG>Thank you for your assistance for resolving this issue in a timely manner.</STRONG></P>
		|<P FONT-SIZE: FONT-FAMILY: Arial, sans-serif; WHITE-SPACE: WORD-SPACING: TEXT-TRANSFORM: none; FONT-WEIGHT: COLOR: rgb(0,0,0); FONT-STYLE: ORPHANS: WIDOWS: 2; TEXT-INDENT: 0px; font-variant-ligatures: font-variant-caps: normal; -webkit-text-stroke-width: 0px?? 17px;><FONT size=4>ERM Team</FONT></P>
		|<BR>
		...
		ПолныеДанныеУведомления = ДанныеInvoiceComments.Скопировать(СтруктураОтбора);
		Attach = ПолучитьAttachДляInvoiceComments(ПолныеДанныеУведомления);
		ТелоHTML = СтрЗаменить(ТелоHTML, Символы.ПС, "");
...
Функция ПолучитьAttachДляInvoiceComments(Таблица)
	
	ТабДок = Новый ТабличныйДокумент;
	
	Если Таблица.Количество() > 0 Тогда
		
		Макет = ПолучитьОбщийМакет("AttachmentInvoiceComment");
		ОбластьМакетаШапка = Макет.ПолучитьОбласть("Шапка");
		ТабДок.Вывести(ОбластьМакетаШапка);
		...
		Для Каждого Элемент Из Таблица Цикл
			...
			ОбластьМакетаСтрока.Параметры.Invoice = Элемент.Invoice;
			ОбластьМакетаСтрока.Параметры.Date = Формат(Элемент.Период, "ДФ=MM/dd/yyyy");
			ОбластьМакетаСтрока.Параметры.Comment = Элемент.Comment;
			ОбластьМакетаСтрока.Параметры.Client = Элемент.Client;
...
Функция НайтиПрисоединенныеФайлыПоВладельцу(ВладелецФайла) Экспорт
	
	МассивПрисоединенныхФайлов = Новый Массив;
	
	ТипВладельца = ТипЗнч(ВладелецФайла);
	
	Если Метаданные.ОпределяемыеТипы.ВладелецПрисоединенныхФайлов.Тип.СодержитТип(ТипВладельца) Тогда
		
		ПолноеИмяВладельца = Метаданные.НайтиПоТипу(ТипВладельца).Имя;
		ИмяСправочника = Метаданные.Справочники.Найти(ПолноеИмяВладельца + "ПрисоединенныеФайлы").ПолноеИмя();
		
	Иначе
		
		Возврат МассивПрисоединенныхФайлов;
		
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	СправочникСПрисоединеннымиФайлами.Ссылка КАК СсылкаНаФайл
	               |ИЗ
	               |	ИмяСправочника КАК СправочникСПрисоединеннымиФайлами
	               |ГДЕ
	               |	СправочникСПрисоединеннымиФайлами.ВладелецФайла = &ВладелецФайла
	               |	И НЕ СправочникСПрисоединеннымиФайлами.ПометкаУдаления";
	//Запрос.УстановитьПараметр("ИмяСправочника", Справочники[ПолноеИмяВладельца + "ПрисоединенныеФайлы"]);
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		МассивПрисоединенныхФайлов.Добавить(ВыборкаДетальныеЗаписи.СсылкаНаФайл);
...
Функция СоздатьПрисоединенныйФайлЗагрузки(ИмяФайла, СсылкаНаВладельца, АдресФайлаВХранилище, ТипЗагрузки) Экспорт
		
		ДлинаИмениФайла = СтрДлина(ИмяФайла);
		ПозицияТочки = СтрНайти(ИмяФайла, ".", НаправлениеПоиска.СКонца);
		ОшибкаВПроцессеСоздания = Ложь;
		
		Если ПозицияТочки = 0 Тогда
			
			СтрокаСообщенияОбОшибке = "Incorrect file format";
			ЗаписьЖурналаРегистрации(ТипЗагрузки + " loading error", УровеньЖурналаРегистрации.Ошибка, , , СтрокаСообщенияОбОшибке);
			ОшибкаВПроцессеСоздания = Истина;
			
		Иначе
			
			РасширениеФайлаБезТочки = Прав(ИмяФайла, ДлинаИмениФайла - ПозицияТочки);
			
			ДлинаРасширенияФайлаСТочкой = СтрДлина(РасширениеФайлаБезТочки) + 1;
			ИмяФайлаБезРасширения = Лев(ИмяФайла, ДлинаИмениФайла - ДлинаРасширенияФайлаСТочкой);
			
			МассивПрисоединенныхФайлов = РГСофт.НайтиПрисоединенныеФайлыПоВладельцу(СсылкаНаВладельца);
			
			Если МассивПрисоединенныхФайлов.Количество() > 0 Тогда
				
				НоваяСтруктураФайла = Новый Структура("АдресФайлаВоВременномХранилище, АдресВременногоХранилищаТекста, ИмяБезРасширения, Расширение",
														АдресФайлаВХранилище, "", Сред(ИмяФайла, 1, ДлинаИмениФайла - ДлинаРасширенияФайлаСТочкой), РасширениеФайлаБезТочки);
					
				РаботаСФайлами.ОбновитьФайл(МассивПрисоединенныхФайлов[0], НоваяСтруктураФайла);
		...
		Возврат ОшибкаВПроцессеСоздания;
	
КонецФункции

// { RGS AGorlenko1 04.03.2020 14:36:34 - S-E-0001469-Процедура для добавления записей в регистр, в котором хранятся даты для отчета Collection Target
Процедура ЗаписатьДанныеВРегистрДатДляCollection(Период, Invoice, СтруктураЗначенийДат) Экспорт
	
	InvoiceCollectionDates = РегистрыСведений.InvoiceCollectionDates;
	ПоследняяЗаписьCollectionDates = InvoiceCollectionDates.СрезПоследних(Период, Новый Структура("Invoice", Invoice));
	
	Если ПоследняяЗаписьCollectionDates.Количество() = 0 
		ИЛИ ПоследняяЗаписьCollectionDates[0].TriggerDate <> СтруктураЗначенийДат.TriggerDate
		ИЛИ ПоследняяЗаписьCollectionDates[0].DateFrom <> СтруктураЗначенийДат.DateFrom
		ИЛИ ПоследняяЗаписьCollectionDates[0].DateTo <> СтруктураЗначенийДат.DateTo Тогда
		
		НЗ = РегистрыСведений.InvoiceCollectionDates.СоздатьНаборЗаписей();
		
		НЗ.Отбор.Invoice.Установить(Invoice);
		НЗ.Отбор.Период.Установить(Период);
		
		Запись = НЗ.Добавить();
		Запись.Период = Период;
		Запись.Invoice = Invoice;
		ЗаполнитьЗначенияСвойств(Запись, СтруктураЗначенийДат);
...
КонецПроцедуры
// } RGS AGorlenko1 04.03.2020 14:38:01 - S-E-0001469-Процедура для добавления записей в регистр, в котором хранятся даты для отчета Collection Target
Функция СоздатьRegEx(Multiline = Истина, Global = Истина, IgnoreCase = Истина)  Экспорт
...
КонецФункции // { RGS Agorlenko1 16.03.2020 23:47:50 - создание файла для обработки регулярных выражений
// } RGS AGorlenko1 04.03.2020 14:38:01 - S-E-0001469-Процедура для добавления записей в регистр, в котором хранятся даты для отчета Collection Target
