...
Функция ПолучитьДанныеПоSalesOrdersДляUnbilledNotification(Период)
	...
	Запрос.Текст =  "ВЫБРАТЬ РАЗЛИЧНЫЕ
	                ...
	                |			И ОчередьУведомлений.Уровень = ПолучателиУведомленийUnbilled.Уровень
	                |			И ОчередьУведомлений.Проблема.SalesOrder.AU.ПодразделениеОрганизации = ПолучателиУведомленийUnbilled.Идентификатор1
...
Функция ПолучитьДанныеДляAttachUnbilledNotification(МассивОрдеров)
	...
	|	КОНЕЦ КАК Ageing
	|ИЗ
	|	РегистрНакопления.UnbilledAR.Остатки(&Дата, SalesOrder В (&МассивОрдеров) {(AU.ПодразделениеОрганизации.БазовыйЭлемент.GeoMarket.Родитель = &GeoMarket) КАК Поле2}) КАК UnbilledARОстатки
