...
Процедура НотификацияПоПросроченнымКомментариям() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |ГДЕ
	               |	ВЫБОР
	               |			КОГДА ЕСТЬNULL(ВнутренниеКурсыВалютСрезПоследних.Курс, 0) = 0
	               |				ТОГДА BilledARОстатки.AmountОстаток > 1
	               |			ИНАЧЕ BilledARОстатки.AmountОстаток / ВнутренниеКурсыВалютСрезПоследних.Курс > 1
	               |		КОНЕЦ
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               ...
	               |	InvoiceCommentsСрезПоследних.Problem КАК Problem,
	               |	ВЫБОР
	               ...
	               |	КОНЕЦ КАК Период,
	               |	InvoiceCommentsСрезПоследних.Problem.User КАК ProblemUser,
	               |	InvoiceCommentsСрезПоследних.Problem.User.ПользовательLDAP КАК ПользовательLDAP,
	               |	InvoiceCommentsСрезПоследних.Problem.CustomerInputDetails КАК ProblemComment,
	               |	InvoiceCommentsСрезПоследних.Invoice.Client КАК Client
	               |ИЗ
...
Функция ПолучитьAttachДляInvoiceComments(Таблица)
	...
	Если Таблица.Количество() > 0 Тогда
		...
		Для Каждого Элемент Из Таблица Цикл
			...
			ОбластьМакетаСтрока.Параметры.Comment = Элемент.ProblemComment;
			ОбластьМакетаСтрока.Параметры.Client = Элемент.Client;
