...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		// Устанавливаем блокировку на УзелИнформационнойБазы получателя.
		Попытка
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
				...
				Если ОтправительИзСообщения = Неопределено
					...
					СтрокаСообщения = НСтр("ru = 'Не найден УзелИнформационнойБазы обмена для загрузки данных. План обмена: %1, Идентификатор: %2'");
				...
				Если КомпонентыОбмена.ИспользоватьКвитирование Тогда
					...
					// Добавляем публичные идентификаторы для ссылочных объектов, о получении которых отчитался УзелИнформационнойБазы-корреспондент.
					ДобавитьВыгруженныеОбъектыВРегистрПубличныеИдентификаторы(КомпонентыОбмена);
					
					// Удаляем регистрацию изменений, о получении которых отчитался УзелИнформационнойБазы-корреспондент.
...
Процедура ВыполнитьПроведениеДокументаПриЗагрузке(
	...
	// Устанавливаем УзелИнформационнойБазы-отправитель для предотвращения регистрации объекта на узле, для которого производим загрузку
...
//  Отправитель - ПланОбменаСсылка - Ссылка на УзелИнформационнойБазы плана обмена, который является отправителем данных.
//
// Возвращаемое значение:
...
//  УзелОбмена     - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, соответствующий корреспонденту.
//
// Возвращаемое значение:
...
//
// Параметры:
//  УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена XDTO соответствующего корреспондента.
//  Режим          - Строка - Вид запрашиваемой информации: "Отправка" | "Получение" | "ОтправкаПолучение".
//                            "Отправка" - будут возвращены все объекты, для которых поддерживается отправка;
//                            "Получение" - будут возвращены все объекты, для которых поддерживается получение;
//                            "ОтправкаПолучение" - будут возвращены все поддерживаемые объекты.
...
//  УзелИнформационнойБазы - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого необходимо получить менеджер обмена.
...
//
// Параметры:
//  Получатель - ссылка на УзелИнформационнойБазы плана обмена, в который производится выгрузка данных.
//
...
// Параметры:
//  УзелИнформационнойБазы - ссылка на УзелИнформационнойБазы-корреспондент.
//
...
Процедура ДобавитьВыгруженныеОбъектыВРегистрПубличныеИдентификаторы(КомпонентыОбмена)
	...
	|ПО ПубличныеИдентификаторы.УзелИнформационнойБазы = &УзелИнформационнойБазы И ПубличныеИдентификаторы.Ссылка = ТаблицаИзменений.Ссылка
	|ГДЕ ТаблицаИзменений.УзелИнформационнойБазы = &УзелИнформационнойБазы И ТаблицаИзменений.НомерСообщения <= &НомерСообщения
	...
	Для Каждого ЭлементСостава Из СоставПланаОбмена Цикл
		...
		Запрос.УстановитьПараметр("УзелИнформационнойБазы", УзелДляОбмена);
