		...
		Иначе
			...
			Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
				...
				Если ТекущееПКО.Значение Тогда
					...
					СтрокаТаблицы.ЭтоКлассификатор                    = ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска";
...
Функция СсылкаПоУИД(ТипЗначенияОбъектаИБ, УИДОбъектаXDTO, УзелОбмена)
	...
	Если НЕ РегистрыСведений.ПубличныеИдентификаторыСинхронизируемыхОбъектов.ЗаписьЕстьВРегистре(СтруктураЗаписи) Тогда
		Возврат НайденнаяСсылка;
	КонецЕсли;
	// Данному УИД уже сопоставлен другой объект. Нужно создавать ссылку с другим УИД.
...
Процедура ЗаписатьПриНеобходимостиПубличныйИдентификатор(
	...
	//RGS AArsentev 17.08.2016
	Структура = Новый Структура;
	Структура.Вставить("УзелИнформационнойБазы", УзелОбмена);
	Структура.Вставить("Ссылка", ?(ДанныеДляЗаписиВИБ = Неопределено, ПолученныеДанныеСсылка, ДанныеДляЗаписиВИБ.Ссылка));
	Если ДанныеДляЗаписиВИБ <> Неопределено
		И РегистрыСведений.ОбъектыОбмена.ЗаписьЕстьВРегистре(Структура) Тогда		
	Иначе
	РегистрыСведений.ОбъектыОбмена.ДобавитьЗапись(Структура, Истина);	
	КонецЕсли;
	//RGS AArsentev 17.08.2016
