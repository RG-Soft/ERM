...
// и записывает результат в регистр сведений УдалитьЗамерыВремени2.
//
// Параметры:
...
// Завершает замер времени ключевой операции
// и записывает результат в регистр сУдалитьЗамерыВремениТехнологическиегические.
//
// Параметры:
// 	КлючеваяОперация	- СправочникСсылка.КлючевыеОперации, Строка - ключевая операция.
//	ВремяНачала			- Число										- универсальная дата в миллисекундах,
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(Параметры)
	...
	Если НЕ Технологический Тогда
		Запись = РегистрыСведений.УдалитьЗамерыВремени2.СоздатьМенеджерЗаписи();
	Иначе
		Запись = РегистрыСведений.УдалитьЗамерыВремениТехнологические.СоздатьМенеджерЗаписи();
	КонецЕсли;
...
Процедура ЗаписатьЗамерыВремени(МассивЗамеров)
	...
	НаборЗаписей = РегистрыСведений.УдалитьЗамерыВремени2.СоздатьНаборЗаписей();
...
Процедура ЭкспортОценкиПроизводительности(КаталогиЭкспорта, ДопПараметры = Неопределено) Экспорт
	...
	|ИЗ 
	|	РегистрСведений.УдалитьЗамерыВремени2 КАК Замеры
	|ГДЕ
...
Процедура ОчиститьРегистрыЗамерыВремени() Экспорт
	...
	|ВЫБРАТЬ
	|	МИНИМУМ(ДатаЗаписиНачалоЧаса) КАК ДатаЗапУдалитьЗамерыВремениТехнологическиестрСведений.ЗамерыВремениТехнологические
	|ГДЕ
	|	ДатаЗаписиНачалоЧаса < &ГраницаУдаления
	|";
	...
	НаборЗаписей = РегистрыСведений.УдалитьЗамерыВремени2.СоздатьНаборЗаписей();
	НаборЗаписейТехнологические = РегистрыСведений.УдалитьЗамерыВремениТехнологические.СоздатьНаборЗаписей();
