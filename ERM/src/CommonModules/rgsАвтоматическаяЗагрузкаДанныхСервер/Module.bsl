...
Функция ПолучитьШаблонИмениФайлаСообщения(Источник, ТипЗагрузки)
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.Lawson Тогда
		//Возврат "*XR222*";
		//Возврат "DnA_FP20_Data_XR222BSN_RCA_" + Строка(Формат(ТекущаяДата(), "ДФ=гггг;")) + Строка(Формат(ТекущаяДата(), "ДФ=ММ;")) + "*.xlsx";	
...
Функция ПолучитьШаблонИмениФайлаДанных(Источник, ТипЗагрузки)
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.OracleSmith Тогда
		Возврат "*.txt";
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.Lawson Тогда
		//Возврат ?(ТипЗагрузки = "LawsonARBalance","DnA_FP20_Data_XR222BSN_RCA_", "DnA_FP20_Data_XR222PnL_Rev_RCA_") + Строка(Формат(ДобавитьМесяц(ТекущаяДата(), -1), "ДФ=гггг;")) + Строка(Формат(ДобавитьМесяц(ТекущаяДата(), -1), "ДФ=ММ;")) + "*.xlsx";
...
Функция ПолучитьИмяПапкиВнутриКаталогаОбменаДляLawson(ТипЗагрузки)
	...
	//В случае, если загрузка AR Balance, то текущий месяц загружается начиная с 4 рабочего дня текущего месяца по 3 день следующего, если загрузка Lawson Revenue - то с 8 рабочего дня текущего месяца по 7 день следующего
	Возврат ?(НачалоДня(ТекущаяДата()) >= НачалоДня(КалендарныеГрафики.ПолучитьДатуПоКалендарю(Календарь, НачалоМесяца(ТекущаяДата()), ?(ТипЗагрузки = "LawsonARBalance", 4, 8))),
...
Процедура ЗагрузитьФайлДанныхLawson(ОбрабатываемыйФайл, СтруктураНастроек)
	...
	Иначе
		...
		СформироватьЗадачуОтветственным(СтруктураНастроек.Источник, "Lawson loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, ДокументЗагрузки.Ссылка);
