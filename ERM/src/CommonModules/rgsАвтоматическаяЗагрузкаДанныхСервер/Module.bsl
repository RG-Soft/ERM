...
Процедура ВыполнитьЗагрузкуДанныхLawson120() Экспорт
	
	СтруктураНастроек = Новый Структура();
	ИнициализироватьСтруктуруНастроек(СтруктураНастроек, Перечисления.ТипыСоответствий.Lawson, "LawsonARBalance");
	
	ПолучениеВыполнено = ВыполнитьПолучениеСообщения(СтруктураНастроек);
	
	Если Не ПолучениеВыполнено Тогда
		
		СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.Lawson, "Lawson AR balance loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
		
		РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.Lawson),
			"ERM: Lawson AR balance loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
			
		ЗаписьЖурналаРегистрации("Lawson AR balance loading error", УровеньЖурналаРегистрации.Ошибка, , , СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
...
Процедура ВыполнитьЗагрузкуДанныхLawson400() Экспорт
	
	СтруктураНастроек = Новый Структура();
	ИнициализироватьСтруктуруНастроек(СтруктураНастроек, Перечисления.ТипыСоответствий.Lawson, "LawsonRevenue");
	
	ПолучениеВыполнено = ВыполнитьПолучениеСообщения(СтруктураНастроек);
	
	Если Не ПолучениеВыполнено Тогда
		
		СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.Lawson, "Lawson revenue loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
		
		РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.Lawson),
			"ERM: Lawson revenue loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
			
		ЗаписьЖурналаРегистрации("Lawson revenue loading error", УровеньЖурналаРегистрации.Ошибка, , , СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
...
Процедура ВыполнитьЗагрузкуДанныхHOB_DIR() Экспорт
	
	СтруктураНастроек = Новый Структура();
	ИнициализироватьСтруктуруНастроек(СтруктураНастроек, Перечисления.ТипыСоответствий.HOBs, "DIR");
	
	ПолучениеВыполнено = ВыполнитьПолучениеСообщения(СтруктураНастроек);
	
	Если Не ПолучениеВыполнено Тогда
		
		СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.HOBs, "HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
		
		РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.HOBs),
			"ERM: HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
			
		ЗаписьЖурналаРегистрации("HOB DIR loading error", УровеньЖурналаРегистрации.Ошибка, , , СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
...
Процедура ИнициализироватьСтруктуруНастроек(СтруктураНастроек, Источник, ТипЗагрузки = Неопределено)
	...
	СтруктураНастроек.Вставить("ИмяКаталогаОбменаИнформацией", ИмяКаталогаОбменаИнформацией(Источник, ТипЗагрузки));
	СтруктураНастроек.Вставить("ШаблонИмениФайлаСообщения", ПолучитьШаблонИмениФайлаСообщения(Источник, ТипЗагрузки));
	СтруктураНастроек.Вставить("ШаблонИмениФайлаДанных", ПолучитьШаблонИмениФайлаДанных(Источник, ТипЗагрузки));
...
Функция ПолучитьШаблонИмениФайлаСообщения(Источник, ТипЗагрузки)
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.Lawson Тогда
		Возврат ?(ТипЗагрузки = "LawsonARBalance", "*XR222BSN_RCA*", "*XR222PnL_Rev_RCA*");
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.HOBs И ТипЗагрузки = "JV" Тогда
		Возврат "HOB_JV*";
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.HOBs И ТипЗагрузки = "DIR" Тогда
		Возврат "HOB_DIR*";
...
Функция ПолучитьШаблонИмениФайлаДанных(Источник, ТипЗагрузки)
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.OracleSmith Тогда
		Возврат "*.txt";
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.Lawson Тогда
		Возврат ?(ТипЗагрузки = "LawsonARBalance","DnA_FP20_Data_XR222BSN_RCA_", "DnA_FP20_Data_XR222PnL_Rev_RCA_") + ПолучитьИмяПапкиВнутриКаталогаОбменаДляLawson(ТипЗагрузки) + "*.xlsx";
...
Функция ВыполнитьПолучениеСообщения(СтруктураНастроек)
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР = ОписаниеОшибки();
		Результат = Ложь;
	КонецПопытки;
...
Функция ПолучитьFTPСоединение()
	...
	// { RGS TAlmazova 06.07.2018 12:00:47 - обновление БСП
	...
	ПараметрыFTP = РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспорта(ПланыОбмена.rgsОбменДокументамиERM.НайтиПоКоду("HB"));
	// } RGS TAlmazova 06.07.2018 12:00:56 - обновление БСП
...
Функция ПолучитьСообщение(СтруктураНастроек)
	...
	Иначе
		...
		Если СтруктураНастроек.Источник = Перечисления.ТипыСоответствий.HOBs Тогда
...
Функция ИмяКаталогаОбменаИнформацией(Источник, ТипЗагрузки)
	...
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.Lawson Тогда
		Возврат Константы.КаталогОбменаLawson.Получить() + ПолучитьИмяПапкиВнутриКаталогаОбменаДляLawson(ТипЗагрузки);
	ИначеЕсли Источник = Перечисления.ТипыСоответствий.HOBs Тогда
...
//Получение названия папки с файлом данных для обмена формата ГГГГММ (также входит в название самого файла обмена)
Функция ПолучитьИмяПапкиВнутриКаталогаОбменаДляLawson(ТипЗагрузки)
	
	Календарь = КалендарныеГрафики.ПроизводственныйКалендарьРоссийскойФедерации();
	//В случае, если загрузка AR Balance, то текущий месяц загружается начиная с 8 рабочего дня текущего месяца по 3 день следующего, если загрузка Lawson Revenue - то с 4 рабочего дня текущего месяца по 7 день следующего
	Возврат ?(НачалоДня(ТекущаяДата()) >= НачалоДня(КалендарныеГрафики.ПолучитьДатуПоКалендарю(Календарь, НачалоМесяца(ТекущаяДата()), ?(ТипЗагрузки = "LawsonARBalance", 8, 4))),
						Строка(Формат(ТекущаяДата(), "ДФ=гггг;")) + Строка(Формат(ТекущаяДата(), "ДФ=ММ;")),
						Строка(Формат(ДобавитьМесяц(НачалоМесяца(ТекущаяДата()), -1), "ДФ=гггг;")) + Строка(Формат(ДобавитьМесяц(НачалоМесяца(ТекущаяДата()), -1), "ДФ=ММ;")));
...
Процедура ЗагрузитьПолученныеДанные(СтруктураНастроек)
	...
	Для каждого НайденныйФайл Из МассивНайденныхФайлов Цикл
		...
		Если Врег(НайденныйФайл.Расширение) <> Врег(СтруктураНастроек.РасширениеФайлаДанных) Тогда
			...
			ИначеЕсли СтруктураНастроек.Источник = Перечисления.ТипыСоответствий.HOBs И СтруктураНастроек.ТипЗагрузки = "DIR" Тогда
				
				СформироватьЗадачуОтветственным(СтруктураНастроек.Источник, "HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
...
Процедура ЗагрузитьФайлДанных(ОбрабатываемыйФайл, СтруктураНастроек)
	...
	ИначеЕсли СтруктураНастроек.Источник = Перечисления.ТипыСоответствий.HOBs И СтруктураНастроек.ТипЗагрузки = "DIR" Тогда
		ЗагрузитьФайлДанныхHOB_DIR(ОбрабатываемыйФайл, СтруктураНастроек);
	ИначеЕсли СтруктураНастроек.Источник = "Currency" Тогда
...
Процедура ЗагрузитьФайлДанныхOracleMI(ОбрабатываемыйФайл, СтруктураНастроек)
	...
	ДокументЗагрузки.ДополнительныеСвойства.Вставить("АдресФайлаВХранилище", АдресФайлаВХранилище);
...
Процедура ЗагрузитьФайлДанныхLawson(ОбрабатываемыйФайл, СтруктураНастроек)
	...
	Иначе
		...
		СформироватьЗадачуОтветственным(СтруктураНастроек.Источник, ?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance loading error", "ERM: Lawson revenue loading error"), СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, ДокументЗагрузки.Ссылка);
		
		РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.Lawson),
			?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance loading error", "ERM: Lawson revenue loading error"), СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
			
		ЗаписьЖурналаРегистрации(?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance loading error", "ERM: Lawson revenue loading error"), УровеньЖурналаРегистрации.Ошибка, , , СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР);
...
Процедура ЗагрузитьФайлДанныхHOB_DIR(ОбрабатываемыйФайл, СтруктураНастроек)
	
	ДокументЗагрузки = Документы.ЗагрузкаДанныхДатыDIRИзГИС.СоздатьДокумент();
	ДокументЗагрузки.Дата = ТекущаяДата();
	ДокументЗагрузки.ИмяФайла = ОбрабатываемыйФайл.Имя;
	//ДокументЗагрузки.ТипТранзакций = Перечисления.HOBTransactionType.JV;
	ДокументЗагрузки.Комментарий = ОбрабатываемыйФайл.Имя;
...
Процедура ЗагрузитьФайлДанныхCRMClients(ОбрабатываемыйФайл, СтруктураНастроек)
	...
	АдресФайлаВХранилище = Новый УникальныйИдентификатор;
	ПоместитьВоВременноеХранилище(ДвоичныеДанные, АдресФайлаВХранилище);
	ДокументЗагрузки.ДополнительныеСвойства.Вставить("АдресФайлаВХранилище", АдресФайлаВХранилище);
	//ДокументЗагрузки.ИсточникДанных = Новый ХранилищеЗначения(ДвоичныеДанные, Новый СжатиеДанных(9));
...
Процедура ОбработатьДокументЗагрузки(ДокументЗагрузки, СтруктураНастроек)
	...
	ИначеЕсли СтруктураНастроек.Источник = Перечисления.ТипыСоответствий.HOBs И СтруктураНастроек.ТипЗагрузки = "DIR" Тогда
		ОбработатьДокументЗагрузкиHOB_DIR(ДокументЗагрузки, СтруктураНастроек);
	ИначеЕсли СтруктураНастроек.Источник = "CRMClients" Тогда
...
Процедура ОбработатьДокументЗагрузкиOracleMI(ДокументЗагрузки, СтруктураНастроек)
	...
	МассивПрисоединенныхФайлов = РГСофт.НайтиПрисоединенныеФайлыПоВладельцу(ДокументЗагрузки.Ссылка);
	...
	СтруктураПараметров.Вставить("СоответствиеРесурсТип", rgsЗагрузкаИзExcel.СформироватьСоответствиеРесурсовИТиповРегистра("OracleSourceData", Ложь));
...
Процедура ОбработатьДокументЗагрузкиLawson(ДокументЗагрузки, СтруктураНастроек)
	...
	Попытка
		...
		Если РезультатЭтапа.Свойство("ОшибкаЗаполнения") Тогда
			...
			СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.Lawson, ?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance loading error", "ERM: Lawson revenue loading error"), СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, ДокументЗагрузки.Ссылка);
			Попытка
				РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.Lawson),
					?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance loading error", "ERM: Lawson revenue loading error"), СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
				СтруктураЗаписиЛогов.Вставить("СформированоУведомлениеОбОшибке", Истина);
	...
	Попытка
		...
		Иначе
			...
			РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.Lawson),
				?(СтруктураНастроек.ТипЗагрузки = "LawsonARBalance", "ERM: Lawson AR balance successful loading", "ERM: Lawson revenue successful loading"), ТекстСообщения, , ТипТекстаПочтовогоСообщения.ПростойТекст);
...
Процедура ОбработатьДокументЗагрузкиHOB_DIR(ДокументЗагрузки, СтруктураНастроек)
	
	МассивПрисоединенныхФайлов = РГСофт.НайтиПрисоединенныеФайлыПоВладельцу(ДокументЗагрузки.Ссылка);
	
	СтруктураПараметров = Новый Структура("Ссылка, Дата, ИсточникДанных, ИменаКолонокВПервойСтроке, ПерваяСтрокаДанных, ПоследняяСтрокаДанных");
	ЗаполнитьЗначенияСвойств(СтруктураПараметров, ДокументЗагрузки);
	СтруктураПараметров.Вставить("СтруктураКолонок", Документы.ЗагрузкаДанныхДатыDIRИзГИС.ПолучитьСтруктуруКолонокТаблицыДанных());
	//СтруктураПараметров.Вставить("ИсточникДанных", ДокументЗагрузки.ИсточникДанных);
	СтруктураПараметров.Вставить("ИсточникДанных", МассивПрисоединенныхФайлов[0]);
	СтруктураПараметров.Вставить("ИменаКолонокВПервойСтроке", Истина);
	СтруктураПараметров.Вставить("ПерваяСтрокаДанных", 2);
	...
	Попытка
		...
		Если РезультатЭтапа.Свойство("ОшибкаЗаполнения") Тогда
			...
			СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.HOBs, "HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, ДокументЗагрузки.Ссылка);
			Попытка
				РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.HOBs),
					"ERM: HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
				СтруктураЗаписиЛогов.Вставить("СформированоУведомлениеОбОшибке", Истина);
	...
	Попытка
		...
		Если РезультатЭтапа.Свойство("ОшибкаЗаполнения") Тогда
			...
			СформироватьЗадачуОтветственным(Перечисления.ТипыСоответствий.OracleMI, "Oracle HOB DIR loading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, ДокументЗагрузки.Ссылка);
			Попытка
				РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.HOBs),
					"ERM:  HOB DIR oading error", СтруктураНастроек.СтрокаСообщенияОбОшибкеЖР, , ТипТекстаПочтовогоСообщения.ПростойТекст);
				СтруктураЗаписиЛогов.Вставить("СформированоУведомлениеОбОшибке", Истина);
			Исключение
			КонецПопытки;
		...
		Иначе
			...
			Если ДокументЗагрузкиОбъект.Модифицированность() Тогда
				//RG-Soft Соболев В.И. 05.09.2018
				ДокументЗагрузкиОбъект.СтатусЗагрузки = Перечисления.СтатусыЗагрузки.UpdatedDocuments;
				//RG-Soft Соболев В.И. 05.09.2018
				ДокументЗагрузкиОбъект.Записать();
			КонецЕсли;
			...
			РГСофт.ЗарегистрироватьПочтовоеСообщение(АдресаПолучателейСообщенийОбОшибках(Перечисления.ТипыСоответствий.HOBs),
				"ERM: HOB DIR successful loading", ТекстСообщения, , ТипТекстаПочтовогоСообщения.ПростойТекст);
...
Процедура ДействияПриОкончанииЗагрузки(СтруктураНастроек)
	...
	Иначе
		Если Не (ПустаяСтрока(СтруктураНастроек.ИмяПолученногоФайла) Или СтруктураНастроек.Источник = Перечисления.ТипыСоответствий.Lawson) Тогда
