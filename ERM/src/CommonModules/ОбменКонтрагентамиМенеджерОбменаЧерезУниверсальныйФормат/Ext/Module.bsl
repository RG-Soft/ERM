// Конвертация ERM от 07.06.2016 10:44:53
...
Процедура ЗаполнитьПравилаКонвертацииОбъектов(НаправлениеОбмена, ПравилаКонвертации) Экспорт
	Если НаправлениеОбмена = "Отправка" Тогда
		...
		ДобавитьПКО_Справочник_ЗаявкаНаИзменениеКлиентаПФ_Отправка(ПравилаКонвертации);
	...
	ИначеЕсли НаправлениеОбмена = "Получение" Тогда
		ДобавитьПКО_Справочник_ЗаявкаНаИзменениеКлиента_Получение(ПравилаКонвертации);
...
КонецПроцедуры
#КонецОбласти
#Область Справочник_ЗаявкаНаИзменениеКлиентаПФ_Отправка
Процедура ДобавитьПКО_Справочник_ЗаявкаНаИзменениеКлиентаПФ_Отправка(ПравилаКонвертации)

	ПравилоКонвертации = ОбменДаннымиXDTOСервер.ИнициализироватьПравилоКонвертацииОбъекта(ПравилаКонвертации);
	ПравилоКонвертации.ИмяПКО = "Справочник_ЗаявкаНаИзменениеКлиентаПФ_Отправка";
	ПравилоКонвертации.ОбъектДанных = Метаданные.Справочники.ЗаявкаНаИзменениеКлиентаПрисоединенныеФайлы;
	ПравилоКонвертации.ОбъектФормата = "Справочник.ЗаявкаНаИзменениеКлиентаПрисоединенныеФайлы";
	ПравилоКонвертации.ПравилоДляГруппыСправочника = Ложь;
	ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору";
	...
	НоваяСтрока = ПравилоКонвертации.Свойства.Добавить();
	НоваяСтрока.СвойствоКонфигурации = "ФайлХранилище";
	НоваяСтрока.СвойствоФормата = "ФайлХранилище";
...
Процедура ПКО_Справочник_Контрагенты_Отправка_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	|ИЗ
	|	РегистрСведений.ИерархияКонтрагентов.СрезПоследних(, Контрагент = &Контрагент) КАК ИерархияКонтрагентовСрезПоследних";
	...
	Иначе
		...
		Выборка.Следующий();
		ДанныеXDTO.Вставить("ГоловнойКонтрагент", Выборка.ParentClient);
	КонецЕсли;
	...
	ВыгрузитьLawsonID(ДанныеИБ, ДанныеXDTO);
...
#КонецОбласти
#Область Получение
#Область Справочник_ЗаявкаНаИзменениеКлиента_Получение
Процедура ДобавитьПКО_Справочник_ЗаявкаНаИзменениеКлиента_Получение(ПравилаКонвертации)

	ПравилоКонвертации = ОбменДаннымиXDTOСервер.ИнициализироватьПравилоКонвертацииОбъекта(ПравилаКонвертации);
	ПравилоКонвертации.ИмяПКО = "Справочник_ЗаявкаНаИзменениеКлиента_Получение";
	ПравилоКонвертации.ОбъектДанных = Метаданные.Справочники.ЗаявкаНаИзменениеКлиента;
	ПравилоКонвертации.ОбъектФормата = "Справочник.ЗаявкаНаИзменениеКлиента";
	ПравилоКонвертации.ПравилоДляГруппыСправочника = Ложь;
	ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору";
...
Процедура ДобавитьПКО_Справочник_Контрагенты_Получение(ПравилаКонвертации)
	...
	ПравилоКонвертации.ПоляПоиска.Добавить("CRMID,ИНН,КПП,НаименованиеПолное,ЮридическоеФизическоеЛицо");
КонецПроцедуры
#КонецОбласти
...
Процедура ЗаполнитьПравилаКонвертацииПредопределенныхДанных(НаправлениеОбмена, ПравилаКонвертации) Экспорт
	...
	ПравилоКонвертации.КонвертацииЗначенийПриОтправке = ЗначенияДляОтправки;
	ЗначенияДляПолучения = Новый Соответствие;
	ЗначенияДляПолучения.Вставить("ИзменениеРеквизитовОбъекта", Перечисления.ВидыОперацийИзмененияНСИ.ИзменениеРеквизитовОбъекта);
	ЗначенияДляПолучения.Вставить("РегистрацияНовогоОбъекта", Перечисления.ВидыОперацийИзмененияНСИ.РегистрацияНовогоОбъекта);
	ЗначенияДляПолучения.Вставить("УдалениеДубликатаОбъекта", Перечисления.ВидыОперацийИзмененияНСИ.УдалениеДубликатаОбъекта);
	ЗначенияДляПолучения.Вставить("УдалениеОбъекта", Перечисления.ВидыОперацийИзмененияНСИ.УдалениеОбъекта);
	ПравилоКонвертации.КонвертацииЗначенийПриПолучении = ЗначенияДляПолучения;
	...
	ПравилоКонвертации.КонвертацииЗначенийПриОтправке = ЗначенияДляОтправки;
	ЗначенияДляПолучения = Новый Соответствие;
	ЗначенияДляПолучения.Вставить("ВРаботе", Перечисления.СостояниеЗаявкиНаИзменениеНСИ.ВРаботе);
	ЗначенияДляПолучения.Вставить("Выполнена", Перечисления.СостояниеЗаявкиНаИзменениеНСИ.Выполнена);
	ЗначенияДляПолучения.Вставить("Новая", Перечисления.СостояниеЗаявкиНаИзменениеНСИ.Новая);
	ЗначенияДляПолучения.Вставить("Отклонена", Перечисления.СостояниеЗаявкиНаИзменениеНСИ.Отклонена);
	ПравилоКонвертации.КонвертацииЗначенийПриПолучении = ЗначенияДляПолучения;
...
Процедура ВыгрузитьLawsonID(ДанныеИБ, ДанныеXDTO)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ втЗаписиИзИстории
		|ИЗ
		|	РегистрСведений.НастройкаСинхронизацииОбъектовСВнешнимиСистемами КАК НастройкаСинхронизацииОбъектовСВнешнимиСистемами
		|ГДЕ
		|	НастройкаСинхронизацииОбъектовСВнешнимиСистемами.ОбъектПриемника = &Ссылка
		|	И НастройкаСинхронизацииОбъектовСВнешнимиСистемами.ТипСоответствия = &Lawson
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		|	НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних.Период,
		...
		|			,
		|			(Идентификатор, ТипСоответствия, ТипОбъектаВнешнейСистемы) В
		|				(ВЫБРАТЬ
		|					втЗаписиИзИстории.Идентификатор,
		|					втЗаписиИзИстории.ТипСоответствия,
		|					втЗаписиИзИстории.ТипОбъектаВнешнейСистемы
		|				ИЗ
		|					втЗаписиИзИстории КАК втЗаписиИзИстории)) КАК НастройкаСинхронизацииОбъектовСВнешнимиСистемамиСрезПоследних";
	
	Запрос.УстановитьПараметр("Lawson", Перечисления.ТипыСоответствий.Lawson);
	...
	Если Выборка.Следующий() Тогда
		LawsonID = Выборка.Идентификатор;
	Иначе
		LawsonID = "";
	КонецЕсли;
	
	ДанныеXDTO.Вставить("LawsonID", LawsonID);
КонецПроцедуры
