...
Процедура ВыгрузитьЭксельВТаблицуДанныхПоИменамКолонок(ПутьКФайлу, ТаблицаДанных, ДанныеДляЗаполнения, АдресХранилища, СтруктураПараметров) Экспорт
	...
	ТекНомерСтроки = СтруктураПараметров.ПерваяСтрокаДанных;
	ПоследняяСтрокаДанных = СтруктураПараметров.ПоследняяСтрокаДанных;
	СоответствиеРесурсТип = СтруктураПараметров.СоответствиеРесурсТип;
	
	ТипСтрока = Тип("Строка");
	ТипЧисло = Тип("Число");
	//МетаданныеРесурсов = Метаданные.РегистрыСведений[СтруктураПараметров.ИмяРегистра].Ресурсы;
	//МетаданныеИзмерений = Метаданные.РегистрыСведений[СтруктураПараметров.ИмяРегистра].Измерения;
	
	//Если СтруктураПараметров.ИмяРегистра = "DSSSourceData" Тогда
	//	
	//	СоответствиеРесурсТип = Новый Соответствие;
	//	
	//	Для Каждого Элемент Из МетаданныеРесурсов Цикл
	//		
	//		СоответствиеРесурсТип.Вставить(Элемент.Имя, Элемент.Тип);
	//		
	//	КонецЦикла;
	//	
	//	Для Каждого Элемент Из МетаданныеИзмерений Цикл
	//		
	//		СоответствиеРесурсТип.Вставить(Элемент.Имя, Элемент.Тип);
	//		
	//	КонецЦикла;
	...
	Пока Не rs.EOF И (ТекНомерСтроки <= ПоследняяСтрокаДанных ИЛИ ПоследняяСтрокаДанных = 0) Цикл
		...
		Для каждого СтруктураКолонки Из СтруктураКолонок Цикл
			...
			Если СоответствиеРесурсТип[СтруктураКолонки.ИмяПоля].СодержитТип(ТипСтрока) 
					И НЕ СоответствиеРесурсТип[СтруктураКолонки.ИмяПоля].СодержитТип(ТипЧисло)
					И ТипЗнч(Значение) = Тип("Число") Тогда
					Значение = Формат(Значение, "ЧДЦ=0; ЧГ=0");
			ИначеЕсли ТипЗнч(Значение) = Тип("Строка") Тогда
				Значение = СокрЛП(Значение);
...
Функция СформироватьСоответствиеРесурсовИТиповРегистра(ИмяРегистра, ВключаяИзмерения) Экспорт
	
	СоответствиеРесурсТип = Новый Соответствие;
	
	МетаданныеРесурсов = Метаданные.РегистрыСведений[ИмяРегистра].Ресурсы;
	...
	Если ВключаяИзмерения Тогда
		
		МетаданныеИзмерений = Метаданные.РегистрыСведений[ИмяРегистра].Измерения;
