// Конвертация ERM (documents) от 19.12.2016 16:17:04
...
Процедура ПКО_Документ_Invoice_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	Если ДанныеИБ = Неопределено Тогда
			...
			НоваяСсылка = Документы.Invoice.ПолучитьСсылку();
			ПолученныеДанные.УстановитьСсылкуНового(НоваяСсылка);
			СсылкаНаИнвойс = ПолученныеДанные.ПолучитьСсылкуНового();
		...
		Если НЕ ПолученныеДанные.ДополнительныеСвойства.Свойство("FiscalPayments") ИЛИ ПолученныеДанные.ДополнительныеСвойства.FiscalPayments.Количество() = 0 Тогда		
			...
			Для Каждого Элемент Из Результат Цикл
				...
				Если ПоследнийСтатус <> Перечисления.InvoiceStatus.ПустаяСсылка() Тогда
					
					НаборЗаписей = РегистрыСведений.InvoiceComments.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Invoice.Установить(СсылкаНаИнвойс);
					...
					Если КомпонентыОбмена.УзелКорреспондента = rgsОбменДокументамиERMПовтИспСеанс.ПолучитьУзелHOB() Тогда
						Запись.Comment = "Loaded from HOB";	
		...
		Если Результат.Количество() > 0 Тогда
			...
			НаборЗаписейСтатусов.Отбор.Invoice.Установить(СсылкаНаИнвойс);
			...
			Если Результат.SettlementAmount >= Amount И ПоследнийСтатус <> Перечисления.InvoiceStatus.InvoicePaid Тогда
				...
				НаборЗаписейСтатусов.Записать(Ложь);
...
Процедура ПКО_Документ_SalesOrder_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	Если ДанныеИБ = Неопределено Тогда
		...
		НоваяСсылка = Документы.Invoice.ПолучитьСсылку();
		ПолученныеДанные.УстановитьСсылкуНового(НоваяСсылка);
		СсылкаНаSalesOrder = ПолученныеДанные.ПолучитьСсылкуНового();
	...
	КонецЕсли;
		...
		СтруктураРеквизитовПроблемы.Вставить("Дата", ТекущаяДата());
		...
		//ЗаполнитьЗначенияСвойств(СтруктураРеквизитовПроблемы, СсылкаНаSalesOrder);
