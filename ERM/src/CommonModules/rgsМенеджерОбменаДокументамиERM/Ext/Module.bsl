// Конвертация ERM (documents) от 03.03.2017 12:09:47
...
Процедура ДобавитьПКО_Документ_Invoice_Получение(ПравилаКонвертации)
	...
	ПравилоКонвертации.ПоляПоиска.Добавить("DocNumber,Source,Дата,Company");
	ПравилоКонвертации.ПоляПоиска.Добавить("Source,Номер,Company");
КонецПроцедуры
...
Процедура ПКО_Документ_Invoice_Получение_ПриКонвертацииДанныхXDTO(ДанныеXDTO, ПолученныеДанные, КомпонентыОбмена)
	...
	ИначеЕсли ПолученныеДанные.Source = Перечисления.ТипыСоответствий.OracleSmith И ДанныеXDTO.Свойство("ERMClientMappingID") И ДанныеXDTO.Свойство("LawsonCompanyCode") Тогда
...
Процедура ПКО_Документ_Invoice_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	Если ДанныеИБ = Неопределено Тогда
		...
		Если ДанныеИБ <> Неопределено Тогда
			...
			СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеИБ.Ссылка, "Номер, DocID, DocNumber, Amount, Agreement, AgreementCode, PassedForApproval, PassedForApprovalDate, PassedForPayment, PassedForPaymentDate, ExpectedDateOfPayment, Дата");
