// Конвертация ERM (documents) от 14.12.2016 17:46:22
...
Процедура ПКО_Документ_Invoice_Получение_ПередЗаписьюПолученныхДанных(ПолученныеДанные, ДанныеИБ, КонвертацияСвойств, КомпонентыОбмена)
	...
	Если ДанныеИБ <> Неопределено Тогда
		...
		КонецЦикла;
			...
			//Если ПолученныеДанные.ДополнительныеСвойства.Оплачен Тогда
			...
			Если FiscalPayments.Количество() > 0 Тогда
				//FiscalPayments.Свернуть("PaymentDocument,PaymentDate,Currency", "Amount");
				...
				Для Каждого Строка из FiscalPayments Цикл
					...
					Запись.Currency = Справочники.Валюты.НайтиПоКоду(Строка.Currency.Код);
