// Конвертация ERM (documents) от 19.11.2019 16:33:19
...
Процедура ЗаполнитьПравилаОбработкиДанных(НаправлениеОбмена, ПравилаОбработкиДанных) Экспорт
	Если НаправлениеОбмена = "Получение" Тогда
		...
		ДобавитьПОД_Справочник_СпособыРасчетаШтрафаПоЗадолженности_Получение(ПравилаОбработкиДанных);
	КонецЕсли;
КонецПроцедуры
...
КонецПроцедуры
#КонецОбласти
#Область Справочник_СпособыРасчетаШтрафаПоЗадолженности_Получение
Процедура ДобавитьПОД_Справочник_СпособыРасчетаШтрафаПоЗадолженности_Получение(ПравилаОбработкиДанных)
	ПравилоОбработки = ПравилаОбработкиДанных.Добавить();
	ПравилоОбработки.Имя = "Справочник_СпособыРасчетаШтрафаПоЗадолженности_Получение";
	ПравилоОбработки.ОбъектВыборкиФормат = "Справочник.СпособыРасчетаШтрафаПоЗадолженности";
	ПравилоОбработки.ИспользуемыеПКО.Добавить("Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение");
КонецПроцедуры
#КонецОбласти
#КонецОбласти
...
Процедура ЗаполнитьПравилаКонвертацииОбъектов(НаправлениеОбмена, ПравилаКонвертации) Экспорт
	Если НаправлениеОбмена = "Получение" Тогда
		...
		ДобавитьПКО_Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение(ПравилаКонвертации);
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьПКО_Документ_Invoice_Получение(ПравилаКонвертации)
	...
	НоваяСтрока = ПравилоКонвертации.Свойства.Добавить();
	НоваяСтрока.СвойствоКонфигурации = "ОтчетныйПериод_ДатаОкончания";
	НоваяСтрока.СвойствоФормата = "ОтчетныйПериод_ДатаОкончания";
	...
	Иначе
		...
		Для Каждого Строка из FiscalPayments Цикл
			...
			Иначе
				Запись.PaymentDate = Строка.PaymentDate;
			КонецЕсли;
			Запись.Prepayment = Строка.Prepayment;
			// } RGS TAlmazova 04.06.2019 17:10:05 - S-E-0001243
...
Процедура ДобавитьПКО_Справочник_ДоговорыКонтрагентов_Получение(ПравилаКонвертации)
	...
	НоваяСтрока = ПравилоКонвертации.Свойства.Добавить();
	НоваяСтрока.СвойствоКонфигурации = "NonTrade";
	НоваяСтрока.СвойствоФормата = "NonTrade";
	...
	НоваяСтрока = ПравилоКонвертации.Свойства.Добавить();
	НоваяСтрока.СвойствоКонфигурации = "ПроцентЗадолженности";
	НоваяСтрока.СвойствоФормата = "ПроцентЗадолженности";
	...
	НоваяСтрока = ПравилоКонвертации.Свойства.Добавить();
	НоваяСтрока.СвойствоКонфигурации = "СпособРасчетаШтрафаПоЗадолженности";
	НоваяСтрока.СвойствоФормата = "СпособРасчетаШтрафаПоЗадолженности";
	НоваяСтрока.ПравилоКонвертацииСвойства = "Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение";
...
КонецПроцедуры
#КонецОбласти
#Область Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение
Процедура ДобавитьПКО_Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение(ПравилаКонвертации)

	ПравилоКонвертации = ОбменДаннымиXDTOСервер.ИнициализироватьПравилоКонвертацииОбъекта(ПравилаКонвертации);
	ПравилоКонвертации.ИмяПКО = "Справочник_СпособыРасчетаШтрафаПоЗадолже_Получение";
	ПравилоКонвертации.ОбъектДанных = Метаданные.Справочники.СпособыРасчетаШтрафаПоЗадолженности;
	ПравилоКонвертации.ОбъектФормата = "Справочник.СпособыРасчетаШтрафаПоЗадолженности";
	ПравилоКонвертации.ПравилоДляГруппыСправочника = Ложь;
	ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору";
