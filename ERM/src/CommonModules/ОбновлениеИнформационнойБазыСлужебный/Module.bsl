...
Функция ТребуетсяПроверитьЛегальностьПолученияОбновления() Экспорт
	...
	// { RGS AGorlenko 29.03.2016 16:12:40 - это не нужно
	Возврат Ложь;
	// } RGS AGorlenko 29.03.2016 16:12:48 - это не нужно
		...
		Если ЭтоПодчиненныйУзелРИБСФильтром И РежимВыполнения = "Параллельно" Тогда
			Для Каждого Библиотека Из ДеревоОбработчиков.Строки Цикл
				Для Каждого Версия Из Библиотека.Строки Цикл
					...
					ОбработчикиТолькоГлавногоУзла = Версия.Строки.НайтиСтроки(ПараметрыОтбора);
