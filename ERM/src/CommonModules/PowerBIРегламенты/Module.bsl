...
Процедура PowerBIОтправкаДанных_RevenueDetails() Экспорт
	
	НачатьТранзакцию();
	
	Отчет = Отчеты.RevenueByClient.Создать();
	СКД = Отчет.СхемаКомпоновкиДанных;
	ИмяВарианта = "PowerBI_Details";
	АктивизироватьВариантОтчета(СКД, ИмяВарианта);
	АдресСхемы = ПоместитьВоВременноеХранилище(СКД, Новый УникальныйИдентификатор());
	ИсточникДоступныхНастроек = Новый ИсточникДоступныхНастроекКомпоновкиДанных(АдресСхемы);
	...
	ТЗ.Свернуть("ПериодМесяц, Source, ClientID, Client, Company, SubGeomarket, SubSegment, Account, BU, Currency, AU, Document", "Amount, USDAmount");
	
	НЗ = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_RevenueDetails.СоздатьНаборЗаписей();
...
Процедура PowerBIОтправкаДанных_BilledRevenue() Экспорт
	
	НачатьТранзакцию();
	
	Отчет = Отчеты.BilledRevenueReport.Создать();
	СКД = Отчет.СхемаКомпоновкиДанных;
	ИмяВарианта = "PowerBI";
	АктивизироватьВариантОтчета(СКД, ИмяВарианта);
	АдресСхемы = ПоместитьВоВременноеХранилище(СКД, Новый УникальныйИдентификатор());
	...
	Пока КонецМесяца(ТекПериод) <= КонецПериода Цикл
		...
		Для каждого СтрокаТЗ Из ТЗ_ТекПериод Цикл
			...
			Иначе
				ЗаписьНабора.CurrencyID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.LocationHFM) Тогда
				ЗаписьНабора.GeomarketHFM_ID = СтрокаТЗ.LocationHFM.УникальныйИдентификатор();
			Иначе
				ЗаписьНабора.GeomarketHFM_ID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.SegmentHFM) Тогда
				ЗаписьНабора.SegmentHFM_ID = СтрокаТЗ.SegmentHFM.УникальныйИдентификатор();
			Иначе
				ЗаписьНабора.SegmentHFM_ID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.AU) Тогда
			...
			Иначе
				...
				ЗаписьНабора.DocumentType = NULL;
			КонецЕсли;
			ЗаписьНабора.Amount = СтрокаТЗ.AmountОборот;
			ЗаписьНабора.AmountUSD = СтрокаТЗ.USDAmount;
...
КонецПроцедуры
// { RGS AGorlenko 05.03.2020 20:07:00 - S-E-0001471- Добавлена процедура выгрузки отчета BadDebts в PowerBI
Процедура PowerBIОтправкаBadDebts() Экспорт
	
	НачатьТранзакцию();
	
	ТекДата = НачалоМесяца(ТекущаяДата());
	НачалоПериода = ДобавитьМесяц(ТекДата, -rgsНастройкаКонфигурации.ЗначениеНастройки("PowerBI_НачалоПериодаBadDebts"));
	КонецПериода = КонецМесяца(ДобавитьМесяц(ТекДата, -rgsНастройкаКонфигурации.ЗначениеНастройки("PowerBI_КонецПериодаBadDebts")));
	
	ТекПериод = НачалоПериода;
	
	Отчет = Отчеты.BadDebts.Создать();
	СКД = Отчет.СхемаКомпоновкиДанных;
	ИмяВарианта = "PowerBI";
	АктивизироватьВариантОтчета(СКД, ИмяВарианта);
	АдресСхемы = ПоместитьВоВременноеХранилище(СКД, Новый УникальныйИдентификатор());
	...
	Пока КонецМесяца(ТекПериод) <= КонецПериода Цикл
		...
		ТЗ = Отчеты.BadDebts.СформироватьТаблицуИсточникДанных(ТекПериод, Новый Массив());
		
		ТЗ.Свернуть("Source, HFMAccount, Currency, Company, AU, CustomerCode, CustomerName, LegalEntity, BDType", "PMBalance, USDPMBalance, CMBalance, USDCMBalance");
		
		НЗ = ВнешниеИсточникиДанных.ERM_BI.Таблицы.dbo_BadDebts.СоздатьНаборЗаписей();
		...
		Для каждого СтрокаТЗ Из ТЗ Цикл
			...
			ЗаписьНабора.Period = ТекПериод;
			ЗаписьНабора.Source = Строка(СтрокаТЗ.Source);
			ЗаписьНабора.ClientNumber = СтрокаТЗ.CustomerCode;
			Если ЗначениеЗаполнено(СтрокаТЗ.CustomerName) Тогда
				ЗаписьНабора.ClientID = СтрокаТЗ.CustomerName.УникальныйИдентификатор();
			Иначе
				ЗаписьНабора.ClientID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.Company) Тогда
			...
			Иначе
				ЗаписьНабора.AU_ID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.HFMAccount) Тогда
				ЗаписьНабора.AccountID = СтрокаТЗ.HFMAccount.УникальныйИдентификатор();
			Иначе
				ЗаписьНабора.AccountID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.Currency) Тогда
			...
			Иначе
				ЗаписьНабора.CurrencyID = NULL;
			КонецЕсли;
			Если ЗначениеЗаполнено(СтрокаТЗ.BDType) Тогда
				ЗаписьНабора.BDType = СтрокаТЗ.BDType;
			Иначе
				ЗаписьНабора.BDType = NULL;
			КонецЕсли;
			
			ЗаписьНабора.PMAmount = СтрокаТЗ.PMBalance;
			ЗаписьНабора.PMAmountUSD = СтрокаТЗ.USDPMBalance;
			ЗаписьНабора.CMAmount = СтрокаТЗ.CMBalance;
			ЗаписьНабора.CMAmountUSD = СтрокаТЗ.USDCMBalance;
...
КонецПроцедуры// } RGS AGorlenko 05.03.2020 20:07:37 - S-E-0001471- Добавлена процедура выгрузки отчета BadDebts в PowerBI
