...
Процедура ПриЗаписиКонтрагентПриЗаписи(Источник, Отказ) Экспорт
	
	Узлы = rgsОбменКлиентамиERMПовтИспСеанс.ПолучитьУзлыПланаОбмена();
	
	Если Узлы = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник.Ссылка, "CRMID")) Тогда
