...
//
// Используется совместно с функцией ДвоичныеДанныеФайлов.СконвертироватьФайлыВПрисоединенные().
//
// Параметры:
...
Функция ИменаСправочниковХраненияФайлов(ВладелецФайлов, НеВызыватьИсключение = Ложь) Экспорт
	...
	ИмяСтандартногоОсновногоСправочника = МетаданныеВладельца.Имя
		+ ?(СтрЗаканчиваетсяНа(МетаданныеВладельца.Имя, "ДвоичныеДанныеФайлов"), "", "ДвоичныеДанныеФайлов");
	...
	Для каждого КлючИЗначение Из ИменаСправочников Цикл
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ДвоичныеДанныеФайлов") И Не КлючИЗначение.Ключ ="Файлы" Тогда
			
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при определении имен справочников для хранения файлов.
				           |У владельца файлов типа ""%1""
				           |указано имя справочника ""%2""
				           |без окончания ""ДвоичныеДанныеФайлов"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
...
Функция ЭтоСправочникФайловИлиВерсийФайлов(ПолноеИмя) Экспорт
	...
	Если СтрЗаканчиваетсяНа(ВРег(ЧастиИмени[1]), ВРег("ДвоичныеДанныеФайлов"))
...
Функция ПодготовитьДанныеДляСозданияФайловогоНачальногоОбраза(СтруктураПараметров) Экспорт
	...
	Узел 							= СтруктураПараметров.УзелИнформационнойБазы;
...
Функция ЕстьФайлыУВладельца(Знач ВладелецФайлов, Знач ФайлИсключение = Неопределено)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ДвоичныеДанныеФайлов.Ссылка
	|ИЗ
	|	&ИмяСправочника КАК ДвоичныеДанныеФайлов
	|ГДЕ
	|	ДвоичныеДанныеФайлов.ВладелецФайла = &ВладелецФайлов";
	
	Если ФайлИсключение <> Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + "
			|	И ДвоичныеДанныеФайлов.Ссылка <> &Ссылка";
...
Функция ПроверитьОбъектПрисоединенныхФайлов(ОбъектМетаданных)
	...
	Если СтрЗаканчиваетсяНа(ОбъектМетаданных.Имя, "ДвоичныеДанныеФайлов") Или ОбъектМетаданных.ПолноеИмя() = "Справочник.ВерсииФайлов" Тогда
		Реквизиты = ОбъектМетаданных.Реквизиты;
...
//                 СправочникСсылка.*ДвоичныеДанныеФайлов.
//
// Возвращаемое значение:
