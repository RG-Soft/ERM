...
Процедура ПрочитатьСообщениеСИзменениямиДляУзла(СтруктураНастроекОбмена, Знач ИмяФайлаСообщенияОбмена = "", СообщениеОбмена = "", Знач ТолькоПараметры = Ложь)
	...
	Иначе
		...
		Иначе // Стандартный обмен (платформенная сериализация).
			...
			СообщениеОбОшибкеДляАдминистратора = Неопределено;
			// } RGS AArsentev 08.11.2016	
			...
			// { RGS AArsentev 08.11.2016
			СтруктураНастроекОбмена.Вставить("СообщениеОбОшибкеДляАдминистратора", СообщениеОбОшибкеДляАдминистратора);
			// } RGS AArsentev 08.11.2016
		КонецЕсли;
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	Исключение
		...
		Если ЭтоОшибкаНомерСообщенияМеньшеИлиРавенНомеруРанееПринятогоСообщения(КраткоеПредставлениеОшибки(ИнформацияОбОшибке)) Тогда
			...
			// { RGS AArsentev 08.11.2016
			СообщениеОбОшибкеДляАдминистратора = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке);
			// } RGS AArsentev 08.11.2016
			//
		Иначе
	...
	Если ЧтениеСообщения.Отправитель <> УзелИнформационнойБазы Тогда // Сообщение предназначено не для этого узла.
		...
		// { RGS AArsentev 08.11.2016
		СообщениеОбОшибкеДляАдминистратора = НСтр("ru = 'Сообщение обмена содержит данные для другого узла информационной базы.'");
		// } RGS AArsentev 08.11.2016
		//
		Возврат;
	КонецЕсли;
...
Процедура ЗафиксироватьЗавершениеОбменаВРегистреСведений(СтруктураНастроекОбмена)
	...
	// { RGS AArsentev 08.11.2016
	Если СтруктураНастроекОбмена.Свойство("СообщениеОбОшибкеДляАдминистратора") Тогда
		СтруктураЗаписи.Вставить("СообщениеОбОшибкеДляАдминистратора",             СтруктураНастроекОбмена.СообщениеОбОшибкеДляАдминистратора);
	КонецЕсли;
	// } RGS AArsentev 08.11.2016
