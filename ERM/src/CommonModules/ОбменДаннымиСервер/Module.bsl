...
//
// Параметры:
//  ТекущийОбъект - ПланОбменаОбъект - записываемый УзелИнформационнойБазы плана обмена.
...
//
// Параметры:
//  Объект - ПланОбменаОбъект - УзелИнформационнойБазы плана обмена, для которого выполняется обработчик.
//  Ссылка - ПланОбменаСсылка - ссылка на УзелИнформационнойБазы плана обмена, для которого выполняется обработчик.
//
// Возвращаемое значение:
...
//
// Параметры:
//  Корреспондент - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого необходимо получить версию конфигурации.
// 
// Возвращаемое значение:
...
//
// Параметры:
//   Отправитель    - ПланОбменаСсылка - УзелИнформационнойБазы, от имени которого было сформировано и отправлено сообщение обмена.
...
//     * Отправитель                 - ПланОбменаСсылка - УзелИнформационнойБазы, от имени которого было сформировано и отправлено
//                                                        сообщение обмена.
//     * НомерПринятого              - Число            - номер принятого сообщения в базе-корреспонденте.
...
// Параметры:
//   УзелПланаОбмена         - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого необходимо получить переопределяемое
//                                                имя.
//
// Возвращаемое значение:
...
//  УзелИнформационнойБазы       - УзелОбменаСсылка - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена,
...
//     * УзелИнформационнойБазы - ПланОбменаСсылка - ссылка на УзелИнформационнойБазы плана обмена.
...
// При сохранении настроек выполняется перенос на УзелИнформационнойБазы обмена переданных данных заполнения,
...
//   * УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого выполняется сохранение настроек синхронизации.
...
//
// Параметры:
//   УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы обмена, для которого необходимо установить признак.
//
...
//
// Параметры:
//   УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы обмена, для которого необходимо получить признак.
//
// Возвращаемое значение:
...
// Возвращает ссылку на УзелИнформационнойБазы плана обмена, найденный по его коду.
// Если УзелИнформационнойБазы не найден, возвращается Неопределено.
//
// Параметры:
...
//
// Возвращаемое значение:
//  ПланОбменаСсылка - ссылка на найденный УзелИнформационнойБазы плана обмена.
//  Неопределено - в случае, если УзелИнформационнойБазы плана обмена не найден.
//
...
// Определяет, является ли переданный УзелИнформационнойБазы плана обмена автономным рабочим местом.
//
// Параметры:
//  УзелИнформационнойБазы - ПланОбменаСсылка - проверяемый УзелИнформационнойБазы.
//
// Возвращаемое значение:
//  Булево - признак того что переданный УзелИнформационнойБазы является автономным рабочим местом.
//
...
// отмеченных к обработке обновления, для передачи их в подчиненный УзелИнформационнойБазы РИБ.
//
...
Процедура ОбработатьДанныеДляОбновленияВПодчиненномУзле(Знач ЗначениеКонстанты) Экспорт
	...
	Для Каждого ХранилищеЗначений Из МассивЗначений Цикл
		...
		Если ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ОбновлениеИнформационнойБазы.Ссылка КАК УзелИнформационнойБазы
			|ИЗ
...
Функция ЗапросНаОчисткуРазрешенийИспользованияВнешнихРесурсов() Экспорт
	...
	Для Каждого ИмяПланаОбмена Из ОбменДаннымиПовтИсп.ПланыОбменаБСП() Цикл
		...
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ПланОбмена.Ссылка КАК УзелИнформационнойБазы
		|ИЗ
...
// Формирует выборку измененных данных для передачи их в тот или иной УзелИнформационнойБазы плана обмена.
...
//
// Параметры:
//  УзелИнформационнойБазы - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого выполняется итерация обмена данными.
...
// 
// Параметры:
//   УзелИнформационнойБазы - ПланОбменаСсылка - Ссылка на удаляемый УзелИнформационнойБазы плана обмена.
// 
...
// 
// Параметры:
//   УзелИнформационнойБазы - ПланОбменаСсылка - Ссылка на главный УзелИнформационнойБазы.
// 
...
//  УзелИнформационнойБазы       - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого выполняется получение сообщения
//                                                    обмена.
...
// Параметры:
//  Отказ                   - Булево - флаг отказа; поднимается в случае возникновения ошибки.
//  УзелИнформационнойБазы  - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, для которого выполняется получение сообщения обмена.
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондентаЧерезВебСервис(
	...
	Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
		...
		// Тогда псевдоним больше не нужен.
		Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
			// Следует проверить есть ли в корреспонденте такой УзелИнформационнойБазы (возможно он уже перекодировался).
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если ПараметрыОбмена.УзелИнформационнойБазы = Неопределено Тогда
		...
		Если ПараметрыОбмена.УзелИнформационнойБазы.Пустая() Тогда
			СтрокаСообщенияОбОшибке = НСтр("ru = 'УзелИнформационнойБазы плана обмена %1 с кодом %2 не найден.'");
...
//
// Параметры:
//  ПланОбмена - Ссылка на УзелИнформационнойБазы плана обмена или имя плана обмена.
//
...
// Параметры:
//
//  Получатель - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена,
...
Процедура ВыполнитьПроверкуОсновныхПолейСтруктурыНастроекОбмена(СтруктураНастроекОбмена)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураНастроекОбмена.УзелИнформационнойБазы) Тогда
		...
		// УзелИнформационнойБазы информационной базы не должен быть пустым.
		СтрокаСообщенияОбОшибке = НСтр(
		"ru = 'Не задан УзелИнформационнойБазы информационной базы с которым нужно производить обмен информацией. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура ВыполнитьПроверкуСтруктурыОбменаНаВалидность(СтруктураНастроекОбмена, ИспользоватьНастройкиТранспорта = Истина)
	...
	ИначеЕсли СтруктураНастроекОбмена.УзелИнформационнойБазы.ПометкаУдаления Тогда
		...
		// УзелИнформационнойБазы информационной базы не должен быть помечен на удаление.
		СтрокаСообщенияОбОшибке = НСтр("ru = 'УзелИнформационнойБазы информационной базы помечен на удаление. Обмен отменен.'",
			ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
...
//	УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы информационной базы из которой получен документ.
...
//	УзелОбмена - ПланОбменаСсылка - УзелИнформационнойБазы информационной базы из которой получен объект.
//	СообщениеОбОшибке - Строка - текст сообщения для журнала регистрации.
//    Рекомендуется передавать в качестве этого параметра КраткоеПредставлениеОшибки(ИнформацияОбОшибке()).
//    Текст сообщения для отображения в мониторе формируется из системных сообщений пользователю, которые
//    были сформированы, но еще не были выведены пользователю. Поэтому рекомендуется, чтобы к моменту вызова
...
//
Функция РаспаковатьTarФайл(Знач ПолноеИмяФайлаАрхива, Знач ПутьРаспаковкиФайлов, Знач ПарольАрхива = "") Экспорт
	
	// Возвращаемое значение функции.
	Результат = Истина;
	
	УтилитаАрхиватора = ПолучитьОбщийМакет("SevenZip");
	
	ИмяФайлаСкрипта = ПутьРаспаковкиФайлов + "\script.cmd";
	Скрипт = Новый ЗаписьТекста(ИмяФайлаСкрипта, КодировкаТекста.ANSI);
	Скрипт.ЗаписатьСтроку(".\7za.exe x """ + ПолноеИмяФайлаАрхива + """ -so | .\7za.exe x -aoa -si -ttar -o""" + ПутьРаспаковкиФайлов + """  2>.\log.txt");
	Скрипт.Закрыть();
	
	Попытка
		
		УтилитаАрхиватора.Записать(ПутьРаспаковкиФайлов + "\7za.exe");
		КодВозврата = Неопределено;
		ЗапуститьПриложение(ИмяФайлаСкрипта, ПутьРаспаковкиФайлов, Истина, КодВозврата);
	...
	ЧтениеЛога = Новый ЧтениеТекста;
	ЧтениеЛога.Открыть(ПутьРаспаковкиФайлов + "\log.txt");
	ТекстЛога = ЧтениеЛога.Прочитать();
	
	Результат = КодВозврата = 0 И (ПустаяСтрока(ТекстЛога) ИЛИ СтрНайти(ТекстЛога, "Everything is Ok"));
...
Процедура ВнешнееСоединениеОбновитьНастройкиОбменаДанными(Знач ИмяПланаОбмена, Знач КодУзла, Знач ЗначенияПоУмолчаниюНаУзле) Экспорт
	...
	Если Не ЗначениеЗаполнено(УзелИнформационнойБазы) Тогда
		Сообщение = НСтр("ru = 'Не найден УзелИнформационнойБазы плана обмена; имя плана обмена %1; код узла %2'");
...
// Получает УзелИнформационнойБазы распределенной информационной базы, являющийся главным для текущей информационной базы при условии,
...
//   или главный УзелИнформационнойБазы для нее не определен (она сама является корневым узлом)
...
//
//  Параметры:
//      УзелОбмена             - ПланОбменаСсылка - УзелИнформационнойБазы плана обмена, возможность выгрузки по которому проверяется.
...
//	ГлавныйУзел - ПланОбменаСсылка - УзелИнформационнойБазы из которого получено сообщение.
//
...
//
// Параметры:
//     УзелИнформационнойБазы - ПланОбменаСсылка                - ссылка на УзелИнформационнойБазы, для которого производится настройка.
