...
//							- см. РаботаСНСИПереопределяемый.СведенияОЮрФизЛице()
...
Функция СведенияОСделке(СсылкаНаОбъект) Экспорт
	...
	// { RGS AGorlenko 18.05.2016 2:07:23 - 
	...
	//КонецЕсли;
	СведенияОСчетеНаОплату = Неопределено;
	// } RGS AGorlenko 18.05.2016 2:07:23 - 
	
	СведенияОбОрганизации = РаботаСНСИПереопределяемый.СведенияОЮрФизЛице(
	...
	СведенияОКонтрагенте = РаботаСНСИПереопределяемый.СведенияОЮрФизЛице(СведенияОДоговоре.Владелец, ДатаСреза);
...
Функция ЗначениеПараметра(Знач ИмяПараметра, СведенияОСделке, ЗначенияСоставногоПараметра, Успешно) Экспорт
	...
	ИначеЕсли ИмяПараметра = Врег("ПредставительКонтрагента") Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(Руководитель) 
				...
				И СтрНайти(СведенияОКонтрагенте.ПолноеНаименование, Руководитель) = 0 Тогда
	...
	ИначеЕсли ИмяПараметра = Врег("СуммаДокумента") Тогда
		...
		Если СведенияОСчетеНаОплату <> Неопределено
			...
			Результат = ОбщегоНазначенияERMВызовСервера.ФорматСумм(
	...
	ИначеЕсли ИмяПараметра = Врег("СуммаДокументаПрописью") Тогда
		...
		Если СведенияОСчетеНаОплату <> Неопределено
			И ЗначениеЗаполнено(СведенияОСчетеНаОплату.СуммаСНДС) Тогда
			Результат = ОбщегоНазначенияERMВызовСервера.СформироватьСуммуПрописью(
				СведенияОСчетеНаОплату.СуммаСНДС, СведенияОСчетеНаОплату.ВалютаДокумента);
		Иначе
			Результат = СтроковыеФункцииКлиентСервер.СформироватьСтрокуСимволов("_", 30);
			Успешно = Ложь;
...
Функция ПредставлениеДопСвойства(СтрокаТаблицыДопСвойств)
	...
	Если ЗначениеЗаполнено(СтрокаТаблицыДопСвойств.ТекстоваяСтрока) Тогда
		Результат = СокрЛП(СтрокаТаблицыДопСвойств.ТекстоваяСтрока);

	ИначеЕсли ЗначениеЗаполнено(СтрокаТаблицыДопСвойств.Значение) Тогда
