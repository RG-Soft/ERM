...
Процедура УстановитьИдентификаторыАдреса(Адрес) Экспорт
	...
	Если ТипЗнч(КодРегиона) = Тип("Число")
		 И ЗагруженныеАдресныеСведения.Получить(КодРегиона) <> Неопределено
...
Функция АдресаДляИнтерактивногоВыбора(Родитель, Уровень, ТипАдреса, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Данные.Количество() = 0 И ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		...
		Попытка
			...
			ТекстGETЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("select?parentId=%1&level=%2&base=0&limit=%3", 
				Строка(Родитель), Строка(Уровень), КоличествоЗаписей);
...
Функция ВариантыАвтоподбора(Текст, ДополнительныеПараметры) Экспорт
	...
	ЗагруженныеАдресныеСведения = АдресныйКлассификаторПовтИсп.СведенияОЗагрузкеСубъектовРФ();
	Если ЗагруженныеАдресныеСведения["ИспользоватьЗагруженные"] Тогда
		ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Текст, ДополнительныеПараметры);
		Если Результат.Количество() > 19 Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗагруженныеАдресныеСведения["КлассификаторДоступен"] = Истина Тогда
		Количество = 20 - Результат.Количество();
		...
		ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(Результат, Текст, Родитель, Количество, ДополнительныеПараметры);
	КонецЕсли;
...
Функция КодыАдреса(Знач Адрес, Источник = Неопределено) Экспорт
	...
	Возврат КодыАдресаИКодыКЛАДР(АдресСтруктура)
...
//
Функция КодыАдресаИКодыКЛАДР(Знач Адрес, Знач ИдентификаторАдресногоОбъекта = "") Экспорт
...
//
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаВнутр(Результат, Знач Текст, ДополнительныеПараметры)
	...
	Для Каждого Строка Из РезультатЗапроса.Выгрузить() Цикл
		...
		Если НЕ ДополнительныеПараметры.Свойство("ТипАдреса") И НЕ Строка.Муниципальный Тогда
			Представление = Новый ФорматированнаяСтрока(Представление, , ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
		КонецЕсли;
		
		ДобавитьВариантАдреса(Результат, Представление, Строка.Идентификатор, Строка.Муниципальный, Истина);
	КонецЦикла;
...
Процедура ЗаполнитьРезультатПроверкиАдресаПоКлассификаторуСервис1С(Результат, Знач АдресаДляПроверки)
	...
	ПрефиксВерсииЗапроса = "rest/v1/";
	
	HTTPЗапрос = Новый HTTPЗапрос(ПрефиксВерсииЗапроса + "verify");
	ДанныеЗапроса = Новый Массив();
	ДанныеЗапроса.Добавить(АдресаДляПроверки.Адрес);
	
	АдресаДляПроверки.Адрес.addressType = ТипАдресаДляВебСервиса(АдресаДляПроверки.Адрес.addressType);
	
	Адрес = СтруктураВСтрокуJSON(ДанныеЗапроса);
	
	HTTPЗапрос.Заголовки.Вставить("Content-Type", "application/json");
	HTTPЗапрос.УстановитьТелоИзСтроки(Адрес);
	...
	Для Каждого ЭлементПроверки Из Результат.Данные Цикл
	
		Если ЭлементПроверки.status Тогда
			ОшибкаАдреса = ЭлементПроверки.errorMessage;
			Результат.Вставить("ОшибкиПроверки", ОшибкаАдреса);
		КонецЕсли;
	КонецЦикла;
...
Функция ТипАдресаДляВебСервиса(ТипАдреса)
	Если СтрСравнить(ТипАдреса, АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес()) = 0 Тогда
		Возврат "TERRITORIAL"
	КонецЕсли;
...
Процедура ЗаполнитьАдресаПоПочтовомуИндексуКлассификатораСервис1С(Результат, Индекс)
	...
	Для каждого СтрокаДанных Из РезультатВебСервис.Данные Цикл
		...
		НоваяСтрока = Результат.Данные.Добавить();
		НоваяСтрока.Представление = СтрокаДанных.value;
		НоваяСтрока.Муниципальный = (СтрСравнить(СтрокаДанных.addressType, "TERRITORIAL") <> 0);
...
// 
Процедура ЗаполнитьСписокАвтоподбораЧастиАдресаСервис1С(ВариантыАдреса, Текст, Родитель, Количество, ДополнительныеПараметры)
	...
	Если РезультатВебСервис.Количество() > 0 И РезультатВебСервис.Данные <> Неопределено Тогда
		Для каждого Данные Из РезультатВебСервис.Данные Цикл
			ЭтоМуниципальныйАдрес = (СтрСравнить(Данные.addressType, "TERRITORIAL") <> 0);
			ДобавитьВариантАдреса(ВариантыАдреса, Данные.value, Данные.id, ЭтоМуниципальныйАдрес, Ложь, СтруктураВСтрокуJSON(Данные));
		КонецЦикла;
	КонецЕсли;
	
КонецПроцедуры

Процедура ДобавитьВариантАдреса(ВариантыАдреса, Представление, Идентификатор, Муниципальный, ЗагруженныеДанные, Данные = "")
...
Процедура ЗаполнитьАктуальныеАдресныеСведенияСервис1С(Результат, Идентификатор, ЭтоМуниципальныйАдрес = Истина)
	...
	Если РезультатВебСервис.Данные.Количество() > 0 Тогда
		Если РезультатВебСервис.Данные.Количество() > 1 Тогда
			ИскомыйТипаАдреса = ?(ЭтоМуниципальныйАдрес, "MUNICIPAL", "TERRITORIAL");
			Для каждого Адрес Из РезультатВебСервис.Данные Цикл
				Если СтрСравнить(Адрес.addressType, ИскомыйТипаАдреса) = 0 Тогда
					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
			Адрес = РезультатВебСервис.Данные[0];
		КонецЕсли;
		
		Если СтрСравнить(Адрес.addressType, "TERRITORIAL") = 0 Тогда
			Адрес.addressType = АдресныйКлассификаторКлиентСервер.АдминистративноТерриториальныйАдрес();
		Иначе
			Адрес.addressType = АдресныйКлассификаторКлиентСервер.МуниципальныйАдрес();
		КонецЕсли;
		
		ОписаниеАдреса = ОписаниеАдреса();
		ЗаполнитьЗначенияСвойств(ОписаниеАдреса, Адрес);
		Для каждого ПолеАдреса Из ОписаниеАдреса Цикл
			Если ПолеАдреса.Значение = Неопределено Тогда
				ОписаниеАдреса[ПолеАдреса.Ключ] = "";
			КонецЕсли;
		КонецЦикла;
		ОписаниеАдреса.houseNumber = Адрес.house;
		
		Результат.Данные = ОписаниеАдреса;
...
Функция ОписаниеДома()
	ОписаниеДома = Новый Структура("ДомЧислом, Дом, Корпус, Строение, ВидСтроения, ВидВладения, НазваниеВладения, НазваниеСтроения, Идентификатор");
...
Процедура ДобавитьИнформациюОДоме(ВариантыДомов, ОписаниеДома, НаименованиеЗданийИСтроений, СтрокаПоиска = "")
	...
	НомерДомаИСтроения = Новый Структура("Идентификатор,Индекс,ОКТМО");
	...
	НомерДомаИСтроения.Идентификатор = ОписаниеДома.Идентификатор;
	НомерДомаИСтроения.Индекс        = ОписаниеДома.Индекс;
	НомерДомаИСтроения.ОКТМО         = ОписаниеДома.ОКТМО;
...
Функция ФайлЗагрузкиСубъектаРФ(КодСубъектаРФ, КаталогФайлов, КлючВида, ИмяОписывающегоУзла)
	...
	// Встаем на первый УзелИнформационнойБазы с данными.
	...
	Если НЕ КореньНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'В файле ""%1"" не найден корневой УзелИнформационнойБазы %2'"), Результат.ПолноеИмя, ИмяОписывающегоУзла);
	КонецЕсли;
...
Функция ВыполнитьЗапросЧерезВебСервис(СтрокаЗапроса, ВремяОжидания = 120)
	...
	Иначе
		Результат.ПодробноеПредставлениеОшибки = НСтр("ru='При получении данных из веб-сервиса возникли проблемы.'");
		Результат.Отказ = Истина;
	КонецЕсли;
...
//
Функция КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация, Представление = "")
	...
	Если ТипЗнч(КонтактнаяИнформация) = Тип("Строка") И ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнформация) Тогда
