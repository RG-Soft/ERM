...
Функция ПроверитьКонтрольныйКлючВНомереБанковскогоСчета(Знач НомерСчета, Знач БИК) Экспорт
	...
	ЗаведомоЦифровыеСимволы = УсловныйНомер + Лев(НомерСчета, 5) + Сред(НомерСчета, 7);
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(АлфавитныйСимвол) Тогда
		ДопустимыеАлфавитныеСимволы = "ABCEHKMPTX";
		ЦифраШестогоРазряда = СтрНайти(ДопустимыеАлфавитныеСимволы, АлфавитныйСимвол);
...
Функция ЭтоБанковскийОрдер(ШифрОперации) Экспорт
	...
	Возврат ШифрОперации = 9   // 09 - Списано, зачислено по мемориальному ордеру
		Или ШифрОперации = 17; // 17 - Списано, зачислено по банковскому ордеру
...
Функция ПривестиКДлинеСчетаРФ(НомерСчета) Экспорт
	
	ДлинаСчетаРФ = БанковскиеПравила.ДлинаНомераСчета();
	ТипСтрока = Новый ОписаниеТипов("Строка", Новый КвалификаторыСтроки(ДлинаСчетаРФ));
	
	Возврат ТипСтрока.ПривестиЗначение(НомерСчета);
...
Функция ПроверитьКонтрольныйКлючIBAN(IBAN) Экспорт
	...
	ТипЧисло = Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла());
	ПроверяемыйIBAN = ТипЧисло.ПривестиЗначение(ПроверяемыйIBAN);
	
	// 3. Применяем Mod 97-10 (ISO 7064).
	// Если остаток равен 1 Тогда
	// это признак того, что счет корректный.
	
	Возврат ПроверяемыйIBAN % 97 = 1;
