...
Процедура ПриПолученииДанныхОтПодчиненного(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, Отправитель) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("РегистрСведенийНаборЗаписей.ВерсииОбъектов") И ЭлементДанных.Количество() > 0 Тогда
		...
		Если НомерВерсииСКоллизией = Неопределено Тогда
			...
			Запись.УзелИнформационнойБазы = ОбщегоНазначения.ПредметСтрокой(Запись.АвторВерсии);
...
Процедура ДобавитьИнформациюОбУзле(Запись, Получатель)
	...
	Если Запись.УзелИнформационнойБазы = ОбщегоНазначения.ПредметСтрокой(Получатель.Ссылка) Тогда
	...
	Иначе
		Если ПустаяСтрока(Запись.Узел) Тогда
			...
			Запись.УзелИнформационнойБазы = ОбщегоНазначения.ПредметСтрокой(ПланОбменаМенеджер.ЭтотУзел());
		КонецЕсли;
	КонецЕсли;
...
Функция ВерсииКВыгрузке(Объект, Узел)
	...
	|	И ВерсииОбъектовИзменения.УзелИнформационнойБазы = &УзелИнформационнойБазы
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерВерсии УБЫВ";
