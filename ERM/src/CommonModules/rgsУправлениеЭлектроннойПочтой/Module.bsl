...
Процедура РегламентDisputeNotifications() Экспорт
	...
	Пока ВыборкаДанныхДляНотификации.Следующий() Цикл
		...
		ЗаполнитьЗначенияСвойств(НовоеПисьмо, СтароеПисьмо, , "Дата, Номер");
		...
		ДобавитьКомментарийОбЭскалации(ВыборкаДетальныеЗаписи.Проблема, НовоеПисьмо.Дата);
...
Процедура ДобавитьКомментарийОбЭскалации(СтарыйКомментарий, ДатаНовогоКомментария)
	
	СтарыйКомментарийОбъект = СтарыйКомментарий.ПолучитьОбъект();
	НовыйКомментарий = Документы.InvoiceProblem.СоздатьДокумент();
	ЗаполнитьЗначенияСвойств(НовыйКомментарий, СтарыйКомментарийОбъект, , "Дата, Номер, RemedialWorkPlan, Comment");
