...
Функция ИзменитьЭлементыОтбора(ОбластьПоиска,
	...
	// { RGS agorlenko 28.12.2018 13:22:22
	//Если ЗначениеЗаполнено(ИмяПоля) Тогда
	Если СтрНачинаетсяС(Представление, "@ПоискПоИмениПоляИПредставлению") Тогда
		ЗначениеПоиска = Новый Структура("ИмяПоля, Представление", Новый ПолеКомпоновкиДанных(ИмяПоля), Представление);
		СпособПоиска = 3;
		...
		Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			ИначеЕсли СпособПоиска = 2 Тогда
				Если ЭлементОтбора.Представление = ЗначениеПоиска Тогда
					МассивЭлементов.Добавить(ЭлементОтбора);
				КонецЕсли;
			// { RGS agorlenko 28.12.2018 13:19:56
			ИначеЕсли СпособПоиска = 3 Тогда
				Если ЭлементОтбора.ЛевоеЗначение = ЗначениеПоиска.ИмяПоля
					И ЭлементОтбора.Представление = ЗначениеПоиска.Представление Тогда
					МассивЭлементов.Добавить(ЭлементОтбора);
				КонецЕсли;
			// } RGS agorlenko 28.12.2018 13:19:56
			КонецЕсли;
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	Если ЭтоWindows Тогда
		ФактНедоступности = (СтрНайти(ЖурналДоступности, "Заданный УзелИнформационнойБазы недоступен") > 0)
